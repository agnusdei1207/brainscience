...
- [`[딥러닝 가속기 (NPU)]`](@/studynotes/1_computer_architecture/12_ai_hardware/_index.md): 현대 아키텍처가 AI 워크로드를 처리하기 위해 진화한 도메인 특화 구조(DSA).
- [`[암달의 법칙 (Amdahl's Law)]`](@/studynotes/1_computer_architecture/3_performance/_index.md): 병렬화를 통한 성능 향상의 한계를 정의하는 핵심 수학 공식.
- [`[해밍 코드 (Hamming Code)]`](@/studynotes/1_computer_architecture/2_arithmetic/111_hamming_code.md): 메모리의 신뢰성을 보장하기 위한 하드웨어 수준의 에러 정정(ECC) 알고리즘.
