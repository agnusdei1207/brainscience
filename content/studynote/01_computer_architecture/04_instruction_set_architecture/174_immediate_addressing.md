...
> **핵심**: 즉시 주소 지정(Immediate Addressing)은 명령어의 피연산자 필드에 **실제 연산에 사용할 데이터(상수)를 직접 포함**시키는 방식이다.
...
- **융합**: 연산 장치(ALU)로 데이터를 즉시 공급하는 하드웨어 경로와 융합되어 있으며, 명령어 비트 내의 한정된 공간 때문에 **표현할 수 있는 숫자의 크기가 제한**된다는 특징을 갖는다.
...
**핵심 조건**: - **컴파일러 최적화 (Constant Folding)**: 컴파일러는 `x = 1 + 2` 같은 코드를 보면, 굳이 실행 시간에 더하지 않고 미리 `3`으로 계산하여 **즉시 주소 지정** 방식으로 구워낸다. 하드웨어의 즉시 모드를 최대한 활용하여 소프트웨어의 짐을 덜어주는 고도의 융합 지능이다.
...
즉시 주소 지정 (Immediate)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 즉시 주소 지정 (Immediate)을 선택할 근거가 분명할 때
...
| 개념 | 연결 포인트 |
|:---|:---|
| 즉시 주소 지정 (Immediate) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
