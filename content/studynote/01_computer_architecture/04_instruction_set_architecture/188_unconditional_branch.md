...
핵심 해설:
- **가치**: 주로 루프(Loop)의 맨 끝에서 다시 처음으로 돌아가거나, 함수(Function) 실행이 끝난 후 원래의 자리로 복귀(Return)할 때 사용되어 **코드의 반복과 재사용**을 물리적으로 연결한다.
- **융합**: 어셈블리어의 꽃이자 독인 `GOTO` 문의 실체이며, 남용할 경우 코드 흐름을 완전히 박살 내는 스파게티 코드를 유발하므로 현대 컴파일러에서는 **철저히 숨겨진 채 구조적 제어의 뒤치다꺼리**를 담당한다.
...
**핵심 조건**: 무조건 분기라도 목적지가 언제 결정되느냐에 따라 난이도가 천지 차이다.
...
무조건 분기 (Unconditional Branch)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 무조건 분기 (Unconditional Branch)을 선택할 근거가 분명할 때
...
주소 지정·레지스터 체계 확립
    ↓
무조건 분기 (Unconditional Branch) 구체화
    ↓
ISA 확장·벡터화
    ↓
도메인 특화 명령어로 진화
...
| 개념 | 연결 포인트 |
|:---|:---|
| 무조건 분기 (Unconditional Branch) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
