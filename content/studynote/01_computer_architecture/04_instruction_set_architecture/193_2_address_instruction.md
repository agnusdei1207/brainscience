...
> **핵심**: 2-주소 명령어(Two-Address Instruction)는 명령어 포맷 안에 데이터를 가리키는 **주소 필드가 2개(Src, Dest 겸용)** 존재하는 명령어 형식이다.
...
핵심 해설:
- **가치**: `A = A + B`처럼 두 개의 피연산자 중 하나를 연산의 목적지(Destination)로 재활용함으로써, 3-주소 명령어보다 **명령어 길이를 절약하면서도 누산기(1-주소)의 병목을 해결**한 절충형 아키텍처다.
- **융합**: 인텔의 **x86 아키텍처(CISC)**를 지배하는 핵심 명령어 포맷으로, "오른쪽 값을 왼쪽에 더해서 덮어써라"라는 이 파괴적 대입(Destructive Assignment) 방식이 오늘날 PC 생태계의 소프트웨어 문법을 결정지었다.
...
|:---|:---|:---|:---|
**핵심 조건**: | **명령어 의미** | `ADD A, B` ─▶ $A = A + B$ | `ADD A, B, C` ─▶ $A = B + C$| Destructive vs Non-destructive |
...
2-주소 명령어를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 2-주소 명령어을 선택할 근거가 분명할 때
...
| 개념 | 연결 포인트 |
|:---|:---|
| 2-주소 명령어 | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
