...
> **핵심**: CISC(Complex Instruction Set Computer)는 하나의 명령어가 길고 복잡한 작업을 한 번에 처리할 수 있도록, **가변 길이 명령어 포맷**과 **다양한 주소 지정 방식(메모리 연산 포함)**을 제공하는 아키텍처다.
...
핵심 해설:
- **가치**: 컴파일러 기술이 미숙하고 메모리가 비쌌던 시절, 소프트웨어 개발자(어셈블리 프로그래머)의 부담을 덜어주고 **코드의 메모리 차지 용량을 극한으로 압축**하는 데 가장 큰 공헌을 했다.
- **융합**: 초고속 병렬 실행(파이프라인)의 한계에 부딪혔으나, 현대 x86(Intel/AMD) 칩은 외부로는 CISC를 유지하고 내부적으로는 RISC 형태의 **마이크로-옵(Micro-op)으로 변환**하여 실행하는 하이브리드 진화를 통해 데스크탑/서버 시장을 지배하고 있다.
...
**핵심 조건**: CISC (Complex Instruction Set Computer) 적용 시 입력 조건, 내부 상태, 제약 자원이 함께 맞물려야 기대한 동작이 성립한다.
...
CISC (Complex Instruction Set Computer)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 CISC (Complex Instruction Set Computer)을 선택할 근거가 분명할 때
...
주소 지정·레지스터 체계 확립
    ↓
CISC (Complex Instruction Set Computer) 구체화
    ↓
ISA 확장·벡터화
    ↓
도메인 특화 명령어로 진화
...
| 개념 | 연결 포인트 |
|:---|:---|
| CISC (Complex Instruction Set Computer) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
