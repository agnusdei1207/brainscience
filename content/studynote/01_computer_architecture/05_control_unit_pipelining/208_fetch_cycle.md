...
> **핵심**: 인출 사이클 (Fetch Cycle)은 CPU가 프로그램 카운터 (PC)가 가리키는 메모리 주소에서 실행할 명령어를 읽어와 명령어 레지스터 (IR)에 저장하는 명령어 사이클의 첫 번째 단계다.
...
핵심 해설:
- **가치**: 명령어의 연산 종류와 무관하게 모든 명령어에 대해 공통적이고 필수적으로 수행되는 과정으로, CPU 외부의 메모리와 통신해야 하므로 폰 노이만 병목을 유발하는 주범이기도 하다.
- **융합**: 느린 메모리와 빠른 CPU 간의 속도 차이를 극복하기 위해, 하드웨어 분기 예측 (Branch Prediction)과 결합된 L1 명령어 캐시 (L1i Cache) 및 프리페처 (Prefetcher) 구조를 통해 성능 저하를 방어한다.
...
**핵심 조건**: | **접근 대상** | 무조건 명령어 (Instruction / Code) | 명령어에 따라 일반 데이터 (Data)를 읽거나 씀 |
...
인출 사이클 (Fetch Cycle)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 인출 사이클 (Fetch Cycle)을 선택할 근거가 분명할 때
...
| 개념 | 연결 포인트 |
|:---|:---|
| 인출 사이클 (Fetch Cycle) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
