...
> **핵심**: 간접 사이클 (Indirect Cycle)은 명령어의 피연산자가 실제 데이터의 위치가 아니라 '데이터의 진짜 주소가 저장된 메모리의 주소'를 가리킬 때, 진짜 유효 주소 (Effective Address)를 얻기 위해 메모리를 한 번 더 읽어오는 과정이다.
...
**핵심 조건**: 간접 사이클 (Indirect Cycle) 적용 시 입력 조건, 내부 상태, 제약 자원이 함께 맞물려야 기대한 동작이 성립한다.
...
간접 사이클 (Indirect Cycle)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 간접 사이클 (Indirect Cycle)을 선택할 근거가 분명할 때
...
| 개념 | 연결 포인트 |
|:---|:---|
| 간접 사이클 (Indirect Cycle) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
