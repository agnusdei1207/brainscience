...
> **핵심**: SRAM (Static Random Access Memory)은 전원이 공급되는 한 데이터를 지속적으로 유지하는 **쌍안정(Bistable) 플립플롭(Flip-Flop) 논리 게이트 기반의 초고속 휘발성 메모리**다.
...
- **융합**: 그러나 1비트를 저장하는 데 6개의 트랜지스터(6T)가 필요하여 집적도가 낮고 가격이 비싸기 때문에, 메인 메모리로는 사용되지 못하고 오직 **CPU 내부의 L1/L2/L3 캐시(Cache) 메모리**라는 특수 목적 생태계를 독점하고 있다.
...
| 요소 | 역할 | 핵심 포인트 |
|:---|:---|:---|
| 핵심 대상 | SRAM (Static RAM)에서 직접 다루는 연산, 자원, 상태, 지표 | 문제 정의와 측정 기준을 먼저 명확히 해야 함 |
...
|:---|:---|:---|
**핵심 조건**: | **저장 원리** | 트랜지스터 래치의 플립플롭 쌍안정 상태 | 커패시터의 전하 충전/방전 여부 |
...
SRAM (Static RAM)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 SRAM (Static RAM)을 선택할 근거가 분명할 때
...
| 개념 | 연결 포인트 |
|:---|:---|
| SRAM (Static RAM) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
