...
> **핵심**: 캐시 메모리(Cache Memory)는 CPU 연산 유닛과 메인 메모리(DRAM) 사이에 위치하여, 메인 메모리의 데이터 중 **가장 자주 쓰일 법한 데이터(복사본)를 CPU 곁에 미리 가져다 두는 초고속 SRAM 버퍼**다.
...
| 요소 | 역할 | 핵심 포인트 |
|:---|:---|:---|
| 핵심 대상 | 캐시 메모리 (Cache Memory)에서 직접 다루는 연산, 자원, 상태, 지표 | 문제 정의와 측정 기준을 먼저 명확히 해야 함 |
...
**핵심 조건**: 캐시 메모리 (Cache Memory) 적용 시 입력 조건, 내부 상태, 제약 자원이 함께 맞물려야 기대한 동작이 성립한다.
...
캐시 메모리 (Cache Memory)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 캐시 메모리 (Cache Memory)을 선택할 근거가 분명할 때
...
| 개념 | 연결 포인트 |
|:---|:---|
| 캐시 메모리 (Cache Memory) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
