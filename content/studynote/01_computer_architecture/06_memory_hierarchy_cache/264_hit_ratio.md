...
> **핵심**: 적중률(Hit Ratio)은 CPU가 요청한 데이터가 캐시 메모리에 존재하여(Hit) 하위 메모리로 내려가지 않고 즉시 반환된 횟수를 전체 요청 횟수로 나눈 **캐시 메모리의 효율성과 생존력을 나타내는 절대 성능 지표**다.
...
| 요소 | 역할 | 핵심 포인트 |
|:---|:---|:---|
| 핵심 대상 | 적중률 (Hit Ratio)에서 직접 다루는 연산, 자원, 상태, 지표 | 문제 정의와 측정 기준을 먼저 명확히 해야 함 |
...
|:---|:---|:---|
**핵심 조건**: | **강제 미스 (Compulsory)** | 프로그램을 처음 켤 때, 캐시가 비어 있어서 **무조건 겪는 1번째 헛방** | 하드웨어 프리페처가 CPU보다 먼저 데이터를 퍼와서 방어 |
...
적중률 (Hit Ratio)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 적중률 (Hit Ratio)을 선택할 근거가 분명할 때
...
| 개념 | 연결 포인트 |
|:---|:---|
| 적중률 (Hit Ratio) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
