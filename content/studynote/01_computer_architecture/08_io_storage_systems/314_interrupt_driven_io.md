...
> **비유**: Interrupt-driven I/O는 조리사가 음식을 다 끝내면 손을 들고 "다 됐습니다!(Interrupt)"라고 종을 쳐서(신호) 매니저(CPU)에게 바로 알려주는 방식이다. 매니저가 그때마다 조리사에게 가서 확인하고 오는 수고를 줄였다.
...
핵심 해설:
- **가치**: 고속 I/O 환경에서 폴링의 Busy-waiting으로 인한 CPU 낭비를 제거하여, 동일 시간에 더 많은 연산을 완료할 수 있게 하며, 시스템 전체 처리량(Throughput)을 획기적으로 향상시켰다.
- **융합**: DMA, APIC, MSI(Message Signaled Interrupt) 등 다른 하드웨어 메커니즘과 결합되어 현대 컴퓨터의 I/O 처리 표준을 이루며, OS의 스케줄러와 긴밀하게 통합된다.
...
**핵심 조건**: → Interrupt 빈도 = 1,000,000 interrupts/sec
...
인터럽트 구동 I/O (Interrupt-driven I/O)를 비교할 때는 단순한 우열보다 어떤 제약 조건에서 어느 방식이 더 적합한지 판단하는 것이 중요하다. 같은 기술도 워크로드, 전력 예산, 검증 난이도, 호환성에 따라 최적 해가 달라진다.
...
**채택 조건**:
- 시스템 병목이나 구조적 한계를 줄이는 수단으로 인터럽트 구동 I/O (Interrupt-driven I/O)을 선택할 근거가 분명할 때
...
인터럽트·DMA 도입
    ↓
인터럽트 구동 I/O (Interrupt-driven I/O) 체계화
    ↓
대용량 저장·네트워크 스토리지 확장
    ↓
고속 저지연 장치 통합
...
| 개념 | 연결 포인트 |
|:---|:---|
| 인터럽트 구동 I/O (Interrupt-driven I/O) | 해당 주제가 다루는 핵심 메커니즘과 설계 판단의 중심축 |
