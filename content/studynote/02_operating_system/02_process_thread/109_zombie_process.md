...
> **핵심**: 좀비 프로세스 (Zombie Process)는 자식 프로세스가 실행을 종료했으나, 부모 프로세스가 `wait()` 또는 `waitpid()` 시스템 콜을 호출하여 종료 상태(Exit Status)를 회수하지 않아 프로세스 테이블 (Process Table)에 항목이 남아 있는 상태를 말한다.
