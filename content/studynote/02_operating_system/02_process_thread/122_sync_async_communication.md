...
> **핵심**: 동기식/비동기식 통신은 메시지 전달에서 송신(Send)과 수신(Receive) 연산이 호출 프로세스를 블로킹(Blocking)시킬 것인지, 논블로킹(Non-blocking)으로 즉시 반환할 것인지에 따라 구분되는 통신 동기화 모델이다.
