...
> **핵심**: 좀비 스레드 (Zombie Thread)는 실행이 종료되었으나 부모 스레드나 프로세스가 `pthread_join()` (또는 유사한 대기 함수)을 호출하여 스레드의 종료 상태(Exit Status)를 수집하지 않아, 커널 내부에 TCB (Thread Control Block) 및 스택 메모리가 해제되지 않고 잔류하는 스레드를 의미한다.
> **비유**: 좀비 스레드 (Zombie Thread)은(는) 복잡한 교통을 한 번에 통제해 전체 흐름을 맞추는 신호 체계와 같다.
...
좀비 스레드 (Zombie Thread)은(는) 단일 기능이 아니라 자원 효율, 격리, 응답성, 복구 가능성을 함께 결정하는 운영체제의 설계 축이다. 따라서 성능만 보고 채택하기보다 실패 비용, 관리 복잡도, 워크로드 특성을 함께 평가해야 한다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| 좀비 스레드 (Zombie Thread) | 운영체제 자원 관리와 보호 정책이 실제로 적용되는 핵심 주제 |
...
|:---|:---|:---|
| 설계 초점 | 좀비 스레드 (Zombie Thread)의 핵심 정책을 직접 제어 | 범용 커널 정책 또는 다른 자원 관리 기법과 연계 |
| 의사결정 영향 | 성능·보호·응답 시간 최적화에 직접 영향 | 운영 단순성과 호환성 측면에서 비교 필요 |

좀비 스레드 (Zombie Thread)은(는) 독립 개념처럼 보이지만 스케줄링, 메모리, I/O, 보안 정책과 맞물릴 때 실제 효과가 결정된다. 따라서 비교의 핵심은 기능 유무보다 어떤 시스템 목표를 우선하는지에 있다.
...
**채택 조건**:
- 좀비 스레드 (Zombie Thread)이 시스템의 성능, 안정성, 격리 수준을 직접 좌우하는 핵심 병목일 때
...
실제 사례: 좀비 스레드 (Zombie Thread)은(는) 리눅스 커널, 서버 워크로드, 컨테이너 기반 인프라에서 성능과 안정성을 함께 조정하는 대표 운영체제 판단 항목으로 활용된다.
