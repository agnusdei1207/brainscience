...
> **핵심**: 스레드 안전(Thread-safe)이란 여러 스레드(Thread)가 동시에 같은 함수나 자원을 호출해도 **경쟁 조건(Race Condition) 없이 항상 올바른 결과를 보장**하는 성질이다.
> **비유**: 스레드 안전 (Thread-safe) 함수 및 라이브러리은(는) 복잡한 교통을 한 번에 통제해 전체 흐름을 맞추는 신호 체계와 같다.
