...
> **핵심**: 우선순위 스케줄링 (Priority Scheduling)은 각 프로세스의 PCB(Process Control Block)에 중요도를 나타내는 정수값(우선순위)을 부여하고, Ready 큐에서 **가장 높은 우선순위를 가진 프로세스에게 무조건 먼저 CPU를 할당**하는 권력 지향적 알고리즘이다.
