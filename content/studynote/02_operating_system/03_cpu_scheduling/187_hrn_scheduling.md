...
> **핵심**: HRN (Highest Response Ratio Next) 스케줄링은 가장 짧은 작업을 편애하여 무거운 작업이 영원히 굶어 죽는 SJF의 치명적 결함(기아 상태)을 보완하기 위해, **"대기 시간"과 "실행 시간"을 동시에 고려한 응답 비율(Response Ratio)** 공식으로 우선순위를 매기는 비선점형 알고리즘이다.
