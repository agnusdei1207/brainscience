...
> **핵심**: 실시간 스케줄링 (Real-time Scheduling)은 프로세스가 아무리 빠르게 끝나는가(속도)보다, 시스템과 약속한 **주어진 마감 시간 (Deadline) 이내에 100% 확실하게 논리적/물리적 연산을 완료하는가(예측 가능성, Determinism)**를 절대적 최우선 목표로 삼는 아키텍처다.
