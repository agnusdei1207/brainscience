...
> **핵심**: 리눅스의 CFS(Completely Fair Scheduler)는 전통적인 우선순위 큐와 타임 슬라이스(Time Quantum) 개념을 폐기하고, 오직 **'가상 실행 시간(vruntime, Virtual Runtime)'이라는 단일 지표를 레드-블랙 트리(Red-Black Tree)에 정렬**하여 가장 적게 실행된 프로세스에게 무조건 CPU를 내어주는 혁명적인 스케줄러다.
