...
> **핵심**: 경쟁 조건 (Race Condition)은 두 개 이상의 프로세스나 스레드가 하나의 공유 자원(메모리, 파일)에 동시에 접근하여 수정하려 할 때, **실행(스케줄링)의 타이밍이나 순서에 따라 최종 결과값이 달라지는 치명적인 비결정적(Non-deterministic) 버그 현상**이다.
