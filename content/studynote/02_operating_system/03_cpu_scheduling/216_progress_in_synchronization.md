...
> **핵심**: 진행 (Progress)은 임계 구역(Critical Section) 보호 알고리즘이 갖춰야 할 3대 필수 조건 중 두 번째로, **"임계 구역이 비어 있고 들어가길 원하는 프로세스들이 있다면, 그중 하나는 반드시 무한한 지연 없이 진입해야 한다"**는 활력(Liveness) 보장 원칙이다.
