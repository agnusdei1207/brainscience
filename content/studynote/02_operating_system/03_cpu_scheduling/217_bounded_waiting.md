...
> **핵심**: 한정된 대기 (Bounded Waiting)는 임계 구역(Critical Section) 보호 알고리즘의 3대 필수 조건 중 마지막으로, 프로세스가 임계 구역에 진입하겠다고 요청한 시점부터 허락될 때까지 **다른 프로세스들이 새치기해서 먼저 들어가는 '횟수'에 수학적인 한계(Bound)가 있어야 한다**는 원칙이다.
