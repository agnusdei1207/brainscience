...
> **핵심**: 소프트웨어적 동기화 해결책은 CPU의 특수한 하드웨어 명령어(TAS, CAS 등) 지원이나 운영체제 커널의 개입 없이, 오직 **프로그래머가 작성한 일반적인 전역 변수(`flag`, `turn`)의 읽기/쓰기 논리만으로 임계 구역(Critical Section)을 보호**하려는 초창기 컴퓨터 과학의 순수 논리적 접근법이다.
