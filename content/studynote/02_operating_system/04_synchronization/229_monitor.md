...
> **핵심**: 모니터 (Monitor)는 개발자가 수동으로 세마포어나 뮤텍스를 쥐락펴락하다가 데드락(Deadlock)을 내는 휴먼 에러를 막기 위해, **공유 자원과 그 자원을 다루는 함수들을 하나의 거대한 캡슐(객체)로 묶고 프로그래밍 언어 차원에서 자동 락(Lock)을 걸어주는 고수준 동기화 구조**다.
