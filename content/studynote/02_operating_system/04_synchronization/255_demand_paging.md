...
> **핵심**: 요구 페이징은 프로그램이 실행될 때 파일 전체를 메모리(RAM)에 한 번에 올리는 무식한 방식을 버리고, **CPU가 실제로 "이 주소 내놔!"라고 요구(Demand)하는 순간에만 해당 데이터가 있는 작은 조각(Page)을 디스크에서 램으로 퍼 나르는 지연 적재(Lazy Loading) 기법**이다.
