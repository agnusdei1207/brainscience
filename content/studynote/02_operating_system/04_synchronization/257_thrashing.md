...
> **핵심**: 스래싱 (Thrashing)은 다중 프로그래밍 환경에서 너무 많은 프로세스가 동시에 실행되어 물리적 램(RAM)이 고갈되었을 때, CPU가 실제 연산(유저 코드)은 전혀 하지 못하고 **디스크와 램 사이에서 페이지(데이터)를 교체하는 데만 시스템 자원을 100% 탕진하는 붕괴 상태**다.
