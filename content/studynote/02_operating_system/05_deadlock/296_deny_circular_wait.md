...
> **핵심**: 교착 상태 예방 관점에서의 순환 대기 부정 (Deny Circular Wait) 기법은 시스템 내의 모든 자원(Lock)에 고유한 일련번호(ID)를 부여하고 무조건 **오름차순(Ascending order)**으로만 자원을 요청하도록 애플리케이션에 룰을 강제하는 체계다(Lock Hierarchy).
