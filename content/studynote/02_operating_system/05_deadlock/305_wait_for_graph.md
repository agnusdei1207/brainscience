...
> **핵심**: 대기 그래프(Wait-for Graph)는 교착 상태를 시각화한 원본 '자원 할당 그래프(RAG)'에서 **'자원 정점(네모 상자)'을 모두 지워버리고**, 오직 "어떤 프로세스가 어떤 프로세스의 완료를 멱살 쥐듯 기다리고 있는가"라는 **사람(프로세스) 간의 직접적인 원한 관계(의존성)만 남긴 초경량 압축 위상 지도**다.
