...
> **핵심**: 교착 상태를 복구(자원 뺏기)할 때 `비용 공식(Min Cost)`이 철저하게 최적화될수록, 매번 데드락이 터질 때마다 **재수 없이 똑같이 싼 비용의 타겟 1명**만 계속 핀포인트로 희생되어 영원히 자원을 못 잡고 굶어 죽어가는 **기아 역병(Starvation)**을 원천 차단하는 브레이크 메커니즘이다.
