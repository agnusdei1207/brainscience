...
> **핵심**: 베이스 레지스터 (Base Register) 또는 재배치 레지스터 (Relocation Register)는 다중 프로그래밍 환경에서 프로세스가 물리 메모리의 어느 위치에 적재되었는지, 그 **시작 기준 주소(Base Address)**를 저장하는 특수 목적 하드웨어 레지스터이다.
