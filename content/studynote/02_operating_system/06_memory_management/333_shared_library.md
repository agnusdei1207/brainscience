...
> **핵심**: 공유 라이브러리 (Shared Library)는 메모리에 단 하나만 적재되어 여러 프로세스가 동시에 사용할 수 있도록 설계된 코드 덩어리이며, 스터브 (Stub)는 실행 파일이 이 공유 라이브러리의 **실제 메모리 주소를 런타임에 찾을 수 있도록 돕는 작은 연락처 코드(포인터)**이다.
