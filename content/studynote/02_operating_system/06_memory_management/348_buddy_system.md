...
> **핵심**: 버디 시스템(Buddy System) 할당기는 메모리를 요청받았을 때, 전체 물리 메모리를 **무조건 2의 승수(2^k) 크기로만 반씩 쪼개어(Split) 할당**하고, 반환될 때는 쪼개졌던 **자신의 쌍둥이 조각(Buddy)과 초고속으로 병합(Coalescing)하는 커널 메모리 할당 기법**이다.
