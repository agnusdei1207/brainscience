...
> **핵심**: CPU가 요구한 가상 페이지 번호(p)가 TLB 하드웨어 캐시 안에 이미 존재하여 램(RAM) 접근 없이 즉시 물리 프레임을 알아내는 것을 **TLB 적중(Hit)**이라 하고, 캐시에 없어 어쩔 수 없이 램의 장부를 뒤지러 가야 하는 상황을 **TLB 미스(Miss)**라고 한다.
