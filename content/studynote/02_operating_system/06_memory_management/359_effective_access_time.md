...
> **핵심**: TLB 적중률(Hit Ratio)은 CPU가 요청한 가상 주소를 캐시(TLB)에서 1번 만에 찾아내는 성공 확률이며, 실질 메모리 접근 시간(EAT: Effective Access Time)은 히트 시의 빠른 시간과 미스 시의 느린 시간을 확률로 가중 평균 낸 **'실제 사용자가 체감하는 평균 메모리 속도'**를 의미한다.
