...
> **핵심**: 역 페이지 테이블(Inverted Page Table)은 "프로세스마다 가상 주소(Page) 기준의 거대한 장부를 하나씩 가진다"는 페이징의 절대 원칙을 부수고, **물리 메모리의 프레임(Frame) 개수와 똑같은 단 하나의 전역 장부**만을 시스템 전체에 두는 상식을 파괴한 아키텍처다.
