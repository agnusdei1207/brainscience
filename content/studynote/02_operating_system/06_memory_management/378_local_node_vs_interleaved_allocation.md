...
> **핵심**: NUMA 멀티코어 서버 환경에서 운영체제가 프로세스에게 물리 메모리(RAM)를 나누어 줄 때, **무조건 내 CPU와 가장 가까운 램 한곳에 몰빵하는 '로컬 노드 할당(Local Node Allocation)'**과 **여러 노드의 램에 카드를 섞듯 번갈아 가며 뿌려주는 '인터리브 할당(Interleaved Allocation)'**의 두 가지 상반된 철학적 선택지다.
