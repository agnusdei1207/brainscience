...
> **핵심**: 가상 메모리(Virtual Memory)는 프로세스 전체가 메모리에 한 번에 적재되어야 한다는 과거의 강박을 버리고, **프로그램의 일부만 물리 메모리(RAM)에 올린 채 나머지는 디스크(HDD/SSD)에 두고 필요할 때마다 교체하며 실행하는 운영체제의 핵심 추상화 기술**이다.
