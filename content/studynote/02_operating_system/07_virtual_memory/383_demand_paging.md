...
> **핵심**: 요구 페이징(Demand Paging)은 프로세스를 시작할 때 거대한 프로그램 전체를 램(RAM)에 미리 올려두는 짓을 금지하고, **CPU가 코드를 실행하다가 정말로 "필요해서 달라고 요구(Demand)"하는 그 순간에만, 딱 그 4KB 페이지 조각 하나만 디스크에서 램으로 가져오는 극한의 게으른(Lazy) 적재 기법**이다.
