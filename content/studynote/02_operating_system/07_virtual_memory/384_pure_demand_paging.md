...
> **핵심**: 순수 요구 페이징(Pure Demand Paging)은 프로세스를 시작할 때 **물리 메모리(RAM)에 단 1장의 페이지조차 미리 올려두지 않고 텅 빈 채로 강제 실행**시켜, 첫 명령어부터 무자비하게 페이지 폴트(Page Fault)를 터뜨리며 시작하는 극단적인 요구 페이징 전략이다.
