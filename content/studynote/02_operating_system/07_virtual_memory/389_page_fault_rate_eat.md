...
> **핵심**: 페이지 부재율($p$)은 메모리 접근 시 필요한 페이지가 램에 없어서 디스크로 달려가야(Page Fault) 할 확률을 의미하며, 실질 접근 시간(Effective Access Time, EAT)은 이 **확률 $p$에 디스크라는 끔찍한 지연 시간(수백만 배 느림)을 가중 평균하여 뽑아낸 시스템의 진짜 체감 속도 공식**이다.
