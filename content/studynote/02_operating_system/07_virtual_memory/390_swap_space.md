...
> **핵심**: 스왑 공간(Swap Space) 또는 베이킹 스토어(Backing Store)는 물리 메모리(RAM)가 꽉 찼을 때, 운영체제가 당장 쓰지 않는 페이지 조각들을 램에서 끄집어내어 임시로 보관해 두는 **하드디스크(또는 SSD) 상의 특수 목적 창고(Hidden Storage)**다.
