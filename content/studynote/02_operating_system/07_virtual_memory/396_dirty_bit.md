...
> **핵심**: 변경 비트(Modify Bit, 일명 Dirty Bit)는 페이지 테이블 엔트리(PTE)에 존재하는 1비트짜리 상태 플래그로, 해당 페이지가 디스크에서 램(RAM)으로 올라온 이후 **CPU에 의해 값이 한 번이라도 수정(Write)되었는지를 추적하여 하드웨어가 1(Dirty)로 자동 마킹해 주는 장치**다.
