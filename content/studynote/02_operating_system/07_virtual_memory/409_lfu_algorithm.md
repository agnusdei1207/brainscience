...
> **핵심**: LFU(Least Frequently Used) 페이지 교체 알고리즘은 램(RAM)에 빈 공간이 없을 때, 과거부터 현재까지 **가장 적게 참조된(호출된 횟수가 제일 적은) 페이지를 "앞으로도 안 쓸 잉여 데이터"로 간주하고 디스크로 쫓아내는 빈도(Frequency) 기반의 타겟팅 기법**이다.
