...
> **핵심**: 에이징(Aging, 나이 먹기)은 단 1비트의 참조 비트만으로 판단하는 Clock 알고리즘의 변별력 한계와, 과거에 얽매이는 LFU 알고리즘의 알박기 문제를 동시에 해결하기 위해 **'시간의 흐름에 따라 과거의 참조 기록을 절반씩 깎아내리는(Right Shift) 소프트웨어적 쇠퇴 연산'**이다.
