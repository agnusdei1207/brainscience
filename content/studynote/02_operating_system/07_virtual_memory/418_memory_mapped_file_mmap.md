...
> **핵심**: 메모리 매핑 파일(mmap)은 하드디스크에 있는 파일(`.txt`, `.db`)을 전통적인 `read/write` 함수로 읽어오는 대신, **파일의 내용을 통째로 프로세스의 가상 주소 공간(Virtual Address Space) 배열에 다이렉트로 꽂아(Mapping) 넣어, 포인터 조작만으로 파일을 램(RAM) 다루듯 씹어먹는 운영체제의 흑마술**이다.
