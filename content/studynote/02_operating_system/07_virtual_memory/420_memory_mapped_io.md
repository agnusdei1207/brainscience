...
> **핵심**: 메모리 맵 I/O(MMIO)는 CPU가 그래픽 카드(GPU)나 랜카드(NIC) 같은 외부 하드웨어 장치와 대화할 때 귀찮은 전용 I/O 명령어를 쓰지 않고, **하드웨어 기기의 제어 레지스터를 일반적인 램(RAM)의 물리 주소 공간 일부에 덮어씌워 맵핑(Mapping)하는 하드웨어-소프트웨어 융합 기법**이다.
