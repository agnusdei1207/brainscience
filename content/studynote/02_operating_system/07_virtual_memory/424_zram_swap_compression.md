...
> **핵심**: ZRAM(커널 스왑 압축 기술)은 물리 램(RAM)이 꽉 차서 데이터를 디스크 스왑(Swap) 파티션으로 쫓아내야 할 때, **디스크로 내보내지 않고 CPU의 압축(LZ4/Zstd) 연산을 이용해 데이터를 1/3 크기로 찌그러뜨려 램(RAM) 내부의 숨겨진 가상 스왑 공간(ZRAM)에 욱여넣는 극한의 인메모리(In-Memory) 생존 기술**이다.
