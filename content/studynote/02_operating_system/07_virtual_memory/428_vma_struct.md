...
> **핵심**: VMA(`vm_area_struct`) 구조체는 리눅스 운영체제 커널 내부에서, 프로세스의 광활한 **가상 주소 공간(Virtual Address Space) 중 '실제로 의미 있게 사용되는 특정 구간(코드, 힙, mmap 등)'의 시작과 끝, 그리고 권한 정보를 기록해 두는 연결 리스트 형태의 샌드박스 지도(Map)**다.
