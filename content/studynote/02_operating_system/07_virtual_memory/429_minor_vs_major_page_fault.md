...
> **핵심**: 가상 메모리 환경에서 발생하는 페이지 폴트(Page Fault)는 해결하기 위해 겪어야 하는 고통의 무게에 따라, **디스크(I/O) 접근 없이 램 내부의 조작만으로 0.001초 만에 쓱 해결되는 '마이너(Minor) 폴트'**와 **무거운 하드디스크(Swap)까지 덜그럭거리며 데이터를 퍼와야 해서 1초 이상 서버를 마비시키는 '메이저(Major) 폴트'**로 극명하게 나뉜다.
