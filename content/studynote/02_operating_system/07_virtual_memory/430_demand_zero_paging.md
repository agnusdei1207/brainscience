...
> **핵심**: 수요 페이지 제로화(Demand Zero Paging 또는 Zero-Fill-On-Demand, ZFOD)는 프로그램이 초기화되지 않은 거대 배열(BSS 영역이나 Heap)을 할당받을 때, **OS가 램을 즉시 주지 않고 빈껍데기만 매핑해 두었다가, 실제 쓸 때(Demand) 빈 프레임을 가져와 보안을 위해 0(Zero)으로 가득 채워(세탁) 건네주는 지연 할당 기법**이다.
