...
> **핵심**: 역 페이지 테이블(Inverted Page Table)은 메모리 낭비를 줄이기 위해 전 세계(시스템)에 장부를 딱 1개만 두어 $O(N)$의 끔찍한 순차 탐색 렉을 유발하는데, 이를 **단 1~2회의 메모리 접근만으로 끊어내기 위해 `[PID, Page Number]`를 키(Key)로 삼아 $O(1)$ 속도로 인덱스를 꽂아주는 하드웨어 해시 함수(Hash Function) 융합 아키텍처**다.
