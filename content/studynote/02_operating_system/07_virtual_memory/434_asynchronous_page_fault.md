...
> **핵심**: 기존의 페이지 폴트가 터지면 해당 프로세스나 스레드가 디스크에서 데이터를 퍼올 때까지(8ms) 꼼짝없이 얼어붙는(Blocked) 동기적 파멸을 극복하기 위해, **OS가 폴트 처리를 백그라운드 워커에게 던져두고 해당 스레드가 다른 유용한 연산을 계속 이어가게끔 강제하는 초고도화된 비동기(Asynchronous) 튜닝 기법**이다.
