...
> **핵심**: 초기 리눅스 커널에서 디스크 블록(하드웨어 섹터)을 담아두는 **'버퍼 캐시(Buffer Cache)'**와 파일의 내용(소프트웨어 파일)을 담아두는 **'페이지 캐시(Page Cache)'**로 쪼개져 램을 이중으로 낭비하던 악습을, **페이지 캐시 하나로 완벽하게 대통합(Unified)시킨 아키텍처 혁명**이다.
