...
> **핵심**: 운영체제는 세상에 존재하는 수만 가지의 각기 다른 하드웨어 I/O 기기들을 통제하기 위해, 이들을 데이터의 전송 단위와 탐색(Seek) 가능 여부에 따라 크게 **'블록 장치(Block Device)'와 '문자 장치(Character Device)'라는 두 개의 거대한 추상화 클래스로 쪼개어 분류**한다.
