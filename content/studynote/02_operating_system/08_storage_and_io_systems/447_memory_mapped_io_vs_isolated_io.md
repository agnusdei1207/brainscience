...
> **핵심**: CPU가 그래픽카드나 마우스 같은 외부 하드웨어 장치를 제어(Read/Write)할 때, 그 장치의 레지스터 주소를 **우리가 아는 일반 물리 램(RAM) 주소 공간의 일부분에 덮어씌워 퉁치는 것이 '메모리 맵 I/O(MMIO)'**이고, **램과 완전히 분리된 별도의 I/O 전용 주소 공간을 파놓고 특수 명령어로만 찌르는 것이 '분리된 I/O(Isolated I/O / PMIO)'**다.
