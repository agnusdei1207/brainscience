...
> **핵심**: 폴링(Polling) 또는 Programmed I/O(PIO)는 CPU가 외부 하드웨어 디바이스에게 일을 시킨 뒤, 기계가 일을 끝마쳤는지 확인하기 위해 **CPU가 직접 무한 루프(`while`)를 돌며 디바이스의 '상태 레지스터(Status Register)'를 1초에 수백만 번씩 물어보고 쳐다보는(바쁜 대기, Busy Wait) 가장 원시적인 I/O 동기화 방식**이다.
