...
> **핵심**: 산란-수집(Scatter-Gather) DMA는 램(RAM)의 물리적 주소가 4KB 페이지 단위로 갈기갈기 찢어져 있을 때, 디스크나 랜카드에서 대용량 데이터를 퍼와 **여러 개의 찢어진 물리 프레임에 알아서 흩뿌리거나(Scatter), 흩어진 데이터들을 한 번에 모아서(Gather) 전송해 주는 고도화된 지능형 하드웨어 I/O 기술**이다.
