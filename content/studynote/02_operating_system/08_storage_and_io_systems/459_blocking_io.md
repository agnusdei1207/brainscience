...
> **핵심**: 블로킹 I/O(Blocking I/O)는 애플리케이션(스레드)이 디스크나 네트워크 같은 외부 기기에 데이터를 요구(read/write)했을 때, **데이터가 도착할 때까지 OS가 해당 스레드의 실행을 강제로 기절(Sleep/Wait 상태)시켜 무한정 대기하게 만드는 가장 직관적이고 고전적인 동기화 모델**이다.
