...
> **핵심**: 논블로킹 I/O(Non-blocking I/O)는 프로세스(스레드)가 OS에게 데이터(디스크/네트워크)를 요구했을 때, **데이터가 아직 도착하지 않아도 스레드를 기절(Sleep)시키지 않고 즉각 "아직 데이터 없음(EAGAIN)" 에러 코드를 뱉어버린 뒤 제어권을 스레드에게 돌려주는 I/O 아키텍처**다.
