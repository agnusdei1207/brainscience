...
> **핵심**: 비동기 I/O(AIO)는 운영체제에게 "이 파일 10MB 읽어와!"라고 명령만 툭 던져두고 **결과를 기다리지 않은 채 즉시 다음 코드를 실행하며, 나중에 OS가 램(RAM)에 데이터를 다 채우면 "다 퍼왔어!"라고 시그널이나 콜백(Callback)으로 알려주는 100% 논블로킹 아키텍처**다.
