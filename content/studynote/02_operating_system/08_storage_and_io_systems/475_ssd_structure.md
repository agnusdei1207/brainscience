...
> **핵심**: 솔리드 스테이트 드라이브(SSD)는 모터와 바늘(Head)로 긁어 읽던 아날로그 하드디스크(HDD)를 멸종시키고, 전자가 방(Cell) 안에 갇혀있는지 없는지로 0과 1을 판별하는 **낸드 플래시(NAND Flash) 반도체 메모리로 저장장치를 100% 디지털화한 하드웨어 혁명**이다.
