...
> **핵심**: SSD의 가비지 컬렉션(GC)은 '덮어쓰기'가 안 되어 쓰레기(Invalid Page)로 가득 차버린 낸드 플래시 메모리 블록에서, **아직 살아있는 정상 데이터(Valid Page)들만 쏙쏙 뽑아 새 블록으로 이사(Copy)시킨 뒤, 옛날 블록 전체에 고압 전기를 쏴서 텅 빈 깨끗한 상태(Erase)로 초기화하는 내부 펌웨어의 대청소 작업**이다.
