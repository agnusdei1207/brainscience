...
> **핵심**: FTL(Flash Translation Layer)은 "덮어쓰기가 불가능(Erase-before-write)하고 블록 단위로만 지워지는" 낸드 플래시(NAND Flash) 메모리의 기형적이고 거친 물리적 쌩얼을 완벽하게 가려, **운영체제(OS)에게 마치 "언제든 맘대로 덮어쓸 수 있는 일반 하드디스크(HDD)"인 것처럼 속여서(Emulation) 보여주는 초고지능형 펌웨어 계층**이다.
