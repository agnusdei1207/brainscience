...
> **핵심**: SAN(Storage Area Network)은 일반적인 인터넷/사무용 TCP/IP 랜선을 타지 않고, 오직 서버(Host)와 거대 스토리지(Storage) 덩어리들만을 연결하기 위해 '블록(Block) I/O 통신' 명령어 전용의 광케이블(Fibre Channel)과 특수 고속 스위치망을 별도로 완전히 구축한 **"초고속 스토리지 전용 지하 고속도로 망"** 모델이다.
