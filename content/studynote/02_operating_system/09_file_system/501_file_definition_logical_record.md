...
> **핵심**: 컴퓨터 하드 디스크 입장에서는 그저 의미 없는 '0과 1' 바이트 데이터 쪼가리와 섹터들의 파편일 뿐이다. 이 무의미한 금속판 조각들에 **"시작과 끝이 있는 의미 있는 뭉텅이 캡슐(논리적 레코드의 연속)"** 이라는 이름과 질서를 부여해 사람이 다룰 수 있게 OS가 창조한 가상의 추상화 단위가 바로 '파일(File)'이다.
