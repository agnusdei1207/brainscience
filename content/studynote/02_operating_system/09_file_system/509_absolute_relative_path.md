...
> **핵심**: 수많은 폴더가 얽힌 트리(Tree) 구조 속에서 목적지(파일)를 찾아갈 때, **"무조건 우주의 시작점(Root `/`)부터 주소를 읊어 내려갈 것인가(절대 경로)"**, 아니면 **"지금 내가 서 있는 방(현재 디렉터리 `.`)을 기준으로 옆방으로 꺾어 뛰어 갈 것인가(상대 경로)"** 를 결정짓는 검색 위치 지정 문법의 두 축이다.
