...
> **핵심**: 앞선 '비순환 그래프' 가 시스템 멸망(무한 루프)을 막기 위해 폴더 간의 링크(뫼비우스 고리) 생성을 철저히 금지했다면, **일반 그래프 디렉터리(General Graph Directory)** 는 "자유가 최고다! A폴더 안에 B폴더, B폴더 안에 다시 A폴더 링크를 무제한 꼬리 물기(순환 Cycle)로 걸게 해 줘라!" 라고 선언한 구조다.
