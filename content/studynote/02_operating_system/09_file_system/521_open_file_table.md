...
> **핵심**: 하드디스크에 있는 파일을 읽으려면 커널은 이를 무조건 메모리에 올려둬야 한다. 이때 "누가, 이 파일을 며칠 몇 시에 열었고, 현재 몇 번째 줄을 읽고 있으며(파일 포인터), 수정 권한은 있는지" 를 추적하는 **동적 감시 장부(배열 Array)** 가 바로 '열린 파일 테이블(Open File Table)'이다.
