...
> **핵심**: 디스크에 파일 조각들을 파편화시켜 흩뿌리는 건 연결 할당(Linked)이나 FAT와 같다. 그러나 이 방식은 찌질하게 꼬리표를 조각 끝에 달거나 거대한 공용 FAT 배열을 만들지 않고, **"아예 파일 1개마다 그 파일만의 전용 주소록(Index Block 색인 블록) 1칸" 을 강제 할당하여 모든 흩어진 조각들의 모터 물리 좌표를 한곳에 싹 다 적어 놓는 방식** 이다.
