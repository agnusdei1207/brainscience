...
> **핵심**: 색인 할당(Indexed 526번)은 파일마다 디스크 주소를 모아두는 4KB짜리 장부(인덱스 블록)를 하나씩 들고 있다. 하지만 영화 파일이 10GB로 커지면, 이 4KB짜리 꼬꼬마 장부 1장으로는 수백만 줄의 포인터 주소를 전부 적어 낼 여백 종이가 모자라 터져버리는 **한계 붕괴 (Index Overflow)** 에러에 직면했다.
