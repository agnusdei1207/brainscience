...
> **핵심**: 디스크 철판에서 1개의 256바이트짜리 유닉스 i-node 껍데기 포인터 배열 장부를 열었을 때, **가장 앞단에 위치하는 12~15개 남짓의 C언어 배열 칸(Array Slots)** 이다. 이곳엔 다중 레벨 트리(간접 지연)를 거치지 않고, 그 칸에 적힌 주소를 읽자마자 디스크 데이터 목적지로 **1방에 다이렉트 레이저 점프 빔 타격** 을 쏠 수 있는 물리 포인터가 적혀 있다.
