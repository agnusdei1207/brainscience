...
> **핵심**: 앞선 543장 NFS가 '서버한테 계속 물어보는 원격 실시간 둔갑술' 이라면, **AFS(Andrew File System)** 는 **"아예 파일을 내 하드디스크(로컬 캐시)로 훔쳐 복사 통째 이주시켜서, 작업 다 끝나고 나서만 서버에 던져주는 은둔형(State stateful) 분산 시스템 아크 결착"** 이다. 한편 **SMB/CIFS**는 마이크로소프트가 윈도우 OS의 `\\공유폴더` 를 지배하기 위해 뿌리내린 또 다른 배타적 잠금(Locking) 중심의 거시 전송 표준 파이프 렌더다.
