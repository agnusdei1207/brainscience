...
> **핵심**: 앞선 리눅스의 ext4가 i-node라는 작고 분산된 인덱스를 사용한다면, 마이크로소프트의 NTFS는 **"모든 파일의 메타데이터를 거대한 하나의 전역 데이터베이스 테이블(MFT: Master File Table)로 모조리 흡수해 B-Tree로 엮어버린 초거대 스토리지 레코드 엔진(Record Engine 렌더!)"** 이다.
