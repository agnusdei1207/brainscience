...
> **핵심**: 커널 뱃속의 파일(텍스트 등)에 누구나 접근하면 해킹으로 서버가 파괴되므로, **"파일 1개마다 그 파일을 지배하는 '주인(User)', 주인의 친구들(Group), 그리고 생판 남(Other)' 이라는 3계급으로 철저히 신분을 쪼개고, 그들에게 각각 읽기(r), 쓰기(w), 실행(x)의 권한 블록을 차등 결합 렌더"** 시키는 가장 고전적이고 원초적인 UNIX 통제 인프라다.
