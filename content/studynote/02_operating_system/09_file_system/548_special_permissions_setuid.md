...
> **핵심**: 앞선 rwx 권한만으로는 "일반 유저가 자기 비밀번호를 바꾸려면 최고 관리자(Root)만 만질 수 있는 `/etc/shadow` 파일을 수정해야 한다" 는 딜레마(권한 모순 늪!)를 풀 수 없다. 이를 해소하기 위해 만들어진 **"이 프로그램을 실행하는 그 순간만큼은, 이 파일의 주인의 영혼(UID)으로 빙의(둔갑 스왑)시켜주마!"** 라는 커널 단위의 조건부 신분 상승 렌더다.
