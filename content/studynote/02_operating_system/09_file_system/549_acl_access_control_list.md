...
> **핵심**: 앞선 rwx 권한 체계가 세상 사람을 딱 3단계(나, 내그룹, 남들)로만 나누는 편협한 틀이었다면, **ACL(접근 제어 목록)** 은 그 제한을 박살 내고 **"존에게는 읽기! 마이크에게는 쓰기 금지! A팀에게는 실행만! 이라는 특정 유저/그룹 100만 명의 지명 수배지를 파일 뒤꼬리에 주렁주렁 무한대로 매달아 핀셋(Fine-grained) 제어 렌더!"** 를 실현한 아키텍처다.
