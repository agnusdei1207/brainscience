...
> **핵심**: 디스크가 깨지거나 랜섬웨어가 걸렸을 때 기업 멸망을 막는 유일한 사슬은 다른 디스크로 데이터를 파내어 격리 보관하는 백업이다. 매일 밤 10TB의 데이터 전체를 무식하게 통째로 복사하는 **전체(Full) 백업 늪** 과, 어제 복사한 이후 '새로 수정된 추가 사항(Diff 찌꺼기)' 만 긁어가는 **증분(Incremental) 백업 렌더** 사이의 극한 줄다리기가 SRE 재난 복원(DR)의 핵심 통치다.
