...
> **핵심**: 커널 뱃속에서 돌아가는 CPU 온도, RAM 여유량, 프로세스 1번의 상태 등은 100% 바이너리 C 구조체(커널 메모리)다. 일반 유저가 이 숫자를 보려면 원래 특수 시스템 콜(`ioctl`) 깡통을 두드려야 하지만, 리눅스는 **"모든 것은 파일이다(Everything is a file)"** 철학을 극대화하여 **이 엄청난 커널 변수들을 단순히 `cat /proc/cpuinfo` 로 읽을 수 있는 가상의 텍스트 파일(가짜 폴더 렌더)** 로 둔갑(Illusion 스왑) 시켰다.
