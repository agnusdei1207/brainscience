...
> **핵심**: ext4 나 NTFS 등 기존 인류의 유산은 모두 "빙빙 도는 쇳덩어리 모터(HDD 자성 디스크)" 를 위해 태어났다. 하드는 빈칸에 그냥 즉시 "덮어쓰기(Overwrite 통치)" 가 되지만, 낸드(NAND) 플래시 반도체는 데이터를 수정하려면 무조건 전기로 그 블록을 **"통째로 불태워 지우고(Erase-before-Write 파단!) 100만 번 충격을 주면 수명이 죽어버리는 최악의 쓰기 약점 늪"** 을 품고 있다.
