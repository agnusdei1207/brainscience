...
> **핵심**: 앞 583장의 SELinux(라벨 기반) 가 설정 파일을 짜다 엔지니어를 정신병에 걸리게 만들었다. 이에 대항(Ubuntu 수세 진영) 하여 튀어나온 **AppArmor(경로 기반 MAC 방패 빔!)** 는, 파일 이마에 더러운 라벨을 박지 않는다. 오직 시스템의 **절대 경로 문자열(`/etc/shadow`, `/var/www/*`) 자체를 족쇄 리스트 룰(Profile) 로 사용해 컨테이너를 가두는 직관적 제어 엔진**이다.
