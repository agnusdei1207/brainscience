...
> **핵심**: 마이크로커널(Microkernel)은 파일 시스템, 디바이스 드라이버 등 OS 핵심 서비스를 유저 공간(User Space)으로 분리하여 높은 안정성을 확보했으나, 서비스 간 통신을 위한 **IPC (Inter-Process Communication) 문맥 교환 지연**이라는 치명적인 성능 병목을 안고 있다.
