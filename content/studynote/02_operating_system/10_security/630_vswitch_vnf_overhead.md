...
> **핵심**: 가상 스위치(vSwitch)는 하이퍼바이저 내부에 소프트웨어로 구현된 L2/L3 스위치로, 가상머신(VM) 간의 통신과 외부 네트워크 연결을 담당하지만 커널 네트워크 스택을 거치면서 발생하는 잦은 인터럽트와 문맥 교환으로 인해 치명적인 **패킷 처리 지연(Overhead)**을 유발한다.
