...
> **핵심**: KSM (Kernel Samepage Merging)은 리눅스 커널이 백그라운드 스레드(ksmd)를 돌려 메모리의 물리 페이지들을 주기적으로 스캔하고, 내용이 완전히 똑같은 페이지들을 찾아내어 하나의 물리 페이지로 통합(Merging)하는 메모리 중복 제거(Deduplication) 기술이다.
