...
> **핵심**: VFIO (Virtual Function I/O)는 호스트 리눅스 커널의 디바이스 드라이버를 우회(Kernel Bypass)하여, PCIe 장치(GPU, NIC 등)를 유저 스페이스(가상머신이나 DPDK 앱)에 직접 꽂아주는(Passthrough) 안전한 I/O 가상화 프레임워크다.
