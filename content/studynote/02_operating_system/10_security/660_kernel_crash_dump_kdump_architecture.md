...
> **핵심**: Kdump는 리눅스 시스템이 치명적인 오류(Kernel Panic, OOM)로 뻗어버렸을 때, 당시 램(RAM)에 떠 있던 커널의 메모리 상태를 고스란히 파일(`vmcore`)로 추출하여 사후 원인 분석(Post-mortem Analysis)을 가능하게 해주는 장애 조사 메커니즘이다.
