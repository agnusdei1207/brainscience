...
> **핵심**: XDP (eXpress Data Path)는 리눅스 커널의 네트워크 스택 중 **가장 얕은 위치(네트워크 카드 드라이버 직후)**에서 eBPF(extended BPF) 프로그램을 실행시켜, 무거운 커널 데이터 구조(`sk_buff`)가 만들어지기도 전에 패킷을 초고속으로 처리하는 인커널(In-Kernel) 데이터 패스 기술이다.
