...
> **핵심**: 제로 트러스트(Zero Trust)는 "내부망에 있으니까 안전하겠지"라는 가정을 버리고, 모든 접근과 실행을 끊임없이 의심하고 검증하는 보안 철학이다. 운영체제 레벨에서 이를 구현하는 핵심이 바로 **런타임 무결성 검증망(Runtime Integrity Measurement)**이다.
