...
> **핵심**: 결함 주입(Fault Injection)은 소프트웨어 인프라가 실제 장애 상황(디스크 I/O 에러, 메모리 할당 실패, 네트워크 단절)에서 올바르게 복구되거나 우아하게 죽는지(Graceful Degradation)를 검증하기 위해 **고의로 커널에 오류를 발생시키는 카오스 엔지니어링 기법**이다.
