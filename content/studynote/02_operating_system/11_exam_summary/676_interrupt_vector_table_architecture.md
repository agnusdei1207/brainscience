...
> **핵심**: 인터럽트 벡터 테이블(Interrupt Vector Table, IVT)은 CPU가 외부 장치의 하드웨어 인터럽트나 소프트웨어 예외(Exception)를 받았을 때, **어떤 커널 함수(ISR: Interrupt Service Routine)를 실행해야 할지 찾아가는 '메모리상의 주소록(배열)'**이다.
