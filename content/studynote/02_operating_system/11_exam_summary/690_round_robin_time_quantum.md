...
> **핵심**: 라운드 로빈(Round Robin, RR)은 모든 프로세스에게 공평하게 동일한 시간 조각, 즉 **시간 할당량(Time Quantum 또는 Time Slice)**을 주고 번갈아 실행하게 만드는 시분할 시스템(Time-sharing)의 가장 기본적인 선점형 스케줄링 알고리즘이다.
