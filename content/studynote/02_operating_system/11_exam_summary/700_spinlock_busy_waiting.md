...
> **핵심**: 스핀락(Spinlock)은 스레드가 임계 구역(Critical Section)의 락을 얻지 못했을 때 OS 스케줄러에게 CPU를 양보(Sleep)하지 않고, **락이 풀릴 때까지 `while` 루프를 무한정 돌며 기다리는(Busy-waiting) 가장 원시적이고 공격적인 동기화 기법**이다.
