...
> **핵심**: 식사하는 철학자 문제(Dining Philosophers Problem)는 에츠허르 다익스트라(Dijkstra)가 고안한 동시성 제어의 클래식 난제로, 한정된 다수의 자원(포크)을 다수의 프로세스(철학자)가 경쟁적으로 요구할 때 발생하는 **교착 상태(Deadlock)와 기아(Starvation) 현상을 직관적으로 모델링**한 것이다.
