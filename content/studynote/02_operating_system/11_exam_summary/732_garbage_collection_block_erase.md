...
> **핵심**: SSD의 가비지 컬렉션(Garbage Collection, GC)은 낸드 플래시 메모리의 "덮어쓰기 불가" 특성 때문에 필연적으로 쌓이는 **무효화된 쓰레기 페이지(Invalid Page)들을 한곳으로 모아, 블록(Block) 단위로 한 번에 폭파(Erase)시켜 새로운 빈 공간(Free Block)을 확보하는 백그라운드 청소 작업**이다.
