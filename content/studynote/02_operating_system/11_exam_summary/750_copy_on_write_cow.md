...
> **핵심**: 쓰기 시 복사 (Copy-On-Write, COW)는 여러 프로세스나 시스템이 동일한 자원(메모리 페이지, 파일 등)을 공유하다가, **그 중 누군가가 데이터를 '수정(Write)'하려는 찰나의 순간에만 원본을 '복사(Copy)'하여 독립적인 공간을 할당**해주는 게으른 최적화(Lazy Optimization) 기법이다.
