...
> **핵심**: 파일 지연 쓰기 (Delayed Write)는 응용 프로그램이 파일에 데이터를 쓸(Write) 때 곧장 느린 하드 디스크로 내려보내지 않고, **메인 메모리의 버퍼 캐시(Page Cache)에만 살짝 기록해 둔 뒤 즉시 성공(Return)으로 속이고, 나중에 OS가 백그라운드에서 여유로울 때 모아서 디스크에 반영하는 비동기 최적화 기술**이다.
