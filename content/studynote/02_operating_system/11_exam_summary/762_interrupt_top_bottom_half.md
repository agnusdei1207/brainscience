...
> **핵심**: 인터럽트 처리는 0.1초의 지연도 허용하지 않는 긴급 작업(상반부, Top-Half)과, 나중에 한가할 때 천천히 처리해도 되는 덜 급한 무거운 작업(하반부, Bottom-Half)으로 **인터럽트 서비스 루틴(ISR)을 두 토막 내어 비동기적으로 분할 처리하는 커널 스케줄링 아키텍처**다.
