...
> **핵심**: 스핀락 (Spinlock)은 다른 스레드가 자원(Lock)을 쥐고 있을 때, 내가 대기 상태(Sleep)로 들어가지 않고 **CPU 클럭을 미친 듯이 태우며 무한 루프(while 문)를 돌아 자원이 풀리자마자 즉시 낚아채는(Spinning) 동기화 원시 타입(Primitive)**이다.
