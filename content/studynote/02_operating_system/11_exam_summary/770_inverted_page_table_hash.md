...
> **핵심**: 역 페이지 테이블 (Inverted Page Table)은 논리 주소를 인덱스로 삼아 물리 주소를 찾는 기존 방식과 정반대로, **물리 메모리(RAM)의 페이지 프레임 번호를 인덱스로 삼아 현재 그 공간을 어떤 프로세스의 어떤 논리 주소가 쓰고 있는지를 기록**하는 아키텍처다.
