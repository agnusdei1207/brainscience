...
> **핵심**: OOM 킬러 (Out-Of-Memory Killer)는 리눅스 커널이 메모리(RAM + Swap) 고갈로 인한 시스템 전체의 동반 자살(Kernel Panic)을 막기 위해 발동시키는 최후의 자위권으로, **가장 쓸모없거나 죄질이 나쁜(메모리를 비정상적으로 많이 먹는) 프로세스 하나를 사형시켜 메모리 숨통을 틔우는 저승사자 알고리즘**이다.
