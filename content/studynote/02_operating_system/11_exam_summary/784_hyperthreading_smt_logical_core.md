...
> **핵심**: 하이퍼스레딩 (Hyper-Threading, SMT)은 하나의 물리적 CPU 코어 안에 **연산 엔진(ALU)은 하나만 두고, 상태를 저장하는 레지스터(Register) 보관함과 파이프라인 관리자만 2세트로 복제**하여, 운영체제가 이를 2개의 독립된 '논리 코어(Logical Core)'로 착각하게 만드는 하드웨어 속임수 기술이다.
