...
> **핵심**: 리눅스의 `clone()` 시스템 콜은 프로세스를 복제하는 `fork()`와 스레드를 생성하는 `pthread_create()`의 근간이 되는 최하단 커널 API로, **부모와 자식 간에 메모리, 파일, 시그널 등 어떤 자원을 "공유(Share)"할지 플래그(Flag) 비트마스크를 통해 레고 블록처럼 정밀하게 조립하는 만능 생성기**다.
