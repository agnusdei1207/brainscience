...
> **핵심**: iOS 앱 샌드박싱(App Sandboxing)은 애플 운영체제(Unix 기반 XNU 커널)가 악성코드나 버그로부터 시스템 전체를 보호하기 위해, **모든 서드파티 앱을 각자 고립된 "가상의 모래놀이터(Sandbox)" 안에 강제로 가두어 파일 시스템과 하드웨어 접근을 원천 차단하는 커널 레벨의 MAC(강제 접근 통제) 아키텍처**다.
