...
> **핵심**: 락 엘리전 (Lock Elision, 락 생략)은 소프트웨어가 뮤텍스(Mutex) 같은 락을 걸려고 할 때, 하드웨어(CPU)가 개입하여 **"일단 락을 걸지 말고 각자 캐시 메모리에서 미친 듯이 병렬로 실행해 봐! 우연히 충돌(Conflict)이 나면 그때 다 취소(Rollback)하고 진짜 락을 걸게!"**라고 락의 병목을 하드웨어 레벨에서 속이고 우회하는 최첨단 최적화 기술이다.
