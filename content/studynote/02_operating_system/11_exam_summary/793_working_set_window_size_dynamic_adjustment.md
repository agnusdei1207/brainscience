...
> **핵심**: 워킹 셋(Working Set) 모델은 다중 프로그래밍 환경에서 특정 프로세스가 **"최근 일정 시간(Window, $\Delta$) 동안 미친 듯이 집중적으로 참조한 페이지들의 묶음"을 수학적으로 예측하여, 그만큼의 메모리는 절대 빼앗지 않고 RAM에 상주시켜 주는 스래싱(Thrashing) 방어 아키텍처**다.
