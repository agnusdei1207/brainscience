...
> **핵심**: NUMA (Non-Uniform Memory Access, 불균일 메모리 접근)는 64코어 이상의 거대 서버 환경에서, 모든 코어가 하나의 공용 메모리 버스에 몰려 병목이 터지는 것을 막기 위해 **CPU 소켓별로 '자신만의 전용 메모리(Local RAM)' 구역을 찢어서 할당해 주는 하드웨어 아키텍처**다.
