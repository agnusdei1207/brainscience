...
> **핵심**: CWDM (Coarse WDM)은 광케이블 1가닥에 여러 빛을 섞어 쏘는 WDM (Wavelength Division Multiplexing) 기술 중, 빛의 파장 간격을 20nm 단위로 넓게(Coarse) 잘라 최대 18개 채널을 합치는 다중화 아키텍처다.
> **비유**: 저밀도 파장 분할 다중화 (CWDM, Coarse WDM)은(는) 다중화와 다중접속 제어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 저밀도 파장 분할 다중화 (CWDM, Coarse WDM)은(는) 채널 분할, 슬롯 할당, 충돌 제어 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 다중화와 다중접속 제어 이해
    ↓
저밀도 파장 분할 다중화 (CWDM, Coarse WDM) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
저밀도 파장 분할 다중화 (CWDM, Coarse WDM)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
