...
> **핵심**: 근거리-원거리 문제 (Near-Far Problem)는 기지국 바로 밑에 있는 단말기의 강력한 전파가 멀리 있는 단말기의 미약한 전파를 완전히 덮어버려 시스템을 마비시키는 CDMA의 태생적 결함이다.
> **비유**: 근거리-원거리 문제 (Near-Far Problem) - CDMA 전력 제어은(는) 다중화와 다중접속 제어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 근거리-원거리 문제 (Near-Far Problem) - CDMA 전력 제어은(는) 채널 분할, 슬롯 할당, 충돌 제어 조건을 함께 만족할 때 안정적으로 동작한다.
...
근거리-원거리 문제 (Near-Far Problem) - CDMA 전력 제어을 적용하면 채널 분할과 슬롯 할당에 대한 통제력이 높아지고, 운영 일관성과 성능 예측 가능성을 확보할 수 있다. 반면 설정 복잡도, 표준 호환성, 장비 비용, 운영 숙련도라는 전제가 뒤따르므로 무조건적인 도입이 정답은 아니다. 결론적으로 이 주제는 다중화와 다중접속 제어를 이해하는 기준점이며, 최근에는 자동화·가상화·지능형 제어와 결합해 더 세밀한 최적화 방향으로 진화하고 있다.
