...
> **핵심**: NOMA (Non-Orthogonal Multiple Access)는 여러 사용자의 데이터를 동일한 주파수와 시간 자원에 중첩하여 전송하되, 전력 도메인 (Power Domain)에서 차이를 두어 구분하는 비직교 다중 접속 방식이다.
> **비유**: NOMA (Non-Orthogonal Multiple Access) - 비직교 다중 접속 (5G/6G 기술)은(는) 다중화와 다중접속 제어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: NOMA (Non-Orthogonal Multiple Access) - 비직교 다중 접속 (5G/6G 기술)은(는) 채널 분할, 슬롯 할당, 충돌 제어 조건을 함께 만족할 때 안정적으로 동작한다.
