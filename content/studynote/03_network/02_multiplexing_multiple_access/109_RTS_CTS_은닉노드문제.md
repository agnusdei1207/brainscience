...
> **핵심**: 은닉 노드 문제는 물리적으로 서로 전파가 닿지 않는 두 단말기가 중앙의 AP(Access Point)에게 동시에 데이터를 전송하여 발생하는 보이지 않는 충돌 현상이다.
> **비유**: RTS/CTS (Request To Send / Clear To Send) - 은닉 노드 문제 해결은(는) 다중화와 다중접속 제어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: RTS/CTS (Request To Send / Clear To Send) - 은닉 노드 문제 해결은(는) 채널 분할, 슬롯 할당, 충돌 제어 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 다중화와 다중접속 제어 이해
    ↓
RTS/CTS (Request To Send / Clear To Send) - 은닉 노드 문제 해결 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
RTS/CTS (Request To Send / Clear To Send) - 은닉 노드 문제 해결은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
