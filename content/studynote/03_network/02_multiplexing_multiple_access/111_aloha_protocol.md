...
> **핵심**: 하와이 대학에서 개발한 최초의 무선 라디오 패킷 통신망으로, 노드가 데이터를 원할 때 언제든 즉시 전송하는 '무작위 다중 접속(Random Multiple Access)' 기술의 근원이다.
> **비유**: ALOHA (순수 알로하) - 하와이 대학, 무작위 채널 접근은(는) 다중화와 다중접속 제어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: ALOHA (순수 알로하) - 하와이 대학, 무작위 채널 접근은(는) 채널 분할, 슬롯 할당, 충돌 제어 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 다중화와 다중접속 제어 이해
    ↓
ALOHA (순수 알로하) - 하와이 대학, 무작위 채널 접근 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
ALOHA (순수 알로하) - 하와이 대학, 무작위 채널 접근은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
