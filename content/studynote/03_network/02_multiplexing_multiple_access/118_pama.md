...
> **핵심**: PAMA (Pre-Assigned Multiple Access)는 통신 네트워크(주로 위성 통신)에서 특정 지상국이나 단말에 주파수, 타임 슬롯, 또는 코드를 고정적으로 미리 할당(Pre-Assigned)하여 독점적으로 사용하게 하는 매체 접근 제어 방식이다.
> **비유**: PAMA (Pre-Assigned Multiple Access)은(는) 다중화와 다중접속 제어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: PAMA (Pre-Assigned Multiple Access)은(는) 채널 분할, 슬롯 할당, 충돌 제어 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 다중화와 다중접속 제어 이해
    ↓
PAMA (Pre-Assigned Multiple Access) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
PAMA (Pre-Assigned Multiple Access)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
