...
> **핵심**: W-CDMA (Wideband CDMA)는 5MHz의 광대역 주파수와 비동기식 기지국 구조를 결합하여 음성과 고속 데이터를 단일 반송파에서 처리하는 3GPP 주도의 3세대 이동통신 핵심 표준이며, HSPA는 이를 기반으로 데이터 속도를 극대화한 확장 기술(3.5G)이다.
> **비유**: W-CDMA (Wideband CDMA) / HSPA (High Speed Packet Access)은(는) 다중화와 다중접속 제어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: W-CDMA (Wideband CDMA) / HSPA (High Speed Packet Access)은(는) 채널 분할, 슬롯 할당, 충돌 제어 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 다중화와 다중접속 제어 이해
    ↓
W-CDMA (Wideband CDMA) / HSPA (High Speed Packet Access) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
W-CDMA (Wideband CDMA) / HSPA (High Speed Packet Access)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
