...
> **핵심**: 광섬유의 코어 직경 차이(8~10µm vs 50~62.5µm)에 따라 허용되는 빛의 진행 경로(모드) 수가 결정되며, 이는 전송 대역폭과 거리를 지배하는 핵심 물리적 파라미터다.
> **비유**: 단일모드 광섬유 (Single-mode Fiber, SMF) / 다중모드 광섬유 (MMF)은(는) 전송 매체와 물리 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 단일모드 광섬유 (Single-mode Fiber, SMF) / 다중모드 광섬유 (MMF)은(는) 매체 특성, 감쇠, 간섭 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 전송 매체와 물리 계층 이해
    ↓
단일모드 광섬유 (Single-mode Fiber, SMF) / 다중모드 광섬유 (MMF) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
단일모드 광섬유 (Single-mode Fiber, SMF) / 다중모드 광섬유 (MMF)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
