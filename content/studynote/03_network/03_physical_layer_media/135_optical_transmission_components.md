...
> **핵심**: 광통신 시스템은 전기 신호를 빛으로 변환(E-O)하는 송신 광원(LED, LD)과, 도달한 빛을 다시 전기 신호로 복원(O-E)하는 수신 수광소자(PIN, APD)의 물리적 결합이다.
> **비유**: 광전송 용어 - 광원 (LED, LD), 수광소자 (PIN 디오드, APD)은(는) 전송 매체와 물리 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 광전송 용어 - 광원 (LED, LD), 수광소자 (PIN 디오드, APD)은(는) 매체 특성, 감쇠, 간섭 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 전송 매체와 물리 계층 이해
    ↓
광전송 용어 - 광원 (LED, LD), 수광소자 (PIN 디오드, APD) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
광전송 용어 - 광원 (LED, LD), 수광소자 (PIN 디오드, APD)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
