...
> **핵심**: LAPB (Link Access Procedure, Balanced)는 X.25 패킷 교환 네트워크에서 터미널(DTE)과 네트워크 노드(DCE) 간의 신뢰성 있는 점대점 (Point-to-Point) 통신을 보장하기 위해 설계된 데이터 링크 계층 프로토콜이다.
> **비유**: LAPB (Link Access Procedure Balanced) - X.25 망은(는) 데이터 링크 계층과 오류 제어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: LAPB (Link Access Procedure Balanced) - X.25 망은(는) 프레이밍, 오류 검출, 재전송 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 데이터 링크 계층과 오류 제어 이해
    ↓
LAPB (Link Access Procedure Balanced) - X.25 망 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
LAPB (Link Access Procedure Balanced) - X.25 망은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.

## 8. 관련 개념 맵

| 개념 | 연결 포인트 |
|:---|:---|
| 프레이밍 | LAPB (Link Access Procedure Balanced) - X.25 망를 이해할 때 함께 봐야 하는 데이터 링크 계층과 오류 제어 핵심 개념 |
| 오류 검출 | LAPB (Link Access Procedure Balanced) - X.25 망를 이해할 때 함께 봐야 하는 데이터 링크 계층과 오류 제어 핵심 개념 |
| 재전송 | LAPB (Link Access Procedure Balanced) - X.25 망를 이해할 때 함께 봐야 하는 데이터 링크 계층과 오류 제어 핵심 개념 |
| 흐름 제어 | LAPB (Link Access Procedure Balanced) - X.25 망를 이해할 때 함께 봐야 하는 데이터 링크 계층과 오류 제어 핵심 개념 |
| MAC/LLC | LAPB (Link Access Procedure Balanced) - X.25 망를 이해할 때 함께 봐야 하는 데이터 링크 계층과 오류 제어 핵심 개념 |
