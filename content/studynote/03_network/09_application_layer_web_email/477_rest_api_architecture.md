...
> **핵심**: REST(Representational State Transfer)는 2000년 로이 필딩(Roy Fielding)이 창안한 아키텍처 제약 조건의 집합으로, 네트워크 상의 **모든 자원(Resource)에 고유한 URI를 부여**하고 **HTTP 메서드(GET, POST, PUT, DELETE 등)를 통해 자원의 상태를 주고받는** 일관된 인터페이스다.
> **비유**: REST API (Representational State Transfer) - 자원 중심 구조, HTTP 메서드 매핑은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: REST API (Representational State Transfer) - 자원 중심 구조, HTTP 메서드 매핑은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
REST API (Representational State Transfer) - 자원 중심 구조, HTTP 메서드 매핑 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
REST API (Representational State Transfer) - 자원 중심 구조, HTTP 메서드 매핑은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
