...
> **핵심**: gRPC(gRPC Remote Procedure Call)는 구글이 오픈소스화한 RPC 프레임워크로, 마치 내 컴퓨터의 함수를 호출하듯 원격 서버의 함수를 호출할 수 있게 해주는 강타입(Strongly-typed) 계약 기반 통신 규약이다.
> **비유**: gRPC - 구글 개발, Protocol Buffers (ProtoBuf), HTTP/2 기반 초고속 RPC, 서비스 메시(MSA)은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: gRPC - 구글 개발, Protocol Buffers (ProtoBuf), HTTP/2 기반 초고속 RPC, 서비스 메시(MSA)은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
gRPC - 구글 개발, Protocol Buffers (ProtoBuf), HTTP/2 기반 초고속 RPC, 서비스 메시(MSA) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
gRPC - 구글 개발, Protocol Buffers (ProtoBuf), HTTP/2 기반 초고속 RPC, 서비스 메시(MSA)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
