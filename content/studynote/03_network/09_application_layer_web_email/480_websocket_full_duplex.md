...
> **핵심**: WebSocket은 HTTP의 `Upgrade` 헤더를 통해 최초 연결을 맺은 후, HTTP의 굴레를 벗어던지고 **하나의 지속적인 TCP 연결 위에서 서버와 클라이언트가 자유롭게 이진/텍스트 프레임을 양방향(Full-Duplex)으로 쏘아대는 독립적인 프로토콜**이다.
> **비유**: WebSocket - 하나의 TCP 연결 위 전이중 (Full-Duplex) 실시간 브라우저-서버 통신은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: WebSocket - 하나의 TCP 연결 위 전이중 (Full-Duplex) 실시간 브라우저-서버 통신은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
WebSocket - 하나의 TCP 연결 위 전이중 (Full-Duplex) 실시간 브라우저-서버 통신 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
WebSocket - 하나의 TCP 연결 위 전이중 (Full-Duplex) 실시간 브라우저-서버 통신은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
