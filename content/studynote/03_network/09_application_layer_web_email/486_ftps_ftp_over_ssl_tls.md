...
> **핵심**: FTPS는 기존 FTP(포트 21, 20) 아키텍처를 그대로 유지한 상태에서, 밑바탕에 **TLS/SSL 암호화 터널**을 끼워 넣어 명령어(제어)와 데이터 채널을 보호하는 보안 확장 스펙(RFC 2228)이다.
> **비유**: FTPS (FTP over SSL/TLS)은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: FTPS (FTP over SSL/TLS)은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
FTPS (FTP over SSL/TLS)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
