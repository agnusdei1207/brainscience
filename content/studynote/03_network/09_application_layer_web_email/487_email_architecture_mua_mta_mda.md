...
> **핵심**: 이메일 시스템은 단일 중앙 서버가 처리하는 것이 아니라, 편지를 쓰는 **MUA**(Mail User Agent), 우체국끼리 릴레이로 편지를 넘기는 **MTA**(Mail Transfer Agent), 도착한 편지를 수신자의 개인 사서함에 꽂아주는 **MDA**(Mail Delivery Agent)의 완벽한 분업 아키텍처다.
> **비유**: 이메일 아키텍처 - MUA(메일 클라이언트), MTA(메일 서버), MDA(메일 수신 에이전트)은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 이메일 아키텍처 - MUA(메일 클라이언트), MTA(메일 서버), MDA(메일 수신 에이전트)은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
이메일 아키텍처 - MUA(메일 클라이언트), MTA(메일 서버), MDA(메일 수신 에이전트) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
이메일 아키텍처 - MUA(메일 클라이언트), MTA(메일 서버), MDA(메일 수신 에이전트)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
