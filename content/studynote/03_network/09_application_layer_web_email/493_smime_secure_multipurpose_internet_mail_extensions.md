...
> **핵심**: S/MIME(Secure/Multipurpose Internet Mail Extensions)은 단순히 이메일의 통신 도로를 지키는 SMTPS를 넘어, 발신자의 PC에서 **편지 내용 자체를 자물쇠(암호화)로 잠그고 도장(전자서명)을 찍은 뒤** 봉투에 넣어 보내는 '종단간(End-to-End) 보안' 규격이다.
> **비유**: S/MIME - 공개키 암호화 이메일 보안은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: S/MIME - 공개키 암호화 이메일 보안은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
S/MIME - 공개키 암호화 이메일 보안은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
