...
> **핵심**: PGP(Pretty Good Privacy)는 필 짐머만(Phil Zimmermann)이 개발한 하이브리드 암호화 프로그램으로, 송신자가 이메일 본문을 대칭키로 잠그고 그 대칭키를 수신자의 **공개키(Public Key)로 한 번 더 감싸서 보내는 종단간(End-to-End) 암호화의 원조**다.
> **비유**: PGP (Pretty Good Privacy) - 웹 오브 트러스트 기반 이메일 암호화 (Phil Zimmermann)은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: PGP (Pretty Good Privacy) - 웹 오브 트러스트 기반 이메일 암호화 (Phil Zimmermann)은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
PGP (Pretty Good Privacy) - 웹 오브 트러스트 기반 이메일 암호화 (Phil Zimmermann) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
PGP (Pretty Good Privacy) - 웹 오브 트러스트 기반 이메일 암호화 (Phil Zimmermann)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
