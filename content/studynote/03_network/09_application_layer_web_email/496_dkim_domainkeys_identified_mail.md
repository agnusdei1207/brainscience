...
> **핵심**: DKIM은 발송 서버가 편지 본문과 핵심 헤더(보내는 사람, 제목 등)를 믹서기(해시)에 갈아 나온 값을 **자신의 '비밀키(Private Key)'로 전자서명하여 메일에 첨부**하고, 수신 서버가 DNS에서 **발송자의 '공개키(Public Key)'를 가져와 도장의 진위와 내용의 위조 여부(무결성)를 0.1초 만에 수학적으로 검증**하는 기술이다.
> **비유**: DKIM (DomainKeys Identified Mail) - 디지털 서명 메일 위변조 검증은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: DKIM (DomainKeys Identified Mail) - 디지털 서명 메일 위변조 검증은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
DKIM (DomainKeys Identified Mail) - 디지털 서명 메일 위변조 검증 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
DKIM (DomainKeys Identified Mail) - 디지털 서명 메일 위변조 검증은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
