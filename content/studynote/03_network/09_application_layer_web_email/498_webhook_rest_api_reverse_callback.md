...
> **핵심**: 웹훅(Webhook)은 클라이언트가 서버에 데이터를 달라고 요청(Request)하는 전통적 API를 180도 뒤집어, **서버에 특정 이벤트(Event)가 발생했을 때 서버가 클라이언트가 미리 등록해 둔 URL(엔드포인트)로 HTTP POST 요청을 쏴주는 '역방향 API'**다.
> **비유**: 웹훅 (Webhook) - REST API의 콜백 역방향 호출 구조 (이벤트 발생 시 푸시)은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 웹훅 (Webhook) - REST API의 콜백 역방향 호출 구조 (이벤트 발생 시 푸시)은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
웹훅 (Webhook) - REST API의 콜백 역방향 호출 구조 (이벤트 발생 시 푸시) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
웹훅 (Webhook) - REST API의 콜백 역방향 호출 구조 (이벤트 발생 시 푸시)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
