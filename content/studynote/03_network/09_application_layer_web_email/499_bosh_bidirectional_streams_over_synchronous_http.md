...
> **핵심**: BOSH(Bidirectional-streams Over Synchronous HTTP)는 태생적으로 '단방향(클라이언트가 물어야만 대답함)'인 HTTP 프로토콜의 한계를 뚫고, 마치 **양방향 통신(서버가 원할 때 클라이언트에게 푸시)**이 되는 것처럼 뇌를 속이는(Emulation) HTTP 확장의 일종이다.
> **비유**: BOSH (Bidirectional-streams Over Synchronous HTTP)은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: BOSH (Bidirectional-streams Over Synchronous HTTP)은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
BOSH (Bidirectional-streams Over Synchronous HTTP) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
BOSH (Bidirectional-streams Over Synchronous HTTP)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
