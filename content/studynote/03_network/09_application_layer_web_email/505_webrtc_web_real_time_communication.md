...
> **핵심**: WebRTC(Web Real-Time Communication)는 별도의 앱(Skype 등)이나 브라우저 플러그인을 설치할 필요 없이, **자바스크립트(JS) API 몇 줄만 치면 웹 브라우저(Chrome, Safari)끼리 음성, 영상, 데이터를 실시간으로 직접(P2P) 주고받을 수 있게 만든 HTML5 오픈소스 표준 아키텍처**다.
> **비유**: WebRTC (Web Real-Time Communication) - 플러그인 없는 브라우저간 P2P 오디오/음성 (Google 주도)은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: WebRTC (Web Real-Time Communication) - 플러그인 없는 브라우저간 P2P 오디오/음성 (Google 주도)은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 웹·메일 중심 응용 계층 이해
    ↓
WebRTC (Web Real-Time Communication) - 플러그인 없는 브라우저간 P2P 오디오/음성 (Google 주도) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
WebRTC (Web Real-Time Communication) - 플러그인 없는 브라우저간 P2P 오디오/음성 (Google 주도)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
