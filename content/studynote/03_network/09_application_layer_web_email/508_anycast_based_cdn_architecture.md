...
> **핵심**: 애니캐스트(Anycast)는 '1개의 IP는 1대의 기계만 가진다(Unicast)'는 인터넷의 오랜 상식을 부수고, **지구 반대편에 흩어진 수백 대의 서버(CDN 엣지 노드)가 똑같은 IP 주소 1개를 다 같이 세팅해 놓고 BGP 라우터로 무한 방송(Broadcasting)을 때리는 1:1 근접 라우팅 기법**이다.
> **비유**: Anycast 기반 CDN 설계은(는) 웹·메일 중심 응용 계층 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: Anycast 기반 CDN 설계은(는) HTTP/SMTP, 상태 코드, 캐시 조건을 함께 만족할 때 안정적으로 동작한다.
...
Anycast 기반 CDN 설계은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
