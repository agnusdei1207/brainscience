...
> **핵심**: RMON (Remote Network Monitoring)은 개별 장비의 성능만 묻고 답하던 SNMP의 한계를 넘어, 네트워크 세그먼트(서브넷) 전체의 트래픽 통계와 에러 상태를 원격의 프로브(Probe)가 능동적이고 독립적으로 수집·분석하게 만든 분산형 모니터링 표준 사양이다.
> **비유**: RMON (Remote Network Monitoring) - OSI 1,2계층 통계/에러 모니터링, MIB 내장은(는) DNS와 네트워크 관리 응용 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: RMON (Remote Network Monitoring) - OSI 1,2계층 통계/에러 모니터링, MIB 내장은(는) DNS, 이름 해석, 분산 관리 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 DNS와 네트워크 관리 응용 이해
    ↓
RMON (Remote Network Monitoring) - OSI 1,2계층 통계/에러 모니터링, MIB 내장 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
RMON (Remote Network Monitoring) - OSI 1,2계층 통계/에러 모니터링, MIB 내장은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
