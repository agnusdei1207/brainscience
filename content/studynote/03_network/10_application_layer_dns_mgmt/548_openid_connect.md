...
> **핵심**: OpenID Connect (OIDC)는 '권한(인가)' 위임에만 특화되어 있던 기존 OAuth 2.0 프로토콜 위에, 사용자의 '신원(인증)' 정보를 전달하는 **ID Token(JWT 포맷)** 구조를 얹어 완성한 현대적 분산 인증 표준이다.
> **비유**: OpenID Connect - OAuth 2.0 기반 사용자 식별 프로토콜 (ID Token, JWT)은(는) DNS와 네트워크 관리 응용 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: OpenID Connect - OAuth 2.0 기반 사용자 식별 프로토콜 (ID Token, JWT)은(는) DNS, 이름 해석, 분산 관리 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 DNS와 네트워크 관리 응용 이해
    ↓
OpenID Connect - OAuth 2.0 기반 사용자 식별 프로토콜 (ID Token, JWT) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
OpenID Connect - OAuth 2.0 기반 사용자 식별 프로토콜 (ID Token, JWT)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
