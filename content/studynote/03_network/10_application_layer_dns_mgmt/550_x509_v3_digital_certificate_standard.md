...
> **핵심**: X.509 v3는 인터넷상에서 누군가가 내민 '공개키(Public Key)'가 해커의 것이 아니라 진짜 그 사람/서버의 것임을 보증하기 위해, 절대적으로 신뢰할 수 있는 제3자 기관(CA)이 디지털 서명(도장)을 찍어 발행한 **전자 신분증(디지털 인증서)의 국제 규격**이다.
> **비유**: X.509 v3 디지털 인증서 표준 규격은(는) DNS와 네트워크 관리 응용 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: X.509 v3 디지털 인증서 표준 규격은(는) DNS, 이름 해석, 분산 관리 조건을 함께 만족할 때 안정적으로 동작한다.
...
X.509 v3 디지털 인증서 표준 규격은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
