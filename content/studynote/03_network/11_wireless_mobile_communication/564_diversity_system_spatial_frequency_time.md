...
> **핵심**: 무선 전파는 빌딩에 부딪혀 산산조각 나고 서로 엉키며 소멸하는 치명적인 '페이딩(Fading)' 현상을 겪는다. 다이버시티(Diversity)는 송신측이 똑같은 데이터를 시간, 주파수, 공간을 달리하여 **'여러 번 다르게 복사해서 쏘고'**, 수신측이 이 찌그러진 조각들을 긁어모아 원래의 완벽한 100점짜리 신호로 조립해 내는 확률론적 방어 아키텍처다.
> **비유**: 다이버시티 시스템(Diversity System) - 공간(안테나), 주파수, 시간, 편파(Polarize) 다이버시티은(는) 무선·이동통신 구조 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 다이버시티 시스템(Diversity System) - 공간(안테나), 주파수, 시간, 편파(Polarize) 다이버시티은(는) 셀룰러, 무선 자원, 핸드오버 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 무선·이동통신 구조 이해
    ↓
다이버시티 시스템(Diversity System) - 공간(안테나), 주파수, 시간, 편파(Polarize) 다이버시티 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
다이버시티 시스템(Diversity System) - 공간(안테나), 주파수, 시간, 편파(Polarize) 다이버시티은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
