...
> **핵심**: MIMO(다중 입출력 안테나)의 궁극적 비기인 **공간 다중화(Spatial Multiplexing)**는 주파수 대역폭을 1Hz도 늘리지 않고, 송신 안테나마다 '서로 완전히 다른 데이터 조각'을 동시에 허공에 쏘아 수신단에서 행렬 수학(Matrix)으로 분리해 냄으로써 전송 속도(Capacity)를 안테나 개수만큼 M배로 뻥튀기하는 마법이다.
> **비유**: MIMO 기반 공간 다중화 체계 (V-BLAST 구조 등)은(는) 무선·이동통신 구조 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: MIMO 기반 공간 다중화 체계 (V-BLAST 구조 등)은(는) 셀룰러, 무선 자원, 핸드오버 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 무선·이동통신 구조 이해
    ↓
MIMO 기반 공간 다중화 체계 (V-BLAST 구조 등) 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
MIMO 기반 공간 다중화 체계 (V-BLAST 구조 등)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
