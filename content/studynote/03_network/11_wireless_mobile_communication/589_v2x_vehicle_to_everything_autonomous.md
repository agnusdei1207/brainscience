...
> **핵심**: V2X (Vehicle to Everything)는 자동차가 멍청한 쇳덩이를 넘어 스스로 "인터넷에 연결된 거대한 스마트폰(Connected Car)"이 되어, 주변의 다른 자동차(V2V), 신호등(V2I), 클라우드(V2N), 보행자(V2P)와 1초에 10번씩 텔레파시를 주고받으며 도로의 완벽한 안전을 통제하는 차세대 지능형 교통 시스템(C-ITS)의 신경망이다.
> **비유**: V2X (Vehicle to Everything) - 차량 자율주행, 차량통신 핵심은(는) 무선·이동통신 구조 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: V2X (Vehicle to Everything) - 차량 자율주행, 차량통신 핵심은(는) 셀룰러, 무선 자원, 핸드오버 조건을 함께 만족할 때 안정적으로 동작한다.
...
기초 무선·이동통신 구조 이해
    ↓
V2X (Vehicle to Everything) - 차량 자율주행, 차량통신 핵심 개념 정립
    ↓
표준화·상용화 적용
    ↓
자동화·가상화 연계
...
V2X (Vehicle to Everything) - 차량 자율주행, 차량통신 핵심은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
