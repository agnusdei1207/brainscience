...
> **핵심**: LoRa(Long Range)는 이름 그대로 배터리 교체 없이 수년간 사용할 수 있는 초저전력으로 수 km에서 수십 km까지 데이터를 날려 보내는 LPWAN(저전력 장거리 통신망)의 대표적인 비면허 대역 무선 통신 기술이다.
> **비유**: LPWAN 로라 (LoRa)은(는) IoT·WPAN·엣지 네트워크 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: LPWAN 로라 (LoRa)은(는) 저전력, 센서, 게이트웨이 조건을 함께 만족할 때 안정적으로 동작한다.
...
LPWAN 로라 (LoRa)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
