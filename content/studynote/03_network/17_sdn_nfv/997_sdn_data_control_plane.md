...
> **핵심**: 기존 라우터 장비 내부에서 함께 동작하던 '길을 찾는 뇌(Control Plane)'와 '패킷을 나르는 손발(Data Plane)'을 물리적, 논리적으로 완벽히 분리하는 것이 SDN(Software-Defined Networking)의 핵심 구조다.
> **비유**: SDN 데이터/컨트롤 플레인은(는) SDN·NFV 자동화 네트워크 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: SDN 데이터/컨트롤 플레인은(는) 컨트롤 플레인, 데이터 플레인, 정책 자동화 조건을 함께 만족할 때 안정적으로 동작한다.
...
SDN 데이터/컨트롤 플레인은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
