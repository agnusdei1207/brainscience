...
> **핵심**: 하이브리드 암호 시스템 (Hybrid Cryptosystem)은 대량 데이터의 고속 암호화에 유리한 '대칭키 (Symmetric Key) 암호 방식'과, 키 교환의 안전성 및 인증에 탁월한 '비대칭키 (Asymmetric Key, 공개키) 암호 방식'의 물리적 장점을 융합한 최적의 실용적 암호화 아키텍처이다.
> **비유**: 하이브리드 암호 시스템은(는) 광전송과 차세대 자동화 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 하이브리드 암호 시스템은(는) 광전송, 자동화, 분산 제어 조건을 함께 만족할 때 안정적으로 동작한다.
...
하이브리드 암호 시스템은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
