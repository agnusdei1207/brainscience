...
> **핵심**: IPSec (Internet Protocol Security)의 두 가지 운영 모드인 수송 모드 (Transport Mode)와 터널 모드 (Tunnel Mode)는 각각 '페이로드 보호'와 '전체 패킷 캡슐화'라는 메커니즘을 통해 네트워크 계층에서의 종단 간 (End-to-End) 또는 거점 간 (Site-to-Site) 보안 통신을 구현한다.
> **비유**: IPSec 터널/수송 모드은(는) 네트워크 핵심 빈출 용어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: IPSec 터널/수송 모드은(는) 핵심 용어, 시험 포인트, 상위 개념 조건을 함께 만족할 때 안정적으로 동작한다.
...
IPSec 터널/수송 모드은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
