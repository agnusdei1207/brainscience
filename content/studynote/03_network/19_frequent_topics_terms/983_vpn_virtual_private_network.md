...
> **핵심**: VPN (Virtual Private Network, 가상 사설망)은 터널링 (Tunneling), 암호화 (Encryption), 인증 (Authentication) 기술을 결합하여, 개방된 퍼블릭 인터넷 망을 마치 기업의 안전한 물리적 전용선 (Leased Line)처럼 사용할 수 있게 해주는 논리적 네트워크 확장 인프라다.
> **비유**: VPN (가상 사설망)은(는) 네트워크 핵심 빈출 용어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: VPN (가상 사설망)은(는) 핵심 용어, 시험 포인트, 상위 개념 조건을 함께 만족할 때 안정적으로 동작한다.
...
VPN (가상 사설망)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
