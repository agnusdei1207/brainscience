...
> **핵심**: 2. **가치**: 데이터가 전송 중 변조되지 않았음(무결성)을 보장할 뿐만 아니라, 오직 개인키 소유자만이 서명할 수 있다는 수학적 특성 덕분에 "내가 보내지 않았다"고 발뺌하는 것을 원천 차단하는 부인 방지(Non-repudiation)의 핵심 기술이다.
> **비유**: 전자 서명은(는) 네트워크 핵심 빈출 용어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 전자 서명은(는) 핵심 용어, 시험 포인트, 상위 개념 조건을 함께 만족할 때 안정적으로 동작한다.
...
전자 서명은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
