...
> **핵심**: 서비스 거부 공격 (DoS, Denial of Service)은 타겟 시스템의 네트워크 대역폭, CPU (Central Processing Unit), 메모리, 커넥션 풀 등의 한정된 자원을 악의적인 트래픽으로 고갈시켜 정상적인 사용자의 서비스 접근을 차단하는 가용성 파괴 공격이다.
> **비유**: 서비스 거부 공격 (DoS)은(는) 네트워크 핵심 빈출 용어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 서비스 거부 공격 (DoS)은(는) 핵심 용어, 시험 포인트, 상위 개념 조건을 함께 만족할 때 안정적으로 동작한다.
...
서비스 거부 공격 (DoS)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
