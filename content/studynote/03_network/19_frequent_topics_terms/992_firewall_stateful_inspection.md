...
> **핵심**: 상태 기반 검사 (Stateful Inspection) 방화벽은 들어오고 나가는 네트워크 패킷을 독립적인 조각으로 보지 않고, 연결의 시작부터 종료까지 전체 세션의 '상태(State)'와 문맥을 메모리(State Table)에 기록하여 허용 여부를 결정하는 지능형(3세대) 보안 아키텍처다.
> **비유**: 방화벽 (Stateful Inspection)은(는) 네트워크 핵심 빈출 용어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 방화벽 (Stateful Inspection)은(는) 핵심 용어, 시험 포인트, 상위 개념 조건을 함께 만족할 때 안정적으로 동작한다.
...
방화벽 (Stateful Inspection)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
