...
> **핵심**: WAF (Web Application Firewall, 웹 방화벽)는 일반적인 네트워크 방화벽이 통과시키는 80/443 포트 트래픽 내에서, HTTP/HTTPS 프로토콜의 페이로드(내용)를 심층 검사(DPI)하여 웹 애플리케이션 취약점을 노리는 악성 코드를 차단하는 OSI 7계층 전용 방어 솔루션이다.
> **비유**: WAF (웹 방화벽)은(는) 네트워크 핵심 빈출 용어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: WAF (웹 방화벽)은(는) 핵심 용어, 시험 포인트, 상위 개념 제약 안에서 일관된 제어 규칙을 유지할 때 효과가 난다.
...
WAF (웹 방화벽)은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
