...
> **핵심**: 네트워크 슬라이싱 (Network Slicing)은 단일 물리적 네트워크 인프라 위에, 요구사항(대역폭, 지연시간, 연결 밀도)이 극명하게 다른 여러 개의 독립적인 논리적 가상 네트워크(슬라이스)를 생성하고 격리하는 5G 통신의 핵심 아키텍처 기술이다.
> **비유**: 네트워크 슬라이싱은(는) 네트워크 핵심 빈출 용어 안에서 데이터와 제어 규칙을 질서 있게 맞춰 주는 교통 체계와 같다.
...
**핵심 조건**: 네트워크 슬라이싱은(는) 핵심 용어, 시험 포인트, 상위 개념 조건을 함께 만족할 때 안정적으로 동작한다.
...
네트워크 슬라이싱은(는) 단일 기능으로 끝나지 않고 표준화, 운영 자동화, 고속화·지능화 단계로 확장된다.
