...
> **핵심**: 골드 플래팅(Gold Plating, 금도금)은 프로젝트 관리에서 합의된 요구사항 명세서(Baseline)에 없는 기능이나 과도한 장식을, **개발자나 디자이너가 고객의 승인(CCB) 없이 자신의 재량과 선의(?)로 임의로 추가하는 오버엔지니어링(Over-engineering) 행위**를 말한다.
