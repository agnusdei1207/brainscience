...
> **핵심**: 중재자(Mediator) 패턴은 수많은 객체들이 서로서로 직접 얽혀서 통신(M:N 구조)하는 혼돈의 스파게티 네트워크를, 모든 객체가 오직 한 명의 통제자(중재자)와만 통신하는 **별 모양(Star, 1:N 구조)의 허브 앤 스포크(Hub-and-Spoke) 아키텍처**로 단순화시키는 행동 패턴이다.
