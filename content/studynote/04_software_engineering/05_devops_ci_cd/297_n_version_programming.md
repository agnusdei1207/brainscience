...
> **핵심**: N-버전 프로그래밍은 소프트웨어 설계상의 결함(Bug)에 대비하기 위해, **동일한 요구사항을 바탕으로 서로 다른 팀(또는 다른 언어/알고리즘)이 N개의 독립적인 버전을 개발하고, 런타임에 이들의 결괏값을 다수결(Voting)로 결정하는 '소프트웨어 다중화(Redundancy)' 기법**이다.
