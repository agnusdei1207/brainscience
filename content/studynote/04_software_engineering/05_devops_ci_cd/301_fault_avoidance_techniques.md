...
> **핵심**: 결함 회피(Fault Avoidance)는 시스템이 운영되는 도중에 결함이 발생하더라도 버텨내는 것(결함 허용)이 아니라, 아예 **개발 및 설계 단계부터 결함(Bug, Fault)이 소프트웨어나 하드웨어에 침투할 원인 자체를 원천 봉쇄하고 예방하려는 전통적이고 보수적인 아키텍처 철학**이다.
