...
> **핵심**: 서비스 디스커버리(Service Discovery)는 클라우드/마이크로서비스 환경에서 동적으로 생성되고 소멸하는 수많은 서버 인스턴스들의 **네트워크 위치(IP 주소와 포트)를 자동으로 등록하고 찾아주는(Routing) 거대한 동적 전화번호부(Registry) 시스템**이다.
