...
> **핵심**: 서킷 브레이커(Circuit Breaker)는 마이크로서비스 간 통신에서, 대상 서비스(B)가 정상적으로 응답하지 않을 때 호출하는 쪽(A)이 계속 기다리며 시스템 자원(스레드)을 낭비하지 않도록, **'두꺼비집(회로 차단기)'을 내려 통신을 강제로 끊어버리는 결함 허용(Fault Tolerance) 패턴**이다.
