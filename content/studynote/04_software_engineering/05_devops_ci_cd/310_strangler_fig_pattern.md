...
> **핵심**: 스트랭글러 피그(Strangler Fig) 패턴은 거대하고 낡은 모놀리식(Monolithic) 레거시 시스템을 새로운 마이크로서비스 아키텍처(MSA)로 교체할 때, 시스템을 한 번에 뒤엎지 않고 **야금야금(점진적으로) 새로운 서비스로 덮어씌워 결국 레거시를 고사(Strangulate)시키는 마이그레이션 전술**이다.
