...
> **핵심**: 데이터베이스 퍼 서비스(Database per Service)는 마이크로서비스 아키텍처(MSA)에서 **각 마이크로서비스가 자신만의 전용 데이터베이스를 소유하고, 다른 서비스는 이 DB에 절대 직접 접근할 수 없도록 물리적/논리적으로 완벽히 격리(Isolation)하는 아키텍처 원칙**이다.
