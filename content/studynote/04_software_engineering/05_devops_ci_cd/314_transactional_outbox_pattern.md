...
> **핵심**: 트랜잭셔널 아웃박스(Transactional Outbox) 패턴은 내 데이터베이스의 상태를 변경함과 동시에 외부 메시지 브로커(Kafka 등)로 이벤트를 쏘아야 할 때, 이 **두 가지 이기종(異機種) 작업 간의 데이터 유실과 불일치(Dual Write Problem)를 완벽하게 막아내는 신뢰성 높은 비동기 메시지 발행 패턴**이다.
