...
> **핵심**: 마이크로 프론트엔드(Micro Frontends)는 백엔드의 마이크로서비스 아키텍처(MSA) 철학을 브라우저(프론트엔드)로 확장하여, 거대해진 웹 화면을 **독립적으로 개발, 테스트, 배포 가능한 여러 개의 작은 프론트엔드 조각(앱)들로 쪼개고 이를 런타임에 하나의 화면으로 조립하는 아키텍처**다.
