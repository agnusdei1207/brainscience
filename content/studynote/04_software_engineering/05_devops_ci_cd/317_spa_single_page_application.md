...
> **핵심**: 단일 페이지 애플리케이션(SPA)은 브라우저가 최초 접속 시 딱 한 장의 HTML 껍데기만 다운로드하고, 이후 메뉴를 클릭할 때마다 화면 이동(페이지 깜빡임) 없이 **자바스크립트(JS)가 브라우저 DOM의 내용만 부분적으로 교체하며 앱(Native App)처럼 부드럽게 동작하게 만드는 프론트엔드 아키텍처**다.
