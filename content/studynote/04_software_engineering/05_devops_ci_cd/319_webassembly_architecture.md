...
> **핵심**: 웹어셈블리(WebAssembly, Wasm)는 C, C++, Rust, Go 등으로 작성된 코드를 브라우저에서 **네이티브 앱(Native App)에 가까운 속도(Near-native speed)로 실행할 수 있게 해주는 저수준 바이너리 포맷(바이트코드)**이자 차세대 웹 가상 머신(VM) 표준이다.
