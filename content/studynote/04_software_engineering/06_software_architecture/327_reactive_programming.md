...
> **핵심**: 반응형 프로그래밍(Reactive Programming)은 모든 것(마우스 클릭, 서버 응답, 주식 틱 데이터)을 **'비동기 데이터 스트림(Stream)'**으로 간주하고, 데이터가 변할 때마다 그 변화를 즉각적으로 관찰자(Observer)에게 **전파(Propagation)**하는 선언형(Declarative) 프로그래밍 패러다임이다.
