...
> **핵심**: 코드 리뷰(Code Review)는 개발자가 작성한 소스 코드가 라이브 서버(Production)에 병합(Merge)되기 전에, **다른 동료 개발자들이 코드를 꼼꼼히 뜯어보고 결함, 보안 취약점, 아키텍처 규칙 위반을 짚어내는 집단 지성 기반의 품질 보증(QA) 활동**이다.
