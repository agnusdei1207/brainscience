...
> **핵심**: 동적 분석(Dynamic Analysis)은 소스 코드를 정적으로 눈으로만 훑어보는 것을 넘어, **실제 소프트웨어를 컴파일하고 라이브 서버(또는 테스트 환경)에서 실행(Run)시켜가며 내부의 메모리 사용량, CPU 점유율, 스레드 교착 상태를 모니터링하는 검사 기법**이다.
