...
> **핵심**: 의존성 주입 (DI, Dependency Injection)은 객체 간의 의존관계를 객체 내부에서 직접 생성하지 않고, 외부(컨테이너 또는 호출자)에서 생성된 객체를 "주입(Injection)" 받는 방식으로, 객체 결합도를 낮추고 테스트 용이성과 유연성을 높이는 설계 기법이다.
