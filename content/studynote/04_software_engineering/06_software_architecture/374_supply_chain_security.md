...
> **핵심**: 소프트웨어 공급망 보안(Supply Chain Security)은 소프트웨어 개발에 사용되는 모든 구성 요소(코드, 라이브러리, 도구, 인프라 등)의 무결성과 출처를 보장하고, 악의적인 개입(타이포스쿼팅,_dependency confusion, 사전 설치된 악성코드 등)을事前防止するための総合的対策である。 이는 软件开发의 全 단계(요구 → 개발 → 빌드 → 배포)에 적용된다.
