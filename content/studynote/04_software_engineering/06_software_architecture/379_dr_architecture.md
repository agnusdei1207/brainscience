...
> **핵심**: 재해 복구(Disaster Recovery)는 화산재, 홍수, 전쟁 등 대규모 재해로 인해 주요 데이터 센터가 전소하거나 기능이 마비되었을 때, 시스템을 다른 위치에 복원하여 서비스를 계속 제공할 수 있도록 하는 비즈니스 연속성(BC) 전략의 핵심 구성 요소이다. RTO (Recovery Time Objective)는 "얼마나 빨리 복구해야 하는가", RPO (Recovery Point Objective)는 "얼마나 많은 데이터 손실을 허용하는가"를 정의한다.
