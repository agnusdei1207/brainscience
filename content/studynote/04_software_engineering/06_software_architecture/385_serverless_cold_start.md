...
> **핵심**: 서버리스(Serverless) 아키텍처에서 콜드 스타트(Cold Start)는 함수가 호출되기까지 대기 시간이 발생하는 현상으로, 특히 AWS Lambda, Azure Functions, Google Cloud Functions 등 FaaS (Function as a Service)에서 주요 성능 문제 중 하나이다. 이는 초기 함수 인스턴스 생성 시 코드의 下载, 実行環境 초기화, 依存関係解决 등에 시간이 소요되기 때문이다.
