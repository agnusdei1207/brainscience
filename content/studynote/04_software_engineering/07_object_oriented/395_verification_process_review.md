...
> **핵심**: 검증(Verification)은 소프트웨어 테스팅의 양대 산맥인 V&V(Verification & Validation) 중 하나로, "우리가 시스템을 **설계서와 스펙(Specification)에 명시된 대로** 한 치의 오차 없이 똑같이(올바르게) 만들고 있는가?"를 확인하는 **'개발자 및 엔지니어 관점'**의 기계적 점검 활동이다.
