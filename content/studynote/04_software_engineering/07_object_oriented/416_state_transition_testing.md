...
> **핵심**: 상태 전이 테스트(State Transition Testing)이란 소프트웨어 시스템의 상태(State)와 상태 간 전이(Transition)를 모델링하여, 각 상태에서 시스템이 올바르게 동작하는지, 상태 전이가 예상대로 이루어지는지를 검증하는 블랙박스 테스트 기법이다.
