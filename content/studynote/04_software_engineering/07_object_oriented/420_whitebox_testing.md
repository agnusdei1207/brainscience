...
> **핵심**: 화이트박스 테스트(White-box Testing)란 소프트웨어의 내부 구조, 코드, 로직을分析하여 테스트 케이스를 설계하는 기법이다. 테스트러가 소스코드를 볼 수 있기 때문에 "화이트(흰색)" 박스라 부르며, 구조 기반 테스트(Structure-based Testing)라고도 한다.
