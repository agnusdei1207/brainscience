...
> **핵심**: 구문 커버리지(Statement Coverage)란 화이트박스 테스트에서 테스트 케이스가 코드의 모든 문장(Statement)을 최소 한 번씩 실행하도록 검증하는 지표이다. 가장 기본적인 커버리지 기준으로, 모든 문장이 한 번 이상 실행되면 100% 구문 커버리지를 달성한다.
