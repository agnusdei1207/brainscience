...
> **핵심**: 변경 조건/결정 커버리지(Modified Condition/Decision Coverage, MC/DC)란 화이트박스 테스트에서 각 개별 조건이 전체 결정 결과에 독립적으로 영향을 미침을 증명하는 테스트 커버리지 지표이다. N개의 조건에 대해 최소 N+1개의 테스트로 모든 조건의 독립적 영향을 검증한다.
