...
> **핵심**: 테스트 시나리오(Test Scenario)는 소프트웨어의 특정 기능이나 테스트 가능한 영역을 한 문장으로 표현한 것으로, "무엇을 테스트할 것인가"에 대한 high-levelな 정의이다. 테스트 시나리오는複数の 테스트 케이스의 논리적 그룹으로서, 특정 테스트 목적을为中心로 관련 테스트 케이스들을 묶는 역할을 한다.
