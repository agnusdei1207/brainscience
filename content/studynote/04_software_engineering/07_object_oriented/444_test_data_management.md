...
> **핵심**: 테스트 데이터(Test Data)는 소프트웨어 테스트를 수행하기 위해 시스템에 입력되는 데이터이며, 테스트 데이터 관리(Test Data Management, TDM)는 테스트 데이터를 효과적으로 생성, 구성, 관리, 배포, 폐기하는 전체 프로세스를 의미한다. 품질 높은 테스트 데이터는 테스트의 정확성과 커버리지를 결정하는 핵심 요소이다.
