...
> **핵심**: 부하 테스트(Load Testing)는 시스템에 예상되는 정상 부하 또는 최대 예상 부하를 가하여,系统在给定条件下是否能够正常运行 및 성능 지표(응답 시간, 처리량 등)가 요구사항을 충족하는지를 검증하는 성능 테스트의 한 유형이다. 이는 시스템이日常적으로 경험하는 부하 수준에서 안정적으로 동작하는지를 확인하는 것이 주요 목적이다.
