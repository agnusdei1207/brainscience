...
> **핵심**: 페이크(Fake)는 테스트 더블(Test Double)의 5형제 중 가장 거대하고 진짜에 가까운 존재로, **대본(Stub)대로 앵무새처럼 대답하는 수준을 넘어 내부에 맵(HashMap) 같은 짭짤한 진짜 로직을 품고 있어 실제로 상태가 변하고 동작하는 완벽한 가짜 구현체**다.
