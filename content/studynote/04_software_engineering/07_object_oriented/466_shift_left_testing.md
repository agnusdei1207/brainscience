...
> **핵심**: 시프트 레프트 테스팅(Shift-Left Testing)은 전통적인 소프트웨어 생명주기(요구사항 ➡ 설계 ➡ 코딩 ➡ 테스트(오른쪽))에서 **가장 우측에 처박혀 있던 '테스트'라는 행위를 시간의 축을 거슬러 타임머신을 타듯 코딩 전 설계(왼쪽) 단계까지 강제로 확 당겨와 선제적으로 뚜드려 패는 품질 검증 철학**이다.
