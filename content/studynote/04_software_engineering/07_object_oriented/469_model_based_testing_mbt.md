...
> **핵심**: 모델 기반 테스팅(MBT)은 테스터가 엑셀표에 "1번 누르면 A가 된다"라고 테스트 케이스를 수동으로 타이핑하는 노가다를 없애고, **시스템의 동작 원리가 그려진 상태 머신이나 UML 다이어그램(Model)을 기계(AI/툴)에 통째로 먹여서, 가능한 모든 경로의 테스트 케이스 스크립트를 1초 만에 자동으로 찍어내는 궁극의 테스팅 자동화 기법**이다.
