...
> **핵심**: 위협 모델링(Threat Modeling)은 코드를 1줄도 짜기 전인 설계(Design) 단계에서, 시스템의 아키텍처 구조도(DFD)를 칠판에 펼쳐놓고 화이트해커의 마인드로 빙의하여 **"적(해커)은 어디로, 어떤 취약점을 뚫고, 무엇을 훔쳐 갈 것인가?"를 조직적으로 상상하고 논리적으로 증명해 내는 '보안 브레인스토밍의 끝판왕'**이다.
