...
> **핵심**: 취약한 접근 제어(Broken Access Control)는 사용자가 로그인은 잘했지만(인증 통과), **"네가 감히 남의 글을 지우거나 관리자 메뉴에 들어갈 자격(권한)이 있는가?"를 검증하는 뒷단 로직(인가, Authorization)을 개발자가 멍청하게 빼먹어서 발생하는 최악의 논리적 보안 결함**이다.
