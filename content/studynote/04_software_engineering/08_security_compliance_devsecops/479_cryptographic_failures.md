...
> **핵심**: 암호화 실패(Cryptographic Failures)는 과거 '민감 데이터 노출(Sensitive Data Exposure)'로 불렸던 취약점으로, 시스템에 저장되거나(Data at Rest) 네트워크로 전송되는(Data in Transit) **중요한 개인정보(비밀번호, 신용카드, 주민번호)를 암호화하지 않거나, 낡고 뚫린 암호화 알고리즘(MD5, DES)을 써서 해커에게 평문(Plaintext)으로 헌납하는 대참사**다.
