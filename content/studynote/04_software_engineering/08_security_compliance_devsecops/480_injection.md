...
> **핵심**: 인젝션(Injection)은 시스템이 사용자로부터 입력받은 값(검색어, ID 등)을 순수한 '데이터(Text)'로 처리하지 못하고, 시스템을 제어하는 **'실행 가능한 명령어(Code)'로 멍청하게 오인하여 해커가 던진 악성 쿼리나 OS 명령어를 자기 손으로 실행해버리는 치명적인 논리적 분리 실패**다.
