...
> **핵심**: 안전하지 않은 설계(Insecure Design)는 개발자가 코드를 완벽하고 버그 없이 짰음에도 불구하고, **애초에 시스템을 기획하고 아키텍처 도면을 그릴 때부터 보안(비즈니스 예외, 인증/인가 룰)을 전혀 고려하지 않아 태생적으로 털릴 수밖에 없는 근본적 논리 결함**이다.
