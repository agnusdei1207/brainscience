...
> **핵심**: SSRF(서버 사이드 요청 위조)는 해커가 밖에서 방화벽을 뚫지 않고, **우리 회사 내부망(DMZ)에 있는 웹 서버에게 "야, 너 사내망에 있는 DB 서버나 클라우드 메타데이터 주소(169.254.169.254)로 대신 접속해서 나한테 결과 좀 갖다줘!"라고 악성 URL을 던져 대신 심부름(위조)을 시키는 해킹 기법**이다.
