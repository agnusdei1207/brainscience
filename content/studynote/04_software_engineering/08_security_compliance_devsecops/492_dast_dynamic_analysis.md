...
> **핵심**: DAST(동적 애플리케이션 보안 테스팅)는 소스코드를 까보지 않고, **실제 해커가 밖에서 인터넷을 통해 살아 숨 쉬는 웹 서버(런타임 환경)를 향해 미친 듯이 수만 개의 가짜 폭탄(공격 페이로드)을 쏴보며 서버가 뻗는지(Crash) 확인하는 블랙박스(Black-box) 파괴 공격 훈련**이다.
