...
> **핵심**: 소프트웨어 구성 분석(SCA)은 개발자가 직접 짠 소스 코드를 쳐다보지 않고, **프로젝트 뱃속에 들어있는 '남이 짠 오픈소스 라이브러리 목록(`pom.xml`, `package.json`)'을 엑스레이로 스캔하여 이미 세상에 뚫렸다고 소문난 해킹 취약점(CVE)이나 법적 지뢰(라이선스 위반)가 숨어있는지 색출하는 공급망 방어의 핵심 로봇**이다.
