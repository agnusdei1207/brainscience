...
> **핵심**: XSS 방어는 사용자가 게시판에 몰래 써둔 `<script>해킹</script>` 이라는 무기(독약)를 서버가 지우지 못했다 하더라도, **브라우저 화면에 출력(Output)될 때 꺾쇠(`<`)를 `&lt;` 라는 단순 텍스트 껍데기로 치환(Escaping)하여, 브라우저가 이를 '실행 코드'가 아닌 멍청한 '글씨'로 읽게 만드는 출력 인코딩 철학**이다.
