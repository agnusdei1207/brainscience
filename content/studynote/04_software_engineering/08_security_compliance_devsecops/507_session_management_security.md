...
> **핵심**: 세션 관리(Session Management) 보완은 사용자가 로그인을 통과한 뒤 발급받는 **'놀이공원 자유이용권(Session ID)'이 해커에게 복제되거나 유추당해 내 권한이 통째로 털리는(세션 하이재킹) 참사를 막기 위해, 팔찌의 수명을 극도로 짧게 치고(Timeout) 위조가 불가능한 난수로 찍어내는(Randomness) 런타임 보안의 절대적 기본기**다.
