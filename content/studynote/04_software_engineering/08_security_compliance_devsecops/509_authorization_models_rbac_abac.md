...
> **핵심**: 인가(Authorization) 모델은 사용자가 정문(로그인/인증)을 통과한 뒤 시스템 내부에 들어왔을 때, **"네 직급(Role)이 뭐냐?" 혹은 "지금 접속한 시간과 IP 위치(Attribute)가 어디냐?"라는 깐깐한 조건표를 들이밀며 폴더와 데이터의 열람 권한을 허락하거나 몽둥이로 쫓아내는(403 Forbidden) 내부 통제 아키텍처**다.
