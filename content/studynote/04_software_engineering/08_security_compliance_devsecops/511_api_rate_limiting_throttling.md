...
> **핵심**: 비율 제한(Rate Limiting)과 스로틀링(Throttling)은 선량한 사용자로 위장하여 서버에 초당 1만 번의 무지성 API 폭격을 날려 서버를 질식시키는 해커(DDoS)나 봇(Bot)의 멱살을 잡고, **"1초에 딱 10번까지만 허락한다!"라는 물리적 밸브를 강제로 채워버리는 인프라 레벨의 숨통 제어 아키텍처**다.
