...
> **핵심**: mTLS(Mutual TLS, 상호 TLS 인증)는 클라이언트가 서버의 신분만 묻는 낡은 HTTPS의 일방통행 방식을 파괴하고, **서버와 서버(마이크로서비스)끼리 통신할 때 "너 누구야(클라이언트 인증서)?"와 "넌 누구야(서버 인증서)?"를 쌍방향으로 깐깐하게 대조한 뒤에야 군사급 암호화 터널을 뚫어주는 내부망(Intranet) 최고의 인증 아키텍처**다.
