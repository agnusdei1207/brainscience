...
> **핵심**: 컨테이너 보안은 "도커(Docker)는 가벼워서 편해!"라며 무지성으로 다운받아 돌리던 개발자들의 뺨을 갈기고, 컨테이너가 태어나는 순간(이미지 스캔)부터 살아 숨 쉬는 순간(Non-root), 그리고 갇혀있는 공간(Namespace 샌드박스)까지 **모든 생명주기에 걸쳐 호스트 OS(커널)로 뚫고 나오는 '탈옥(Escape)'을 물리적으로 사형시키는 극한의 인프라 통제술**이다.
