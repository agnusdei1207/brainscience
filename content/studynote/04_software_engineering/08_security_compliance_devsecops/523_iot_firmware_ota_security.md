...
> **핵심**: IoT 기기의 OTA(Over-The-Air) 배포 아키텍처는 스마트 냉장고, 자율주행차 등 전 세계에 흩어진 수백만 대의 깡통 하드웨어에 무선으로 펌웨어(OS/앱)를 업데이트할 때, **해커가 중간에 악성 펌웨어를 섞어 좀비 봇넷으로 만들지 못하도록 '전자서명(Digital Signature)'과 '시큐어 부트(Secure Boot)'를 통해 무결성을 100% 보증하는 인프라 생명선**이다.
