...
> **핵심**: 컴플라이언스 애즈 코드(Compliance as Code)는 "주민번호는 암호화해라, S3 버킷은 열어두지 마라" 같은 수백 페이지짜리 사람의 법률 언어(Word, PDF 문서)를 버리고, 이를 **기계가 1초 만에 읽고 채점할 수 있는 '실행 가능한 텍스트 코드(Rego 등)'로 완벽하게 번역하여 파이프라인에 이식하는 규제 자동화 철학**이다.
