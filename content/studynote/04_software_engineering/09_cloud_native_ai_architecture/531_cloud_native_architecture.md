...
> **핵심**: 클라우드 네이티브 아키텍처(Cloud Native Architecture)는 단순하게 "우리 서버 AWS 샀어!"라고 자랑하는 렌탈(Hosting) 개념이 아니다. **"서버는 언제든 죽을 수 있는 소모품(가축)이다"라는 철학 아래, 앱을 잘게 찢고(MSA), 컨테이너(Docker)로 감싸 1초 만에 복제하며, 모든 인프라를 코드(IaC)로 쥐고 흔드는 '구름(Cloud) 환경에 태생적으로 최적화된 새로운 진화 생명체'를 창조하는 사상**이다.
