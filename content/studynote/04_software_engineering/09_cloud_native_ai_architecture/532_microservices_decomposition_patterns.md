...
> **핵심**: 마이크로서비스 분해 패턴(Decomposition Pattern)은 수백만 줄의 코드가 한 덩어리로 엉겨 붙은 거대 모놀리식(Monolithic) 괴물을, 무작정 난도질하는 것이 아니라 **결합도(Coupling)는 찢어발기되 응집도(Cohesion)는 뭉치게 하여, 독립적으로 배포 가능하고 지들끼리 멱살 잡고 싸우지 않는 50개의 작고 우아한 자율 부대(Service)로 정밀 해부하는 외과 수술 기법**이다.
