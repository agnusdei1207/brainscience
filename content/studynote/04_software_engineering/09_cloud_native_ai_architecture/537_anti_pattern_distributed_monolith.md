...
> **핵심**: 분산 모놀리스(Distributed Monolith)는 "MSA로 쪼개면 멋있겠지!"라는 허영심에 취한 멍청한 아키텍트가, **코드 폴더(컨테이너)만 50개로 화려하게 찢어놨을 뿐 실제로는 공용 DB 1개를 같이 쓰거나 서버끼리 강한 결합(Sync RPC)으로 스파게티처럼 얽혀있어, 단 1개의 부품(서비스)만 고장 나도 50개 전체가 동반 셧다운 되는 최악의 프랑켄슈타인 아키텍처**다.
