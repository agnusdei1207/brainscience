...
> **핵심**: 마이크로서비스 아키텍처(MSA)에서 상대방 서버의 변동하는 IP 주소를 찾아내는 '서비스 디스커버리(Service Discovery)'를 할 때, **주문 서버(클라이언트)가 직접 114 전화번호부(레지스트리)를 뒤져 길을 찾는가(Client-Side), 아니면 중간에 대빵(로드밸런서/프록시)을 두고 "네가 대신 찾아서 연결해!"라고 짬처리를 하는가(Server-Side)의 거대한 인프라 권력 투쟁**이다.
