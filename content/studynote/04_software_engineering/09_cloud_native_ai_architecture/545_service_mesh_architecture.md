...
> **핵심**: 서비스 메시(Service Mesh)는 마이크로서비스 50개가 서로 통신할 때 필요한 잡다한 로직(인증, 암호화, 재시도, 로깅)을 **개발자가 짜는 자바(Java) 소스코드 뱃속에서 완전히 뜯어내어, 앱 컨테이너 바깥의 보이지 않는 인프라(Proxy) 계층으로 100% 밀어내버리는(Offloading) 궁극의 통신 은폐 아키텍처**다.
