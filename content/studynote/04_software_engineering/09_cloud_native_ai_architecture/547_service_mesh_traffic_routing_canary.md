...
> **핵심**: 트래픽 라우팅과 카나리 배포(Canary Release)는 "새벽 2시에 전 서버 끄고 새 코드 올리자"는 낡은 빅뱅(Big Bang) 배포의 공포를 찢어버리고, **서비스 메시(Istio/Envoy)의 인프라 권력을 이용해 실제 라이브 트래픽을 정밀한 수도꼭지처럼 1%, 5%, 10%씩 살금살금 새 서버(V2)로 흘려보는 무중단 무결점 테스트 비행술**이다.
