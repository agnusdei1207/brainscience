...
> **핵심**: 2PC(Two-Phase Commit)는 분산된 2개 이상의 DB를 1대의 통짜 DB처럼 완벽하게 동기화하기 위해, 코디네이터(지휘자)가 모든 DB에 "준비해!(Prepare)" ➡ "쏴!(Commit)"라는 2단계 자물쇠(Lock)를 강제로 채우는 극단적 **무결성(Consistency) 맹신 아키텍처**다.
