...
> **핵심**: 오케스트레이션 사가(Orchestration Saga)는 10개의 마이크로서비스가 얽힌 복잡한 분산 트랜잭션에서, 멍청하게 지들끼리 이벤트를 던지고 핑퐁(Choreography) 치게 놔두지 않고 **중앙에 절대 군주(Orchestrator 서버)를 하나 박아둬서 "결제해!", "재고 까!", "에러 났네? 결제 다시 환불해!"라고 모든 통제와 명령(Command)을 1곳에서 지휘하는 독재 아키텍처**다.
