...
> **핵심**: 웹팩 모듈 페더레이션(Webpack Module Federation)은 마이크로 프론트엔드를 구현하기 위한 궁극의 흑마법 플러그인으로, 브라우저가 실행되는 찰나(Runtime)에 전혀 다른 서버에 배포된 A팀의 React 컴포넌트를 B팀 화면에 마치 내 컴퓨터 로컬 파일을 `import`하듯 투명하게 꽂아 넣는 **다이나믹 코드 로딩(Dynamic Code Loading) 기술**이다.
