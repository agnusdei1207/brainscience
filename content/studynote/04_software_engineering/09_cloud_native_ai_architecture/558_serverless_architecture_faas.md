...
> **핵심**: 서버리스(Serverless/FaaS) 아키텍처는 "서버가 없다"는 사기성 짙은 거짓말이 아니라, **"개발자가 1년 365일 램(RAM), CPU, OS 보안 패치를 관리하던 짐덩어리를 클라우드 벤더(AWS, Google)에게 100% 떠넘겨버리고(Offloading), 오직 순수한 '비즈니스 코드(함수)'만 허공에 띄워 놓는 극강의 인프라 은닉 사상"**이다.
