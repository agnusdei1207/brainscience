...
> **핵심**: 콜드 스타트(Cold Start)는 서버리스(FaaS) 함수가 돈을 아끼기 위해 트래픽이 0일 때 컨테이너를 차갑게 죽여놨다가(Shutdown), 갑자기 **새로운 트래픽이 훅 들어오면 그제야 허겁지겁 새 컨테이너 자리를 뚫고 OS 환경을 세팅하고 뚱뚱한 프레임워크(Spring/Node)를 띄우느라 1~3초간 화면이 멈춰 뻗어버리는 극한의 부팅 딜레이 버그**다.
