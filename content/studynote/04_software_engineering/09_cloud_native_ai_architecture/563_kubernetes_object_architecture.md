...
> **핵심**: 쿠버네티스(Kubernetes, K8s)는 1만 개의 도커 컨테이너를 수동으로 껐다 켰다 하다가 미쳐버린 데브옵스 엔지니어들을 구원하기 위해 구글이 내려준 **'선언적(Declarative) 오토파일럿 항해사'**로, "이렇게 만들어줘!(YAML)" 도면만 틱 던져놓고 자면, 인프라가 스스로 서버 1만 대를 24시간 내내 좀비처럼 멱살 잡아 복구해 내는 신(God)의 통제술이다.
