...
> **핵심**: 쿠버네티스 오퍼레이터(Operator) 패턴은 K8s의 멍청한 기본 기능(파드 죽으면 무지성으로 살려주기)을 넘어, **'인간 DBA(데이터베이스 관리자)의 뇌(도메인 지식)'를 아예 소프트웨어 봇(Controller)으로 코딩해서 K8s 커널 뱃속에 이식해 버리는 무인 자동화 흑마법**이다.
