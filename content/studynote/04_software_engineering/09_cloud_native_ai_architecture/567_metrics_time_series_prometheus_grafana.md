...
> **핵심**: 메트릭(Metrics)은 서버가 토해내는 무겁고 더러운 100만 줄의 텍스트 에러(Log) 쓰레기를 다 지워버리고, 오직 **`현재 CPU 80%`, `초당 결제 응답 5ms`, `500에러 12회 발생` 같은 차갑고 가벼운 '숫자 데이터(수치)'와 '시간(Timestamp)'의 조합만을 극한으로 쥐어짜 압축한 시계열(Time-Series) 심전도 측정 아키텍처**다.
