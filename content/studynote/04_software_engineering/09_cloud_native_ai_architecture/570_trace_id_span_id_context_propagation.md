...
> **핵심**: 분산 추적(569장)이 화살표 궤적을 그리려면 50대의 무상태(Stateless) 깡통 서버들이 "아, 이게 방금 걔가 찌른 그 트래픽이구나!"라고 기억해야 한다. 이를 위해 최초 대문(API Gateway)에서 **단 하나의 고유한 바코드(`Trace ID`)와 현재 구간 번호(`Span ID`)를 찍어내고, 50대 서버가 서로 API를 찌를 때마다 HTTP 헤더(Header)에 이 바코드를 몰래 숨겨서 릴레이 바통 터치하듯 100% 무조건 넘겨주는 강박적 흑마법이 '컨텍스트 전파(Context Propagation)'**다.
