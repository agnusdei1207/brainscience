...
> **핵심**: 벌크헤드(Bulkhead, 격벽) 패턴은 거대한 배(시스템)의 밑바닥을 여러 개의 튼튼한 강철 방(격벽)으로 쪼개어 놓아, **1번 방(상품 추천 API)에 구멍이 나서 물이 꽉 차도 2번 방(결제 API)에는 물 한 방울 스며들지 않게 막아내어 배 전체의 침몰(전사 셧다운)을 완벽히 차단하는 궁극의 자원 물리적 격리술(Isolation)**이다.
