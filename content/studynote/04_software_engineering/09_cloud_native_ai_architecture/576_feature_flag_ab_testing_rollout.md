...
> **핵심**: 피처 플래그(Feature Flag)는 "코드 배포(Deployment)"와 "고객에게 기능 노출(Release)"이라는 한 몸이던 두 단어를 도끼로 완전히 찢어발겨버린 클라우드 시대의 런타임 if문 스위치다. 서버를 재부팅하거나 코드를 다시 젠킨스로 10분씩 말아 올릴 필요 1도 없이, **그저 중앙 대시보드(LaunchDarkly)에서 스위치만 탁! 켜면 앱 소스코드 안의 신규 결제 버튼이 0.1초 만에 살아서 유저 화면에 뿅 튀어나오는 마술**이다.
