...
> **핵심**: 정적 사이트 생성(SSG)은 유저가 접속할 때마다(Runtime) 백엔드 서버가 땀 뻘뻘 흘리며 DB를 찌르고 HTML을 굽는 짓(SSR)을 혐오하며, **"개발자가 코드를 빌드(Build)하는 그 찰나의 순간에 10만 개의 페이지를 10만 장의 완성된 HTML 파일 쪼가리로 미리 다 찍어내어 전 세계 CDN(Edge)에 뿌려두는" 렌더링 서버 0대짜리 극강의 스피드 흑마법**이다.
