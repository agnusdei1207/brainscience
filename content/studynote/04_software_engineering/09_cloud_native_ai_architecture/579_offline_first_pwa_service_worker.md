...
> **핵심**: 오프라인 우선(Offline-first) 아키텍처는 스마트폰이 지하철 터널(네트워크 단절)에 들어가면 500 에러를 뿜으며 공룡 게임을 띄우던 낡은 웹 브라우저의 나약함을 도끼로 부숴버리고, **인터넷이 끊겨도 마치 네이티브 앱(카카오톡)처럼 화면이 1초 만에 뜨고 스와이프 조작이 매끄럽게 돌아가도록 웹과 앱의 경계를 허문 궁극의 생존 UX 기술(PWA)**이다.
