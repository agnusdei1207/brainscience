...
> **핵심**: 웹어셈블리(WASM)는 "자바스크립트(JS) 따위의 느려 터진 텍스트 스크립트 언어로는 3D 렌더링이나 무거운 동영상 편집을 브라우저에서 돌릴 수 없다"는 20년 묵은 태생적 한계를 도끼로 박살 내버린, **C/C++, Rust 언어의 압도적인 바이너리(기계어) 코드를 브라우저 V8 엔진 한가운데서 네이티브 스피드(Native Speed)로 때려 박아 실행시키는 외계인 급 런타임 혁명**이다.
