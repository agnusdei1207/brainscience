...
> **핵심**: 프롬프트 엔지니어링(Prompt Engineering)은 "AI야 결제 모듈 짜줘" 같은 인간의 멍청하고 모호한 지시를 버리고, **"너는 시니어 백엔드 아키텍트야(페르소나). Java 17을 써서(기술 제약), 10만 트래픽 동시성을 방어하는 SAGA 패턴 코드를 작성하고(명확한 목표), 주석을 달아 JSON으로 반환해 줘(출력 형식)"라고 AI의 멱살을 잡고 100점짜리 정답을 강제로 쥐어짜 내는 '기계와의 텔레파시(조련술)' 예술**이다.
