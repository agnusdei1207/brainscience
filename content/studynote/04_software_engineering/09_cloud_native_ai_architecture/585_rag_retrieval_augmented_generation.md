...
> **핵심**: RAG(검색 증강 생성) 패턴은 ChatGPT가 "없는 사실을 지어내는 환각(Hallucination)" 정신병을 막기 위해, 거대 언어 모델(LLM)의 뇌를 포맷하고 질문을 던질 때 **"우리 회사의 1만 장짜리 사내 매뉴얼 DB에서 팩트만 검색(Retrieval)해서 1차로 긁어온 뒤, 그 텍스트를 프롬프트에 쑤셔 박아 넣고 무조건 '이것만 보고' 요약(Generation)해라!"**라고 멱살을 잡는 팩트 기반 강제 주입술이다.
