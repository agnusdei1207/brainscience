...
> **핵심**: RPA(로봇 프로세스 자동화)는 개발자가 수만 줄의 백엔드 API 연동 코드를 짜는 짓을 쿨하게 포기하고, **"API가 안 뚫려있는 20년 전 낡은 관공서 웹사이트나 구닥다리 윈도우 엑셀(ERP) 화면을, 소프트웨어 봇(Bot)이 마치 진짜 사람(Human)처럼 눈(OCR)으로 모니터를 쳐다보고 마우스로 딸깍(Click)거리며 키보드를 치는, 무식하지만 짱 빠른 화면 조종(UI Automation) 흑마법"**이다.
