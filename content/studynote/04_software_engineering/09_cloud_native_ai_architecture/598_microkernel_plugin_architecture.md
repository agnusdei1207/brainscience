...
> **핵심**: 마이크로 커널(Microkernel/Plugin) 아키텍처는 프로그램의 심장부(Core System)를 고작 텍스트만 겨우 쳐지는 빈 깡통 수준으로 극단적으로 깎아내고, **Python 컴파일러, 화면 테마, 깃허브 연동 같은 무겁고 변동성이 큰 모든 기능들을 100% 뗐다 붙였다 할 수 있는 '플러그인(Plug-in)' 모듈로 조각조각 찢어 외부에 주렁주렁 매달아 두는 극강의 확장성(Extensibility) 뼈대**다.
