...
> **핵심**: 정보 은닉(Information Hiding)과 캡슐화(Encapsulation)는 "남의 클래스 뱃속에 있는 내장(변수)을 밖에서 직접 후벼파서(Direct Access) 1줄 고쳤다가, 전사 서버가 셧다운되는 연쇄 폭발(Ripple Effect)을 원천 차단하기 위해, **모든 상태 데이터를 콘크리트 금고(`private`)에 가두고 오직 허락된 대화 창구(`public` 메서드)로만 소통하게 강제하는 철창 방어술"**이다.
