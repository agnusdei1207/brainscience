...
> **핵심**: 컴포넌트(Component)는 단순한 자바 클래스(Class) 파일 하나 쪼가리가 아니다! **"수십 개의 클래스와 로직들을 한 덩어리로 예쁘게 뭉쳐 압축(`.jar`, `.dll`, Docker Image)하여, 우주가 두 쪽 나도 그 자체로 혼자서 컴파일(Compile)되고 독립적으로 AWS 서버에 배포(Deploy)될 수 있는 가장 작고 무자비한 '완제품 레고 블록' 단위"**다.
