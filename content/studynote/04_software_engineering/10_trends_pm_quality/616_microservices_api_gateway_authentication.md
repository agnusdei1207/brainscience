...
> **핵심**: API 게이트웨이 (API Gateway) 인증 통합은 수십, 수백 개의 분산된 마이크로서비스가 개별적으로 인증(Authentication)과 인가(Authorization)를 처리하는 중복과 보안 취약점을 해결하기 위해, 시스템 진입점(단일 관문)에서 인증을 중앙 집중화하는 아키텍처 패턴이다.
