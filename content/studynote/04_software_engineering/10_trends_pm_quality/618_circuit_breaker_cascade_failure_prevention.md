...
> **핵심**: 서킷 브레이커 (Circuit Breaker) 패턴은 원격 서비스 호출 실패율이 임계치를 초과할 때, 대상 서비스로의 요청을 일시적으로 차단(Open)하여 시스템 전체로 장애가 전파되는 연쇄 장애 (Cascading Failure)를 방지하는 회복 탄력성 (Resiliency) 메커니즘이다.
