...
> **핵심**: 이벤트 소싱(Event Sourcing)은 데이터의 최종 상태(Current State)를 데이터베이스에 덮어쓰는(Update/Delete) 대신, 상태를 변경시키는 모든 비즈니스 이벤트(Event)들을 발생 순서대로 덧붙여(Append-only) 저장하는 아키텍처 패턴이다.
