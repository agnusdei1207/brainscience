...
> **핵심**: 모듈러 모놀리스(Modular Monolith)는 단일 프로세스와 단일 데이터베이스(Monolithic)로 배포되지만, 내부 코드는 바운디드 컨텍스트(Bounded Context)에 따라 엄격하게 분리된 모듈(Modular)로 구성되어 도메인 간 강결합을 원천적으로 차단하는 아키텍처 패턴이다.
