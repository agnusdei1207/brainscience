...
> **핵심**: 서버리스 콜드 스타트(Cold Start)는 AWS Lambda 등 FaaS(Function as a Service) 환경에서 함수가 오랫동안 호출되지 않아 유휴(Idle) 상태로 파기된 후, 새로운 요청이 들어왔을 때 컨테이너를 프로비저닝하고 런타임을 초기화하느라 발생하는 **초기 응답 지연(Latency)** 현상이다.
