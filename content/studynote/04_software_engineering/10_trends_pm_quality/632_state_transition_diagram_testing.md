...
> **핵심**: 상태 전이(State Transition) 테스팅은 소프트웨어가 과거의 입력값(History)에 따라 현재의 **상태(State)**를 다르게 기억하고, 특정한 이벤트(Event)가 발생했을 때 어떻게 다음 상태로 변하는지(Transition)를 다이어그램으로 도식화하여 검증하는 블랙박스 테스트 기법이다.
