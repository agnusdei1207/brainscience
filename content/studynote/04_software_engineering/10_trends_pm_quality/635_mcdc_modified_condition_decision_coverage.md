...
> **핵심**: MC/DC(변경 조건/결정 커버리지)는 `if (A && B || C)`와 같은 복잡한 다중 조건문에서, **"다른 변수들은 고정해 둔 채 오직 하나의 변수(Condition)만 True/False로 변경했을 때, 그 단 하나의 변수가 전체 결과(Decision)를 뒤집을 수 있음을 독립적으로 증명"**하는 가장 치밀하고 정교한 화이트박스 테스트 기법이다.
