...
> **핵심**: 뮤테이션 테스트(돌연변이 테스팅)는 개발자가 짜놓은 프로덕션 원본 소스 코드에 **고의로 미세한 오류(돌연변이, Mutation)를 주입해 놓고, 기존에 짜둔 테스트 코드(Unit Test)가 이 오류를 눈치채고 빨간불(Fail)을 뿜어내는지 역으로 감시(Audit)**하는 메타(Meta) 테스팅 기법이다.
