...
> **핵심**: A/B 테스팅은 기존 화면(A안, 대조군)과 새롭게 바꾼 화면(B안, 실험군)을 실제 고객 트래픽에 무작위(Randomized)로 반반씩 노출시켜, **어떤 디자인이나 기능이 회사의 핵심 지표(매출, 클릭률 등)를 더 극적으로 상승시키는지 통계학적으로 검증하는 실전 실험(Experimentation) 기법**이다.
