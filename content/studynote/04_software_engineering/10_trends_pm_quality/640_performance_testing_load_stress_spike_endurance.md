...
> **핵심**: 성능 테스팅(Performance Testing)은 시스템이 단순히 '에러 없이 돌아가느냐(기능)'를 넘어, 수많은 유저가 동시에 접속하는 실제 운영 환경에서도 **'약속된 응답 시간(Latency)과 처리량(Throughput)을 유지하며 터지지 않고 버티느냐(비기능)'**를 시뮬레이션하는 극한의 인프라 검증 기법이다.
