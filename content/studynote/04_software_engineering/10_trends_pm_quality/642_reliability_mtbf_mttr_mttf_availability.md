...
> **핵심**: 시스템의 생명력을 측정하는 지표로, 고장 나기까지 버티는 시간인 **MTTF(평균 무고장 시간)**와, 고장 났을 때 고치는 데 걸리는 시간인 **MTTR(평균 수리 시간)**을 합쳐서, 시스템의 수명 한 사이클인 **MTBF(평균 고장 간격 = MTTF + MTTR)**를 도출하는 인프라 공학의 절대 수학 공식이다.
