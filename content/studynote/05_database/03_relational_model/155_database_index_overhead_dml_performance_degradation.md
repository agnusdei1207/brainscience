...
> **핵심**: 인덱스(Index)는 공짜 마법이 아니다. 데이터를 검색(SELECT)할 땐 날아다니지만, 원본 테이블에 데이터가 추가(INSERT), 수정(UPDATE), 삭제(DELETE)되는 순간, DB 엔진은 **원본 테이블뿐만 아니라 주렁주렁 매달린 수많은 인덱스 '가나다순 장부' 서랍들을 전부 다 열어젖혀서 위치를 다시 고쳐 적어야 하는 끔찍한 이중 노동(DML Overhead)**을 떠안는다.
