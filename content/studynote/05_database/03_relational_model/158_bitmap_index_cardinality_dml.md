...
> **핵심**: 비트맵 인덱스(Bitmap Index)는 B-Tree처럼 값의 크기 순서대로 나뭇가지를 그리는 대신, 컬럼의 종류(성별: 남/여 2가지)만큼 **'지도(Map)'를 쫙 펴놓고 1억 명의 데이터 위치에 자신이 해당하면 1, 아니면 0으로 체크(Bit)해두는 0과 1의 극단적 압축 배열판**이다.
