...
> **핵심**: DB 옵티마이저(Optimizer)는 사용자가 날린 `SELECT` 텍스트 쿼리를 건네받아, 어떻게 디스크(창고) 문을 열고 어떤 인덱스 장부를 타야 가장 싸게(Cost) 그리고 빨리 데이터를 뽑아올지 **10만 개의 조합을 연산하여 단 하나의 '실행 계획(Execution Plan)' 경로를 깎아내는 RDBMS 커널의 절대 지능(Brain) 파이프라인**이다.
