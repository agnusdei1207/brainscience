...
> **핵심**: 데이터 레이크(Data Lake)는 정형(Structured), 반정형(Semi-structured), 비정형(Unstructured) 데이터를 원시(Raw) 형태로 저장하는 중앙 집중식 저장소로, 스키마 온 리드(Schema-on-Read) 방식으로 분석의 유연성을 제공하는 것이 핵심이다.
