...
> **핵심**: 글로벌 보조 인덱스(GSI, Global Secondary Index)는 NoSQL 데이터베이스(예: DynamoDB, Cassandra)에서 메인 파티션 키(PK)가 아닌 다른 속성으로 데이터를 전역적으로 빠르게 검색하기 위해 데이터 자체를 재정렬하여 별도의 파티션에 비동기 복제해 두는 메커니즘이다.
