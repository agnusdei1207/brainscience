...
> **핵심**: 참조(Reference) 패턴은 MongoDB 같은 문서형(Document) NoSQL에서 연관된 데이터(1:N)를 부모 문서 안에 통째로 집어넣는 임베디드(Embedded) 패턴의 한계를 극복하기 위해, **연관 데이터를 별도의 독립된 컬렉션(Collection)에 저장하고 자식 문서의 ID(Object ID)만 부모 문서에 링크(참조) 형태로 저장하는 데이터 모델링 기법**이다.
