...
> **핵심**: 버저닝(Versioning) 패턴은 데이터베이스 설계 시 레코드가 수정(Update)될 때 원본 데이터를 덮어쓰지(Overwrite) 않고, 수정될 때마다 **새로운 버전 번호(v1, v2, v3...)를 매겨 데이터의 전체 변경 이력(History)을 영속적으로 보존**하는 스키마 디자인 패턴이다.
