...
> **핵심**: ELT(Extract, Load, Transform)는 더러운 데이터를 밖에서 먼저 씻고(Transform) 넣었던 과거의 무거운 ETL 방식과 달리, **"일단 수백 테라바이트의 원시 데이터(Raw)를 클라우드 데이터 웨어하우스나 데이터 레이크(S3)에 무식하게 다 부어놓고(Load), 그 안에서 데이터 분석가들이 직접 SQL로 지지고 볶아(Transform) 분석하는 최신 파이프라인 아키텍처"**다.
