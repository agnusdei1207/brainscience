...
> **핵심**: RAG (Retrieval-Augmented Generation, 검색 증강 생성) 패턴은 거대 언어 모델(LLM)이 답변을 생성(Generation)하기 전에, 외부 데이터베이스(주로 벡터 DB)에서 질문과 관련된 최신/사내 전문 지식을 먼저 검색(Retrieval)하여 프롬프트에 주입(Augmentation)하는 AI 시스템 아키텍처다.
