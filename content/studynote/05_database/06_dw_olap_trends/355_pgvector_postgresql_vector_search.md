...
> **핵심**: PGVector는 전 세계적으로 가장 널리 쓰이는 오픈소스 관계형 데이터베이스인 PostgreSQL(포스트그레스큐엘)에 텍스트, 이미지의 의미를 담은 수백 차원의 실수 배열인 **'임베딩 벡터(Embedding Vector)' 데이터를 저장하고 고속으로 코사인/유클리디안 거리를 계산할 수 있게 해 주는 혁명적인 C 언어 기반 확장(Extension) 플러그인**이다.
