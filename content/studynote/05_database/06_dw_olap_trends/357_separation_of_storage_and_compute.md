...
> **핵심**: 스토리지와 컴퓨팅의 분리 현상은 데이터베이스 서버 안에서 하드디스크(Data)와 CPU 연산기(Query)가 본드로 강하게 붙어있던 기존의 결합형(Coupled) 한계를 부수고, 비용이 싼 오브젝트 스토리지(S3 등)에 데이터만 영구 보관한 뒤 쿼리를 던질 때만 CPU 클러스터를 독립적으로 소환해 붙이는 이원화 설계다.
