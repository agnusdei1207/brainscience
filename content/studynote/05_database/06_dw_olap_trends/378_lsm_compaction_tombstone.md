...
> **핵심**: 콤팩션 (Compaction)은 극단적인 쓰기 속도를 위해 데이터를 지우거나 덮어쓰지 않고 새로운 파일(SSTable)로 무한정 쌓아두기만 하는 LSM-Tree 엔진에서, 파편화된 파일들을 백그라운드에서 주기적으로 하나로 뭉쳐(Merge) 중복을 제거하고 정돈해 주는 청소부이자 심장 박동 메커니즘이다.
