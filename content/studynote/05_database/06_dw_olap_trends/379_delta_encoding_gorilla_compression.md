...
> **핵심**: 델타 인코딩 (Delta Encoding)은 시계열 데이터(Time-Series Data)처럼 시간의 흐름에 따라 연속적으로 발생하는 데이터 쌍을 저장할 때, 원본 수치 전체를 저장하지 않고 직전 값과의 '차이(Delta)'만을 기록하여 스토리지 비트(Bit) 수를 극단적으로 줄이는 무손실 압축 기법이다.
