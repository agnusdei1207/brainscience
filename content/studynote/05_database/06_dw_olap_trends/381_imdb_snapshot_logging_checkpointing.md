...
> **핵심**: 메인 메모리 데이터베이스 (IMDB, 예: Redis, SAP HANA)는 마이크로초(μs) 단위의 극한 성능을 내지만 전원이 끊기면 데이터가 증발하는 휘발성(Volatility) 약점을 안고 있다. 이를 극복하기 위해 메모리 상태를 비동기로 영구 디스크에 백업하는 아키텍처가 바로 '스냅샷 로깅(RDB)'과 '명령어 로깅(AOF)'이다.
