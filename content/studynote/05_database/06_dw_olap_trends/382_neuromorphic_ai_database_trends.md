...
> **핵심**: 뉴로모픽 연동형 AI DB는 연산부(CPU/GPU)와 저장부(Memory)가 분리되어 데이터를 주고받느라 막대한 전력을 낭비하는 기존의 폰 노이만 (Von Neumann) 아키텍처를 탈피하여, 인간 뇌의 뉴런(연산)과 시냅스(저장)처럼 두 기능이 물리적으로 융합된 반도체 위에서 동작하는 차세대 검색 엔진이다.
