...
> **핵심**: 영지식 증명(ZKP, Zero-Knowledge Proof)은 증명자(Prover)가 검증자(Verifier)에게 자신이 특정 정보(비밀번호, 잔고 등)를 알고 있거나 특정 조건을 만족한다는 사실을 증명할 때, 그 정보의 '원본 내용(지식)'은 단 1비트도 공개하지 않고 오직 수학적 확률을 통해 참(True)임을 완벽히 입증해 내는 궁극의 암호학 프로토콜이다.
