...
> **핵심**: 머클 트리(Merkle Tree)는 해시 트리(Hash Tree)라고도 불리며, 거래 데이터를 叶노드(Leaf Node)에 놓고, 각 두 노드를 순차적으로 해시하여 上流으로 올라가며 최종 단일 해시값인 머클 루트(Merkle Root)를 도출하는 이진 트리(Binary Tree) 구조이다.
