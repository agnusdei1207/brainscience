...
> **핵심**: MaaS (서비스형 모빌리티)는 파편화된 다양한 교통수단(버스, 지하철, 택시, 렌터카, 킥보드, 자전거)을 클라우드 기반의 단일 디지털 플랫폼으로 통합하여, 사용자가 출발지에서 목적지까지 이동하는 모든 여정(Routing)을 하나의 앱에서 검색, 예약, 결제(One-stop)하게 해주는 패러다임 전환이다.
