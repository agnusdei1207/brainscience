...
> **핵심**: 엣지 AI (Edge AI)는 데이터를 수집해 저 멀리 클라우드 서버(데이터센터)로 보내 AI 연산을 수행하고 결과를 돌려받는 기존의 핑퐁(Round-trip) 아키텍처를 파괴하고, 데이터가 생성되는 가장 끝단의 디바이스(스마트폰, CCTV, 로봇, 차량) 내부에 초경량 신경망 칩(NPU)과 압축된 AI 모델을 직접 심어 **현장에서 즉시 오프라인 추론(Inference)을 끝내버리는 분산 지능 체계**다.
