...
> **핵심**: HMD 기반의 VR/AR 메타버스 환경은 양쪽 눈에 4K 화질을 초당 90번 이상 뿌려야 멀미(Motion Sickness)가 나지 않는다. 이 미친듯한 렌더링 부하를 HMD 안의 조그만 모바일 GPU가 감당할 수 없자, 안경 안에 적외선 카메라를 박아 사용자의 시선(Eye Tracking)을 추적한 뒤, 시선이 꽂히는 정중앙 바늘구멍만 4K로 정밀하게 그리고 주변 시야각은 깍두기 그래픽으로 대충 뭉개버리는 뇌과학 결합 최적화 기법이 바로 **포비티드 렌더링(Foveated Rendering)**이다.
