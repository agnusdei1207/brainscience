...
> **핵심**: 엣지 컴퓨팅(Edge Computing)은 데이터 생성 지점 최근방에서 연산을 수행하고, 포그 컴퓨팅(Fog Computing)은 엣지와 클라우드 사이의 중간 계층에서 집계·전처리를 담당한다. 두 기술은 클라우드 집중화의 지연(Latency)·대역폭(Bandwidth) 병목을 분산으로 해결한다.
