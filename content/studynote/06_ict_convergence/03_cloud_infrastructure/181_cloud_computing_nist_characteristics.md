...
> **핵심**: 클라우드 컴퓨팅은 단순히 '내 파일을 네이버 N드라이브에 올리는 것'이 아니라, 거대한 데이터센터의 하드웨어 자원을 가상화(Virtualization)하여 IT 인프라를 '자산(소유)'에서 수도나 전기 같은 '구독형 유틸리티(임대)'로 완벽하게 전환한 컴퓨팅 제공 패러다임이다.
