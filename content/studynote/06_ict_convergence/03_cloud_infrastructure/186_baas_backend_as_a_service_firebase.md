...
> **핵심**: BaaS (Backend as a Service)는 모바일 앱이나 웹 프론트엔드를 개발할 때 무조건 필요한 필수 뒷단 기능들—로그인(인증), 실시간 데이터베이스(DB), 푸시 알람, 파일 스토리지—을 개발자가 서버를 파고 코딩할 필요 없이 **클라우드 벤더가 API 형태로 통째로 빌려주는(Outsourcing) 서비스**다.
