...
> **핵심**: 가상 머신(VM) 안에서 도는 윈도우나 리눅스(Guest OS)가 하드웨어를 통제하려 할 때, 자신이 가상 현실(매트릭스)에 갇힌 가짜라는 사실을 **전혀 모르게 완벽히 속여 넘기면 '전가상화'**이고, 자신이 가짜 가상 머신임을 **인지시키고 하이퍼바이저와 직접 협력하게 만들면 '반가상화'**다.
