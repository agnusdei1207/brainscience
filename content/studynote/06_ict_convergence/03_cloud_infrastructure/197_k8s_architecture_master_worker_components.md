...
> **핵심**: 쿠버네티스(K8s)의 구조는 철저한 **'지휘관(Master Node)'과 '병사(Worker Node)'의 군대식 수직 계열화 아키텍처**다. 마스터는 오직 머리(두뇌)만 써서 1만 대의 서버에 명령을 내리고, 워커 노드들은 실제 앱(컨테이너)을 몸으로 돌리며 마스터의 명령에 무조건 복종하는 이원화 체계다.
