...
> **핵심**: 파드(Pod)는 쿠버네티스(K8s)가 컨테이너 1개를 직접 조종하지 않고, 1개 또는 긴밀하게 결합된 여러 개의 도커 컨테이너를 **하나의 단단한 캡슐(콩투투리)로 묶어서 하나의 IP 주소와 하드디스크(Volume)를 공유하게 만든 K8s 생태계의 최소 배포(Deployment) 단위**다.
