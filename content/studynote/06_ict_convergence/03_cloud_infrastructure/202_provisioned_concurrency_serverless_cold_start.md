...
> **핵심**: 프로비저닝된 동시성(Provisioned Concurrency)은 접속자가 0명일 때 서버를 아예 폭파시켜(Scale-to-Zero) 돈을 아끼는 서버리스(FaaS/AWS Lambda)의 기본 원칙을 스스로 타협(위반)하여, **일정 개수의 빈 컨테이너를 절대 죽이지 않고 코드를 램(RAM)에 상시 예열(Warm-up)해두는 강제 대기 옵션**이다.
