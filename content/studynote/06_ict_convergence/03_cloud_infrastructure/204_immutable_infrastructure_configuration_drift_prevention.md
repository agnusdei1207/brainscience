...
> **핵심**: 불변 인프라(Immutable Infrastructure)는 라이브 환경에 돌아가는 서버에 원격 접속(SSH)하여 패치를 깔거나 코드를 수정하는 행위를 원천 금지하고, 변경이 필요하면 **서버를 업데이트하는 대신 기존 서버를 폭파시키고 새로운 버전이 구워진 완전한 새 서버(컨테이너/VM 이미지)를 찍어내어 통째로 갈아 끼우는 극단적 교체 패러다임**이다.
