...
> **핵심**: 하이퍼컨버지드 인프라(HCI)는 거대한 전산실을 구성하던 3개의 물리적 파편—서버(컴퓨팅), 전용 스토리지(SAN), 네트워크 라우터—을 박살 내고, 오직 소프트웨어 가상화(SDDC)의 힘을 빌려 **단 1대의 일반 x86 서버 깡통(Appliance) 안에 3가지 기능을 100% 압축시켜 때려 박은 융합 인프라의 종착역**이다.
