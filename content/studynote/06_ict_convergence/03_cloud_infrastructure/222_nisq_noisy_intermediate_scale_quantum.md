...
> **핵심**: NISQ(Noisy Intermediate-Scale Quantum)는 50개에서 수백 개의 큐비트(Qubit)를 다루는 **'현재 진행형(2020년대)의 불완전한 양자 컴퓨터 시대'**를 지칭하는 용어다. 큐비트가 열과 진동(Noise)에 미친 듯이 취약해 자꾸 연산 에러를 뿜어내지만, 이를 완벽하게 고쳐주는 '오류 정정(QEC)' 기술이 아직 개발되지 않은 뼈아픈 과도기적 단계다.
