...
> **핵심**: 그로버 알고리즘(Grover's Algorithm)은 아무 순서 없이 100만 장의 서류가 뒤섞인 난장판(비정렬 데이터베이스) 속에서, 내가 원하는 서류 딱 1장을 일반 컴퓨터처럼 한 장씩 뒤지지 않고 **확률의 파동(진폭 증폭)을 이용해 한 방울의 정답만 화산처럼 솟구치게 끄집어내는 기적의 양자 검색 마술**이다.
