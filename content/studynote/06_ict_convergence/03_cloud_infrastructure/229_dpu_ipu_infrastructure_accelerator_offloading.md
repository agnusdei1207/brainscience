...
> **핵심**: DPU(데이터 처리 장치)와 IPU(인프라 처리 장치)는 클라우드 서버의 메인 뇌(CPU)가 가상화, 네트워크 패킷 쪼개기, 디스크 암호화 같은 '인프라 잡일'을 하느라 체력을 30%나 갉아먹는 비효율(Data Center Tax)을 박살 내기 위해, 아예 **서버 랜카드 쪽에 독립적인 ARM 코어(미니 두뇌)를 박아 넣어 인프라 통제권을 100% 강제 이주시켜버린 최첨단 반도체 아키텍처**다.
