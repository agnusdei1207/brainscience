...
> **핵심**: 뉴로모픽(Neuromorphic) 반도체는 연산(CPU)과 저장(RAM)이 분리되어 핑퐁 치며 전기를 퍼먹는 낡은 폰 노이만 아키텍처를 완전히 부숴버리고, **연산과 저장이 하나의 소자(시냅스)에 뭉쳐서 인간의 뇌신경망 작동 방식을 100% 하드웨어 물리 회로로 모방해 낸 궁극의 생체 모방 AI 칩**이다.
