...
> **핵심**: 탄소 인지 컴퓨팅(Carbon-Aware Computing)은 "어차피 전기를 써야 한다면 전기를 아끼는 것(PUE)을 넘어, **석탄을 태워 만든 '더러운 전기'가 흐르는 지역을 피해 태양광/풍력이 쌩쌩 도는 '깨끗한 전기'가 흐르는 서버 리전(Region)으로 내 쇳덩어리(워크로드)를 빛의 속도로 도망가게(Routing) 만드는 극강의 공간 이동 아키텍처**다.
