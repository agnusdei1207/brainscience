...
> **핵심**: 분산 클라우드(Distributed Cloud)는 "왜 퍼블릭 클라우드는 무조건 멀리 있는 아마존 거대 데이터센터에만 있어야 해?"라는 고정관념을 깨부수고, 아마존이나 구글이 아예 **자기네 클라우드 기계(쇳덩어리와 S/W)를 고객의 공장 지하, 통신사 전봇대 밑(Edge) 등 지구 어디든 물리적으로 배달해 뿌려두면서도(분산), 관리는 본사(중앙) 포털에서 100% 알아서 해주는 확장형 모델**이다.
