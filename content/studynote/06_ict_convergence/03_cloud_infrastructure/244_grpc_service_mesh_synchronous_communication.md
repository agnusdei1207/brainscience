...
> **핵심**: 마이크로서비스(MSA) 시대가 오며 100개로 찢어진 서버(컨테이너)들끼리 서로 말을 걸 때 무겁고 느린 텍스트(JSON/REST API)를 주고받다 병목이 터지자, **데이터를 이진수(Binary)로 극단적으로 압축하는 gRPC와, 통신 길을 알아서 통제해 주는 서비스 메시(Istio/Envoy 프록시)를 융합해 꽂아버린 클라우드 내부 신경망 교체 수술**이다.
