...
> **핵심**: 배치 정규화(Batch Normalization)는 딥러닝의 은닉층(Layer)을 통과할 때마다 데이터의 분포가 제멋대로 비틀어지는 '내부 공변량 변화(Internal Covariate Shift)' 현상을 막기 위해, 매 층마다 데이터를 강제로 평균 0, 분산 1로 깎고 다듬어주는 기계 공학적 조율 장치다.
