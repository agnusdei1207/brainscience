...
> **핵심**: 어텐션(Attention)은 번역기(Seq2Seq)가 100단어짜리 긴 문장을 겨우 1개의 고정된 벡터(Context Vector)로 억지로 압축하려다 앞 단어를 다 까먹어버리는 '병목(Bottleneck) 현상'을 타파하기 위해, 번역할 때마다 **"원본 문장 전체를 다시 쓱 훑어보며, 지금 해석해야 할 단어와 가장 관련이 깊은 단어에만 집중(Attention)하는"** 천재적인 컨닝 기법이다.
