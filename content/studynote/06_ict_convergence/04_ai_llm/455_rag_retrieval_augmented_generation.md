...
> **핵심**: RAG(검색 증강 생성)는 생성형 AI(LLM)가 자신이 모르는 외부 정보나 최신 데이터를 물어볼 때 거짓말(환각)을 뱉는 것을 막기 위해, 질문에 답하기 직전에 회사 내부의 문서나 구글 검색을 먼저 수행하고 그 검색 결과를 AI에게 쥐여주며 "이것만 보고 대답해!"라고 강제하는 아키텍처다.
