...
> **핵심**: 양자화(Quantization)는 딥러닝 모델의 파라미터(가중치)를 담고 있는 엄청나게 무겁고 정밀한 32비트 소수점(FP32) 숫자들을, 아주 가볍고 투박한 8비트 정수(INT8)나 4비트로 강제로 깎아내려(반올림) 모델의 용량과 연산량을 기적적으로 압축하는 기술이다.
