...
> **핵심**: 마르코프 결정 과정(MDP, Markov Decision Process)은 로봇이 미로를 탈출하거나 자율주행차가 도로를 달릴 때, 현재의 "상태(State)"에서 어떤 "행동(Action)"을 해야 가장 큰 "보상(Reward)"을 받을 수 있는지를 수학적으로 계산해 내는 강화학습의 절대적인 뼈대(프레임워크)다.
