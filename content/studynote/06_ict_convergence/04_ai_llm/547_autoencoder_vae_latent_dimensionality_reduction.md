...
> **핵심**: 오토인코더(Autoencoder)는 고차원 입력을 저차원 병목(Bottleneck) 잠재 코드로 압축한 후 원본 재구성을 학습하는 비지도 차원 축소 모델이며, VAE(Variational Autoencoder)는 잠재 공간을 연속 확률 분포(가우시안)로 모델링해 새로운 데이터 생성까지 가능하게 한다.
