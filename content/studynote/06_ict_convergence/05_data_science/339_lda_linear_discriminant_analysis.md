...
> **핵심**: LDA(Linear Discriminant Analysis)는 고차원의 데이터를 저차원으로 깎아낼 때, "정답(클래스)이 같은 데이터끼리는 최대한 똘똘 뭉치게 하고, 정답이 다른 데이터끼리는 최대한 멀리 떨어지게" 축을 긋는 지도 학습(Supervised Learning) 기반의 차원 축소 기법이다.
