...
> **핵심**: K-Means 클러스터링은 정답이 없는 데이터들을 비슷한 특징을 가진 K개의 그룹으로 묶어주는 비지도 학습(Unsupervised Learning)의 대표 알고리즘이며, 엘보우 기법과 실루엣 계수는 "대체 몇 개(K)의 그룹으로 쪼개는 것이 가장 완벽한가?"라는 딜레마의 정답을 찾아주는 수학적 검증 도구다.
