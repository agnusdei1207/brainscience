...
> **핵심**: MCMC(Markov Chain Monte Carlo)는 수학적으로 정확히 풀 수 없는(NP-Hard) 극도로 복잡한 확률 분포나 적분 문제를, 컴퓨터의 난수 발생(Monte Carlo)과 무기억성 상태 전이(Markov Chain)를 이용해 '대충 수만 번 점을 찍어' 정답을 추정(근사)해 내는 샘플링 기법이다.
