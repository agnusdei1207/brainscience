...
> **핵심**: BPE(Byte Pair Encoding)는 인간의 언어(문장)를 인공지능이 이해할 수 있는 조각(Token)으로 자를 때, 글자 단위의 비효율성과 단어 단위의 무한대 사전(Vocabulary) 문제를 해결하기 위해 '자주 같이 쓰이는 글자들을 하나의 서브워드(Subword)로 병합'하는 압축 알고리즘이다.
