...
> **핵심**: 계산 그래프(Computational Graph)는 딥러닝의 수많은 수학 연산(덧셈, 곱셈, 활성화 함수)들을 노드와 간선으로 연결한 설계도이며, 프레임워크가 이 그래프를 먼저 완벽히 그리고 나서 실행할지(지연 실행), 아니면 코드를 한 줄 읽을 때마다 그때그때 실행할지(즉시 실행)를 결정하는 실행 모드 아키텍처다.
