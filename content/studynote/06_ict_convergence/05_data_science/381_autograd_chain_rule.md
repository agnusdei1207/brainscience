...
> **핵심**: 자동 미분(Autograd)은 딥러닝 모델이 뱉어낸 오차(Loss)를 역전파(Backpropagation)할 때, 고등학교 수학 시간에 배운 '연쇄 법칙(Chain Rule)'을 컴퓨터 메모리에 계산 그래프(Computational Graph) 형태로 저장하여 미분값을 자동으로 쪼개서 곱해주는 핵심 엔진이다.
