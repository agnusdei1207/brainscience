...
> **핵심**: 가우시안 혼합 모델(GMM)은 K-Means처럼 데이터를 딱딱하게 무자비하게 쪼개는 대신, 여러 개의 가우시안(정규) 분포가 겹쳐 있다고 가정하고 "이 데이터가 A 분포일 확률은 70%, B 분포일 확률은 30%"라며 부드럽게 소속을 매기는 연성 군집(Soft Clustering) 모델이다.
