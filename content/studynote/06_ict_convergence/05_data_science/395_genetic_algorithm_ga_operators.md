...
> **핵심**: 유전 알고리즘(GA, Genetic Algorithm)은 찰스 다윈의 "적자생존(Survival of the Fittest)" 진화론을 컴퓨터 코드로 옮겨, 무작위로 만든 수만 개의 정답 후보(유전자) 중 가장 훌륭한 놈들만 짝짓기시키고 돌연변이를 일으켜 수만 세대에 걸쳐 가장 완벽한 정답으로 진화시키는 최적화 기법이다.
