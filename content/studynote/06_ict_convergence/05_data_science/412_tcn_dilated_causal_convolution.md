...
> **핵심**: TCN(Temporal Convolutional Network)은 시계열 데이터(시간 흐름)를 예측할 때, 전통적으로 쓰이던 순환 신경망(RNN, LSTM)을 버리고, 이미지 처리에 쓰이는 CNN(합성곱 신경망)의 아키텍처를 시계열 구조에 맞게 완벽히 뜯어고친 딥러닝 모델이다.
