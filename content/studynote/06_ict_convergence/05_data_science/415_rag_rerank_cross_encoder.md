...
> **핵심**: RAG(검색 증강 생성) 시스템에서 리랭커(Re-ranker)는 1차 검색(Vector DB)으로 대충 찾아온 100개의 문서 후보군을, 질문과 문서의 문맥을 아주 깊고 깐깐하게 다시 뜯어보고(크로스 인코더) 진짜 정답이 들어있는 5개만 최종적으로 추려내어 순위를 재배열하는 2차 필터링 엔진이다.
