...
> **핵심**: 차원의 저주(Curse of Dimensionality)는 머신러닝에서 예측 성능을 높이려고 변수(차원)를 수십, 수백 개로 계속 늘리다 보면, 어느 순간 데이터들이 텅 빈 우주 공간에 모래알처럼 뿔뿔이 흩어져(희소성 증가) 인공지능이 어떤 패턴도 찾지 못하고 바보가 되는 치명적인 수학적 함정이다.
