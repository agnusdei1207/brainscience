...
> **핵심**: 배깅(Bagging, Bootstrap Aggregating)은 하나의 데이터셋을 여러 개로 무작위 복원 추출(Bootstrap)하여 각각 독립적인 멍청한 모델들을 수백 개 만들고, 그들의 의견을 다수결(Aggregating)로 합쳐 최종 결정을 내리는 앙상블(Ensemble) 학습의 조상이다.
