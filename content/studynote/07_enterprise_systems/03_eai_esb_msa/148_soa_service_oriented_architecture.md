...
> **핵심**: SOA(Service Oriented Architecture)는 거대한 단일 쇳덩이(모놀리식) 시스템을 해체하여, 독립적인 명세서(WSDL)를 가진 '서비스(Service)' 덩어리로 쪼개고 이를 중앙 미들웨어 버스(ESB) 위에서 레고 블록처럼 조합(Orchestration)하는 설계 철학이다.
