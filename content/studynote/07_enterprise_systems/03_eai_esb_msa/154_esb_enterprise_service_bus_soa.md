...
> **핵심**: ESB (Enterprise Service Bus)는 거대한 전사 기업 인프라에서 수백 개의 낡고 새로운 이기종 서버(ERP, CRM, C언어 메인프레임) 쇳덩이들이 서로 1:1(Point-to-Point) 다이렉트 직통 찌르기로 얽혀 뻗어버리는 스파게티 강결합 지옥을 척살 타파하기 위해 ➔ **정중앙을 관통하는 거대한 1통짜리 '메시지 데이터 버스(Bus 고속도로)'를 뚫어주는 SOA 0순위 지능형 백본 미들웨어(Middleware) 대장 봇**이다.
