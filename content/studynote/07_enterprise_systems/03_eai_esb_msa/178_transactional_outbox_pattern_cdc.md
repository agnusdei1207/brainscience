...
> **핵심**: 트랜잭셔널 아웃박스(Transactional Outbox) 패턴은 마이크로서비스(MSA) 분산 환경에서, **'내 로컬 DB에 데이터를 안전하게 쓰는 행위(Commit)'**와 **'외부 시스템(Kafka 브로커)에 알림 메시지를 던지는 행위(Publish)'**라는 두 개의 이질적인 액션을 절대로 쪼개지지 않는 하나의 덩어리(원자성, Atomicity)로 완벽하게 묶어주는 초강력 동기화 아키텍처다.
