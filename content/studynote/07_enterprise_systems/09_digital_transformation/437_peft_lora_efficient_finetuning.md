...
> **핵심**: PEFT(Parameter-Efficient Fine-Tuning)는 LLM의 전체 파라미터 대신 소수의 추가 파라미터만 학습하여 전체 파인튜닝 대비 메모리·연산 비용을 90%+ 절감하며, LoRA(Low-Rank Adaptation)는 가중치 행렬을 저차원 행렬 곱으로 근사하여 PEFT를 구현하는 가장 대표적 기법이다.
