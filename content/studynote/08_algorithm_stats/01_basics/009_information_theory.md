...
> **핵심**: 클로드 섀넌이 창시한 이론으로, '정보'라는 추상적 개념을 '불확실성을 줄여주는 척도(비트)'로 정량화하여 데이터의 한계를 수학적으로 정의했다.
> **비유**: 타뷸레이션 (Tabulation)은(는) 복잡한 문제를 푸는 절차를 설계도처럼 정리해 두는 것과 같다.
...
**핵심 조건**: 타뷸레이션 (Tabulation)은(는) 입력 규모, 데이터 분포, 정확성 요구, 메모리 계층, 종료 조건이 맞아야 기대 성능을 낸다. 평균 성능만이 아니라 최악 조건과 경계 사례까지 함께 검토해야 한다.
...
타뷸레이션 (Tabulation)은(는) 알고리즘 설계에서 문제를 어떻게 구조화하고 자원을 어떻게 배분할지 결정하는 기초 프레임이다. 기대효과는 정확성과 효율성을 동시에 통제하는 데 있지만, 만능 해법은 아니므로 입력 규모와 제약 조건에 따라 다른 전략과 결합해야 한다. 앞으로는 병렬 처리, 메모리 계층 최적화, 확률적/근사적 기법과의 결합이 핵심 발전 방향이 된다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| 문제 모델링 | 타뷸레이션 (Tabulation)는 어떤 입력 구조와 제약을 상정하는지와 연결 |
