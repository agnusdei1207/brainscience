...
> **핵심**: 클리크(Clique)란 그래프에서 모든 정점이 서로 연결된 완전 부분 그래프(Complete Subgraph)를 말하며 — k-Clique 문제(크기 k 이상의 클리크가 존재하는가?)는 SAT로부터 다항 시간 귀납에 의해 NP-완전임이 증명된다.
> **비유**: 클리크 문제은(는) 풀 수 있는 문제와 검증만 쉬운 문제의 경계를 가르는 난이도 지도를 읽는 것과 같다.
...
**핵심 조건**: 클리크 문제은(는) 문제의 결정 버전, 다항 시간 검증 가능성, 환산 관계, 근사 가능성을 함께 봐야 정확한 난이도 판단이 가능하다.
...
클리크 문제은(는) 어떤 문제가 효율적으로 풀리는지, 검증은 쉽지만 해결은 어려운지, 왜 근사와 휴리스틱이 필요한지를 판단하는 기준을 제공한다. 효과는 난이도를 명확히 분류해 비현실적인 설계를 피하게 하는 데 있지만, 실제 시스템에서는 최악 복잡도만으로 충분하지 않아 평균 성능·입력 구조·근사 품질도 함께 봐야 한다. 앞으로는 매개변수 복잡도, 확률/양자 복잡도와의 연결이 더 중요해진다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| 문제 클래스 | 클리크 문제가 P, NP, NP-Complete, NP-Hard 중 어디에 놓이는지 판단 |
