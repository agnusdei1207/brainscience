...
> **핵심**: 데이터 압축은 *중복성 (Redundancy) 제거* — 무손실은 통계적 중복 제거, 손실은 지각적 불필요 정보 제거라는 근본 원리가 다르다.
> **비유**: 데이터 압축 (Compression)은(는) 불확실성과 정보량을 비트 단위로 재는 저울과 같다.
...
**핵심 조건**: 데이터 압축 (Compression)은(는) 확률 모델, 부호화 길이, 잡음 채널, 손실 허용 범위를 어떻게 정의하느냐에 따라 의미와 한계가 달라진다.
...
데이터 압축 (Compression)은(는) 정보량, 압축 가능성, 전송 한계를 정량화하여 통신과 학습 시스템의 설계 기준을 제공한다. 하지만 실제 시스템에서는 이상적 확률 모델과 실제 잡음 환경 사이에 차이가 있으므로, 수식만이 아니라 구현 비용과 오류 복원력도 함께 판단해야 한다. 앞으로는 대규모 학습 손실 함수, 효율적 부호화, 저전력 통신과 결합되며 중요성이 더 커진다.
