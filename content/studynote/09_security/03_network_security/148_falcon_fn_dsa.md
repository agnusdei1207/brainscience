...
> **핵심**: FALCON (표준명 FN-DSA)은 범용 1등 전자서명인 딜리슘(ML-DSA)과 동일한 '격자(Lattice)' 수학을 기반으로 하지만, 촘촘한 $NTRU$ 격자 구조에 **고속 푸리에 변환(FFT)** 알고리즘을 섞어 서명의 덩치를 극단적으로 쥐어짜 낸 초경량 양자 내성 서명(PQC) 알고리즘이다.
