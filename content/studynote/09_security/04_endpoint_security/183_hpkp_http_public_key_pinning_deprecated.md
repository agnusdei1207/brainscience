...
> **핵심**: HPKP (HTTP Public Key Pinning)는 웹 서버가 클라이언트(브라우저)에게 "앞으로 우리 사이트에 접속할 때는 내가 지정한 공개키(또는 해시)가 포함된 인증서만 믿어라"라고 지시하는 HTTP 응답 헤더 기반의 동적 핀닝(Dynamic Pinning) 보안 표준이었다.
