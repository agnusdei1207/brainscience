...
> **핵심**: 동적 핀닝(Dynamic Pinning, 예: HPKP)은 클라이언트가 처음 접속할 때 서버로부터 인증서 지문을 전달받아 기억(캐싱)하는 방식이었으나, 이 치명적인 자폭 리스크(서버 장애 유발)를 극복하기 위해 등장한 대안 아키텍처가 **인증서 투명성(CT, Certificate Transparency) 로그 기반의 공개 감시망 체계**다.
