...
> **핵심**: mTLS(상호 TLS, Mutual Transport Layer Security)는 웹브라우저 통신처럼 클라이언트가 서버의 신원만 일방적으로 확인하는 기존 단방향 TLS를 넘어, **서버와 클라이언트 양쪽이 모두 암호화된 디지털 인증서를 제시하여 서로의 신원을 100% "상호 검증(Mutual Authentication)"**한 후 암호화 통신을 맺는 강력한 보안 프로토콜이다.
