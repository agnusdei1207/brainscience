...
> **핵심**: CRL(Certificate Revocation List) Scope는 인증기관(CA)이 정지된 인증서의 블랙리스트를 발행할 때, "이 명단 파일 하나에 내가 취급하는 모든 고객의 파기 내역을 다 구겨 넣을 것인가(Complete CRL), 아니면 지역별/목적별로 블랙리스트 파일을 여러 개로 쪼개서 발급할 것인가(Partitioned CRL)"를 결정하는 **블랙리스트 파일의 물리적 커버리지(분할) 아키텍처**다.
