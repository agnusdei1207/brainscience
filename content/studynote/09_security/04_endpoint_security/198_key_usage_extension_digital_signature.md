...
> **핵심**: X.509 인증서의 `Key Usage`(키 용도) 확장 필드는, 내가 가진 공개키/개인키 쌍(Key Pair)이 오직 **'전자서명(digitalSignature)용'인지, '암호화(keyEncipherment)용'인지, 아니면 '자식 인증서를 발급할 도장(keyCertSign)용'인지를 운영체제(OS)와 브라우저에게 엄격하게 강제 통제하는 권한의 명찰(Label)**이다.
