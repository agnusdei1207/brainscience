...
> **핵심**: X.509 인증서의 `Basic Constraints` (기본 제약 조건) 확장 필드는, 발급된 이 인증서의 주인이 평범한 서민(서버/일반인)인지, 아니면 자식 인증서를 도장 찍어낼 수 있는 황족(CA, Certificate Authority)인지를 **`cA = TRUE` 또는 `cA = FALSE` 로 명확히 가르는 PKI 계급 사회의 최상위 신분증 감별 마크**다.
