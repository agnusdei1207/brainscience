...
> **핵심**: SPC(Signed Public Key Challenge)는 마이크로소프트(Microsoft) 등에서 웹 기반 인증서 발급이나 코드 서명을 수행할 때, 보안 모듈 내에 저장된 개인키 증명서 및 클라이언트의 브라우저 기반 공개키 챌린지 생성을 안전하게 원격 인증 서버로 던지기 위해 포맷팅된 "서명된 데이터 덩어리(Blob)" 규격이다.
