...
> **핵심**: Kernel Mode Signing(커널 모드 코드 서명)은 윈도우(Windows) 운영체제의 가장 깊고 강력한 권한을 가진 심장부(Kernel Space)에 설치되는 장치 드라이버나 시스템 파일들이, 마이크로소프트의 엄격한 서명 규격을 통과한 안전한 코드임을 암호학적으로 증명하는 보안 통제 시스템이다.
