...
> **핵심**: DKIM(DomainKeys Identified Mail)은 우리가 주고받는 이메일의 헤더와 본문에 보내는 서버 측(발송자)이 가지고 있는 **비밀키(Private Key)로 수학적 암호 도장(전자 서명)을 쾅 찍어서 메일을 쏘고, 받는 쪽이 이를 검증하도록 하는 이메일 전용 암호화 아키텍처**다.
