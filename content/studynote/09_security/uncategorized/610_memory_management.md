...
> **핵심**: 메모리 관리 보안은 메모리 할당, 접근, 해제의 각 단계에서 발생할 수 있는 취약점(Use After Free, Double Free, Null Pointer Dereference, Memory Leak 등)을防止하여 공격자가 메모리腐败를 利用하여 코드를実行하거나 정보를 탈취하는 것을방어하는 것이다.
