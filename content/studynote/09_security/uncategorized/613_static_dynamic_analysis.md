...
> **핵심**: SAST(Static Application Security Testing)는 코드를 실행하지 않고 소스 코드/바이너리를 분석하여 취약점을 탐지하고, DAST(Dynamic Application Security Testing)는 애플리케이션을 실행한 상태에서 실제 공격 시뮬레이션을 수행하여 취약점을 탐지하는 보안 테스팅 방법론이다.
