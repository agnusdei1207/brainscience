...
> **핵심**: 머신러닝(ML, Machine Learning)은 인간이 규칙(Rule)을 직접 코딩하는 대신, 대량의 데이터(Data)와 정답(Label)을 알고리즘에 주입하면 **기계가 스스로 수학적 패턴(가중치, Weight)을 최적화**하며 예측 함수를 완성하는 인공지능의 핵심 하위 분야다. (Tom Mitchell, 1997)
