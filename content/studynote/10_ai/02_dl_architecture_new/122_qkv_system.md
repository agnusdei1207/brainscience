...
> **핵심**: Q(Query)·K(Key)·V(Value)는 **Attention 연산의 3대 구성 요소**로, Query가 "무엇을 찾는가", Key가 "각 위치의 매칭 키", Value가 "실제 정보"를 담당한다. $\text{Attention}(Q,K,V) = \text{softmax}(\frac{QK^T}{\sqrt{d_k}})V$
