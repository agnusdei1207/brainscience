...
> **핵심**: 스케일링 법칙(Scaling Laws)은 **모델 크기(N)·데이터양(D)·컴퓨팅(C)이 멱법칙(Power Law)에 따라 Loss를 예측 가능하게 감소**시킨다는 OpenAI/DeepMind의 실증적 발견이며, Emergence는 **일정 규모 이상에서 예측 불가능한 새 능력**이 갑자기 나타나는 현상이다.
