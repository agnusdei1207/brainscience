...
> **핵심**: 디퓨전 모델 (Diffusion Model)은 원본 이미지에 노이즈(지지직거리는 점)를 점진적으로 추가해 형태를 완전히 파괴한 뒤, 이 노이즈를 역으로 한 꺼풀씩 걷어내며(Denoising) 무에서 유를 창조하는 완벽한 이미지를 복원해 내는 과정(역확산)을 학습한 생성 모델이다.
