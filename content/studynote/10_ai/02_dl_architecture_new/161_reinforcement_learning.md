...
> **핵심**: 강화 학습 (Reinforcement Learning)은 정답(Label)이 없는 미지의 환경(Environment) 속에서, 인공지능 에이전트(Agent)가 수많은 시행착오와 행동(Action)을 통해 얻는 보상(Reward)을 바탕으로 최종 누적 보상을 극대화하는 최적의 행동 정책(Policy)을 스스로 깨우치는 학습 패러다임이다.
