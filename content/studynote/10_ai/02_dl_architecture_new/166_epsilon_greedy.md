...
> **핵심**: 엡실론-그리디 ($\epsilon$-Greedy)는 강화 학습 에이전트가 "탐험(Exploration)과 활용(Exploitation)의 딜레마"에 빠졌을 때, 우물 안 개구리가 되지 않도록 수학적 확률인 엡실론($\epsilon$) 퍼센트만큼 **강제로 '무작위 변덕(Random)'**을 부리게 만드는 가장 유명하고 실용적인 알고리즘이다.
