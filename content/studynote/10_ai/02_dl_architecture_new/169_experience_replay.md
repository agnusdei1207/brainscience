...
> **핵심**: 경험 재생 (Experience Replay)은 강화 학습 에이전트가 겪은 매 순간의 기억(상태, 행동, 보상, 다음 상태)을 거대한 '메모리 버퍼(Memory Buffer)'에 차곡차곡 저장해 두었다가, 나중에 학습할 때 순서대로 꺼내지 않고 무작위로 섞어서(셔플링) 복습하는 데이터 재활용 훈련 최적화 기법이다.
