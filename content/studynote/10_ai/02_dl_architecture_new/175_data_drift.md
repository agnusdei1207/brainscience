...
> **핵심**: 데이터 드리프트 (Data Drift, Covariate Shift)는 시간이 지남에 따라 머신러닝 모델이 실전(Serving)에서 만나는 '입력 데이터(X)의 통계적 분포'가 과거 실험실에서 훈련(Training)받았던 데이터의 분포와 미묘하게 틀어지며 어긋나는 현상이다.
