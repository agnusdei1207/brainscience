...
> **핵심**: 피처 스토어 (Feature Store)는 머신러닝 모델 훈련(학습)과 실시간 서빙(예측)에 쓰이는 수많은 정제된 특징 데이터(Feature)들을, 전사적으로 중복 개발하지 않고 누구나 꺼내 쓸 수 있도록 중앙에 묶어놓고 캐싱하는 **'데이터의 공용 냉장고(Hub)'** 인프라다.
