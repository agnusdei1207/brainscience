...
> **핵심**: 아파치 에어플로우 (Apache Airflow)는 수십 개의 파편화된 데이터 전처리(ETL/ELT) 작업과 머신러닝 학습 스크립트들이 "누가 먼저 실행되고, 누가 끝난 뒤에 다음 놈이 돌아갈지" 순서를 엮어주는 **워크플로우 오케스트레이션 (Workflow Orchestration) 및 스케줄링 플랫폼**의 황제다.
