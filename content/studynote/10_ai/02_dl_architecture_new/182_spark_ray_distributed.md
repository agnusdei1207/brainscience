...
> **핵심**: 머신러닝의 재료가 되는 테라바이트(TB) 급의 막대한 데이터를 전처리하거나, 수천억 개의 파라미터를 가진 거대 AI 모델을 훈련시킬 때, 컴퓨터 1대의 CPU/RAM 한계(OOM)를 박살 내고 **수백 대의 노드(컴퓨터)를 하나의 거대한 거인처럼 엮어 연산을 병렬로 쪼개어 돌리는 클러스터링 인프라 아키텍처**다.
