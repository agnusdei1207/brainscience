...
> **핵심**: 하이퍼파라미터 오토튜닝(HPO)과 NAS(Neural Architecture Search)는 묶어서 **AutoML(자동화 머신러닝)**이라 불린다. 인간 데이터 과학자가 밤새워 "학습률은 0.01로 해볼까? 신경망 층수는 5층으로 깎을까?" 감으로 찍어대던 수작업 노가다를 버리고, **인공지능을 훈련시키는 완벽한 설정값(레시피) 자체를 또 다른 인공지능이 자동으로 탐색해서 찾아주는 궁극의 메타(Meta) AI 기술**이다.
