...
> **핵심**: 텐서 코어 (Tensor Core)는 엔비디아(NVIDIA)가 GPU 안에 박아 넣은 인공지능 전용 특수 엔진으로, 딥러닝 연산의 99%를 차지하는 **'거대한 $4\times4$ 행렬의 곱셈과 덧셈(FMA, Fused Multiply-Add)'**을 단 1 클럭(Clock) 사이클 만에 쾅 하고 한꺼번에 끝내버리는 깡패 같은 하드웨어 가속기다.
