...
> **핵심**: 오토인코더 (Autoencoder)는 입력 데이터(사진, 소리)를 모래시계처럼 좁아지는 '인코더(Encoder)'를 통해 핵심 진액(잠재 공간 벡터 $Z$)으로 꾹꾹 짓눌러 압축한 뒤, 다시 '디코더(Decoder)'를 통해 원래의 사진과 똑같은 모습으로 부풀려 복원해 내는 **비지도 학습(Unsupervised) 기반의 압축 및 복원 딥러닝 뼈대 구조**다.
