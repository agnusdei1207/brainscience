...
> **핵심**: 액티브 러닝 (Active Learning)은 쓸데없는 데이터 100만 장에 전부 정답표(라벨링)를 다는 무식한 인간의 노가다를 멈추고, AI 모델이 먼저 대충 학습한 뒤 **"나 이 데이터는 헷갈려서 도저히 모르겠으니까, 이것만 인간 선생님이 정답 좀 알려주세요!"**라고 가장 훈련 가치가 높은 핵심 데이터만 스스로 쏙쏙 골라 요청하는 지능형 학습 최적화 파이프라인이다.
