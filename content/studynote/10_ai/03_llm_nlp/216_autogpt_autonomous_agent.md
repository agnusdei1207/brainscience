...
> **핵심**: 자율 에이전트 오토지피티 (AutoGPT)는 질문하면 1번 대답하고 끝나는 기존 챗GPT(수동적 LLM)에 **'메모리(기억)', '웹 브라우저(손발)', '계획 수립(뇌)'이라는 사이보그 신체를 덧붙여**, 거대한 추상적 목표를 주면 스스로 세부 계획을 쪼개고 무한 루프를 돌며 과업을 완수할 때까지 멈추지 않는 **행동하는 AI(Agentic AI)**다.
