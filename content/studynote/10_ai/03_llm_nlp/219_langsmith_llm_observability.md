...
> **핵심**: LangSmith는 거대 언어 모델(LLM) 기반의 복잡한 애플리케이션(특히 LangChain 기반) 내부에서 도대체 무슨 일이 벌어지고 있는지, **프롬프트가 어떻게 변형되고 어떤 툴(Tool)을 썼으며 에러가 어디서 터졌는지 그 보이지 않는 블랙박스의 뇌파 흐름을 1초 단위로 까발려주는 엑스레이(Observability, 관측성) 솔루션**이다.
