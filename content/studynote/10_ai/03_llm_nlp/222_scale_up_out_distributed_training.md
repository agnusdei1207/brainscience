...
> **핵심**: 거대 언어 모델(LLM) 훈련 인프라에서 **스케일 업(Scale-up)은 "제일 비싸고 뇌가 큰 H100 GPU 1대를 억소리 나게 업그레이드하는 것"**이고, **스케일 아웃(Scale-out)은 "싸구려 GPU 1,000대를 끈으로 묶어 하나의 거대한 슈퍼컴퓨터 군단처럼 병렬로 부려 먹는 분산 처리 아키텍처"**다.
