...
> **핵심**: RAGAS (RAG Assessment) 프레임워크는 RAG(검색 증강 생성) 챗봇이 유저의 질문에 대답했을 때, "이 자식이 또 지어낸 소리(환각)를 하는 건가, 아니면 진짜 사내 문서를 보고 똑똑하게 대답한 건가?"를 **인간이 일일이 읽어보지 않고, 똑똑한 심판(LLM-as-a-Judge)을 띄워 0점부터 100점까지 자동으로 수학적 수치(정량 평가)를 매겨주는 품질 보증(QA) 파이프라인**이다.
