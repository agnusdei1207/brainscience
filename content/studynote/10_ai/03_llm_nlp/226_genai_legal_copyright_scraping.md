...
> **핵심**: 생성형 AI의 법적 논쟁은 크게 두 가지 전쟁터로 나뉜다. 첫째는 **"AI를 똑똑하게 훈련(Training)시킬 때 남의 그림이나 글을 허락 없이 긁어다(Scraping) 써도 되는가(공정 이용)?"**이고, 둘째는 **"그렇게 똑똑해진 AI가 버튼 하나 딸깍 눌러 만들어낸 기가 막힌 소설과 그림(생성물)에 인간의 저작권을 인정해 줄 것인가?"**이다.
