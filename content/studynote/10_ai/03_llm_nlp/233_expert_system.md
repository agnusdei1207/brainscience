...
> **핵심**: 전문가 시스템 (Expert System)은 1970~80년대를 지배했던 인공지능의 할아버지 격 아키텍처로, 의사나 법률가 같은 스페셜리스트의 머릿속에 있는 규칙(Rule)들을 싹 다 텍스트로 뽑아내어 **지식 베이스(DB)**에 우겨넣고, **추론 엔진(If-Then 코드)**이 이 규칙들을 따라가며 정답을 찍어내는 지극히 깐깐한 논리 기계다.
