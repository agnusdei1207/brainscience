...
> **핵심**: 상태 공간 트리(State Space Tree)는 인공지능이 퍼즐이나 미로를 풀 때, **"내가 여기서 선택할 수 있는 모든 경우의 수(상태)를 점(Node)으로 찍고, 선택의 흐름을 선(Edge)으로 연결해 우주 끝까지 뻗어나가는 거대한 족보(Tree) 그래프"**를 그려 정답을 찾는 노가다 탐색법이다.
