...
> **핵심**: 객체 탐지(Object Detection)는 이미지 내 모든 객체의 **분류(Classification)와 위치(Localization)**를 동시에 수행하며, 1단계 탐지기(YOLO, SSD)와 2단계 탐지기(R-CNN, Faster R-CNN)의 정확도-속도 트레이드오프가 핵심이다.
