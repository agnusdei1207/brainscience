...
> **핵심**: RAG (Retrieval-Augmented Generation, 검색 증강 생성)는 LLM의 파라미터 지식을 외부 벡터 DB에서 검색한 관련 문서로 보완하며, HNSW (Hierarchical Navigable Small World) 그래프가 대규모 벡터 ANN (Approximate Nearest Neighbor) 검색의 사실상 표준이다.
