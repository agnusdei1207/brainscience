...
> **핵심**: SMT (Statistical Machine Translation, 통계 기계 번역)는 언어 모델·번역 모델·구절 테이블을 별도 구축 후 로그 선형 결합으로 번역하고, NMT (Neural Machine Translation, 신경망 기계 번역)는 인코더-디코더 구조로 엔드투엔드 학습한다.
