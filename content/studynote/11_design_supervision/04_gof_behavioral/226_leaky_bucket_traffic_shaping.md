...
> **핵심**: Leaky Bucket (리키 버킷) 알고리즘은 입력 트래픽의 버스트(Burst) 여부와 무관하게 **일정한 속도(Constant Rate)** 로만 패킷을 출력하는 Traffic Shaping (트래픽 쉐이핑) 알고리즘이다 — 물이 구멍 뚫린 양동이에서 일정 속도로 새어나오는 것처럼.
