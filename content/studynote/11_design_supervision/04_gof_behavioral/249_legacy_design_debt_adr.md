...
> **핵심**: 레거시 시스템 (Legacy System) 의 설계 부채 (Design Debt) 는 과거의 빠른 의사결정이 현재와 미래의 유지보수 비용으로 돌아오는 복리 이자이며, ADR (Architecture Decision Record, 아키텍처 결정 기록) 은 그 의사결정을 가시화하고 맥락을 보존하는 문서 체계다.
