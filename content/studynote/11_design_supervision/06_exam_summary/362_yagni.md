...
```
    ...
    | 구조 안정성 | 원칙 준수 구조는 확장 지점은 최소한으로 두고 반복적으로 나타난 변화만 리팩터링으로 끌어올린다를 기준으로 경계를 유지한다 | 원칙 무시 구조는 책임과 의존이 섞여 규칙이 흐려진다 |
...
4. 추상화 추가 비용이 얻는 안정성보다 크지 않은가?
    ...
    지금 필요하지 않은 기능 배제 (You Aren't Gonna Need It, YAGNI)의 기대효과는 분명하다. 불필요한 설계 비용을 줄이고 실제 문제 해결 속도를 높인다. 다만 미래를 전혀 고려하지 않는 수준으로 오해하면 필요한 확장성까지 놓칠 수 있다. 결국 이 주제는 "현재 검증된 요구만 구현 대상으로 제한하는 "을 오래 버티는 구조 규칙으로 만드는 방법으로 기억하면 된다.
...
| 오버엔지니어링 | 지금 필요하지 않은 기능 배제 (You Aren't Gonna Need It, YAGNI)을 설계하고 감리할 때 함께 보는 연관 개념 |
    ...
    2. 이 약속이 있으면 불필요한 설계 비용을 줄이고 실제 문제 해결 속도를 높인다.
    3. 그래서 규모가 커질수록 현재 검증된 요구만 구현 대상으로 제한하는 일을 더 꼼꼼히 챙겨야 해요.
