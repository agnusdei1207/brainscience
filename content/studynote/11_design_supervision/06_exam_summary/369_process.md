...
```
    ...
    | 구조 안정성 | 구조 적용 상태는 도메인 이벤트를 append-only 로그에 기록하고 필요 시 재생(replay)과 스냅샷으로 상태를 복원한다를 기준으로 경계를 유지한다 | 경계 혼재 상태는 책임과 의존이 섞여 규칙이 흐려진다 |
...
4. 도입 복잡도가 조직 규모와 팀 성숙도에 맞는가?
    ...
    이벤트 소싱 (Event Sourcing)의 기대효과는 분명하다. 감사 가능성, 재현성, 시간 축 분석 능력을 크게 높인다. 다만 이벤트 스키마 진화, 재생 비용, 읽기 모델 구성 난도가 높아 운영 성숙도가 필요하다. 결국 이 주제는 "상태 변화 자체를 원본 사실로 보존하는 "을 오래 버티는 구조 규칙으로 만드는 방법으로 기억하면 된다.
...
| 감사 추적 | 이벤트 소싱 (Event Sourcing)을 설계하고 감리할 때 함께 보는 연관 개념 |
    ...
    2. 이 약속이 있으면 감사 가능성, 재현성, 시간 축 분석 능력을 크게 높인다.
    3. 그래서 규모가 커질수록 상태 변화 자체를 원본 사실로 보존하는 일을 더 꼼꼼히 챙겨야 해요.
