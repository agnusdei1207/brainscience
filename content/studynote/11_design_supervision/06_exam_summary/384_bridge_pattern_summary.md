...
```
    ...
    | 구조 안정성 | 패턴 적용 상태는 추상화 계층이 구현 인터페이스를 참조하고 구체 구현을 위임해 조합을 런타임에 구성한다를 기준으로 경계를 유지한다 | 즉흥 구현 상태는 책임과 의존이 섞여 규칙이 흐려진다 |
...
4. 테스트와 디버깅 관점에서 협력 구조를 설명할 수 있는가?
    ...
    브리지 패턴 (Bridge Pattern)의 기대효과는 분명하다. 추상 기능과 구현 세부를 독립적으로 진화시킨다. 다만 변화 축이 하나뿐인 경우에는 구조가 과하게 느껴질 수 있다. 결국 이 주제는 "두 개의 변화 축을 분리하는 "을 오래 버티는 구조 규칙으로 만드는 방법으로 기억하면 된다.
...
| 플랫폼 추상화 | 브리지 패턴 (Bridge Pattern)을 설계하고 감리할 때 함께 보는 연관 개념 |
    ...
    2. 이 약속이 있으면 추상 기능과 구현 세부를 독립적으로 진화시킨다.
    3. 그래서 규모가 커질수록 두 개의 변화 축을 분리하는 일을 더 꼼꼼히 챙겨야 해요.
