...
> **핵심**: RNN (Recurrent Neural Network)은 데이터의 시간적 흐름(Sequence)을 기억하기 위해 스스로에게 다시 연결되는 순환 구조를 가진 신경망이며, LSTM (Long Short-Term Memory)은 그 기억이 시간이 지남에 따라 소실되는 문제를 해결한 발전형 모델이다.
