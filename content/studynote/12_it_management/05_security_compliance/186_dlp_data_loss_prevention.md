...
> **핵심**: DLP(Data Loss Prevention, 데이터 유출 방지)는 민감정보가 조직 외부로 유출되는 것을 탐지·차단하는 보안 솔루션으로, 엔드포인트(Endpoint)·네트워크(Network)·스토리지(Storage) 세 채널에서 데이터 흐름을 실시간 모니터링하고 정책에 따라 차단·알림·암호화 조치를 수행한다.
