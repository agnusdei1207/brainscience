...
> **핵심**: 클라우드 네이티브(Cloud Native)는 클라우드 환경의 탄력성·자동화·관찰 가능성을 최대한 활용하도록 처음부터 설계된 아키텍처 패러다임으로, MSA (Microservices Architecture, 마이크로서비스 아키텍처)·컨테이너(Container)·CI/CD (Continuous Integration/Continuous Deployment, 지속적 통합/배포)·DevOps의 네 기둥으로 구성된다.
