...
> **핵심**: 서버리스(Serverless)는 개발자가 서버 인프라를 관리하지 않고 비즈니스 로직 코드만 작성하면 되는 아키텍처 패러다임이며, FaaS (Function as a Service, 서비스로서의 함수)가 그 핵심 구현 방식이다. "서버가 없는 것"이 아니라 "서버 관리가 없는 것"이 본질이다.
