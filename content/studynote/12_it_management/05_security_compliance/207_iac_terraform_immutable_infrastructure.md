...
> **핵심**: IaC (Infrastructure as Code, 코드로서의 인프라)는 인프라 구성을 사람이 읽고 버전 관리할 수 있는 코드로 정의하는 패러다임이며, Terraform은 멀티 클라우드를 지원하는 가장 널리 사용되는 IaC 도구다. 불변 인프라(Immutable Infrastructure)는 "수정하지 않고 교체"하는 원칙으로 설정 드리프트(Configuration Drift)를 근본적으로 제거한다.
