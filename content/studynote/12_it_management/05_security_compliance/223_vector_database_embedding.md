...
> **핵심**: 벡터 데이터베이스 (Vector Database) 는 텍스트·이미지·오디오를 고차원 임베딩(Embedding) 벡터로 변환하여 저장하고, ANN (Approximate Nearest Neighbor, 근사 최근접 이웃) 알고리즘으로 의미적으로 유사한 데이터를 밀리초 단위로 검색하는 특수 목적 데이터베이스다.
