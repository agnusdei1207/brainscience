...
> **핵심**: 모델 레지스트리(Model Registry)는 ML 모델의 버전·메타데이터·상태(Staging/Production/Archived)를 중앙 관리하는 Git 같은 저장소이고, 모델 서빙(Model Serving)은 이 모델을 REST API 또는 gRPC로 실시간 추론 서비스로 제공하는 인프라다.
