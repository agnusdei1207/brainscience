...
(Velocity)        (버퍼링/실시간)      (Volume/Variety)     (대규모 병렬 처리)      (Veracity/Value)
                    │                  │                   │                    │
...
미래에는 이 5V 특성 위에 AI가 결합되면서 데이터를 물리적으로 이동시키지 않고도 논리적으로 융합하는 **데이터 패브릭 (Data Fabric)** 사상이 표준이 될 것이다. 또한, LLM(대규모 언어 모델)의 등장으로 비정형 데이터(Variety)의 활용 가치가 극대화됨에 따라, 진실성(Veracity)을 확보하기 위한 데이터 옵스(DataOps) 품질 관리 체계의 중요성은 더욱 커질 것이다.
...
* 스파크 (Apache Spark) | 데이터의 Velocity와 연산 병목을 인메모리 기반으로 해결하는 엔진
