...
> **핵심**: 무상태 프로세스 원칙은 애플리케이션이 각 요청/응답 사이클 간에 상태를 저장하지 않고 실행되며, 필요한 상태는 데이터베이스나 캐시 같은 외부 자원에 저장해야 한다는 12팩터 앱의 제6원칙이다.
> **비유**: 무상태 원칙은"호텔의 짐寄存サービス"와 같다. 지갑과 카드키를 호텔 짐寄存소에預けて（外部 상태 저장）바다 입실하면(무상태 요청 처리), 어느柜台(인스턴스)에서 체크인해도 동일한 절차를 따른다. 만약 한柜台(인스턴스)가故障되어关闭해도(장애) 다른柜台(인스턴스)에서 즉시 업무를 continuity할 수 있다.
