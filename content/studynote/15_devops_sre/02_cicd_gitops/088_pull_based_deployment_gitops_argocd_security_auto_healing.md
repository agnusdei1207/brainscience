...
> **핵심**: 풀 기반(Pull-based) 배포는 클러스터 내부에 설치된 에이전트(예: ArgoCD)가 외부 Git 저장소를 주기적으로 관찰하여, 변경된 배포 명세서를 스스로 끌어와 적용하는 아키텍처다.
> **비유**: 배달원이 집 안까지 들어와 냉장고에 반찬을 넣고 가는 위험한 방식(Push) 대신, 집주인이 우편함에 도착한 레시피를 보고 스스로 요리를 채워 넣는 안전한 방식(Pull)이다.
...
하지만 모든 변경이 Git을 거쳐야 하므로 긴급 상황에서의 우회 대처가 몹시 까다로워진다는 단점이 있다.

---

## 6. 기대효과 및 결론

적으로 Pull-based Deployment는 "보안과 인프라 자동화의 수준을 극대화하기 위해, 인프라 제어권을 사람의 손에서 떼어내어 Git과 내부 로봇에게 온전히 이양하는 아키텍처"다.
