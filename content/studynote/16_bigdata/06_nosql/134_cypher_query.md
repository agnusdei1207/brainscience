...
>(노드)` 문법이 관계를 직관적으로 기술하게 해준다.

> 📝 모범 답안

- **본질**: Cypher는 그래프 패턴을 ASCII 아트처럼 시각적으로 표현하는 선언적 쿼리 언어로, `(노드)-[:관계]-- **가치**: SQL의 복잡한 다중 JOIN을 `(a)-[:KNOWS*1..3]->(b)` 단 한 줄의 가변 길이 패턴으로 표현할 수 있어, 관계 탐색 쿼리 개발 생산성이 극적으로 향상된다.
