...
|:---|:---|:---|:---|
| **Tightly Coupled (강결합)**<br>Shared Memory | 여러 개의 CPU가 하나의 거대한 메인 메모리(RAM)를 공유함 | 모든 CPU가 같은 주소 공간을 보므로, **메모리에 읽고 쓰는 행위 자체**로 고속 통신 수행 | 현대의 멀티코어 데스크탑 CPU (i9, Ryzen) |
| **Loosely Coupled (약결합)**<br>Distributed Memory | 각 CPU 노드가 자기만의 완벽히 독립된 전용 메모리(RAM)를 가짐 | 옆 서버의 메모리를 볼 수 없으므로, LAN 케이블 등을 통한 **메시지 패싱(Message Passing)**으로 통신 | 구글 검색 서버 클러스터, 하둡(Hadoop) 팜 |
