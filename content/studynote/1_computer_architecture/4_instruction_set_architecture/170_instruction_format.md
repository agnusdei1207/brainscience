...
- **概念**: 명령어 형식은 컴퓨터 언어의 '문장 구조'다. 32비트나 64비트라는 종이 한 장 위에 "무엇을(Opcode)", "어디서(Operand)", "어떻게(Addressing Mode)" 적을지 칸을 나누는 규칙이다. 이 칸막이가 얼마나 정교하냐에 따라 CPU는 복잡한 명령어를 척척 알아듣거나, 아주 단순하고 빠르게 연산할 수 있게 된다.
