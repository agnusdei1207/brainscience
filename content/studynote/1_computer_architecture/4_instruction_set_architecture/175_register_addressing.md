...
- **概念**: 레지스터 주소 지정은 '내 손안의 데이터'다. 밥을 먹으러 마트(메모리)에 가는 대신, 이미 식탁 위 그릇(레지스터)에 담긴 음식을 바로 먹는 것과 같다. 명령어에 메모리 번지수 대신 "R1 레지스터를 써라"라고 적어두면, CPU는 0.1ns 만에 해당 비트를 낚아채서 연산을 끝낸다.
- **💡 비유**: 레지스터 주소 지정은 '단축 번호 누르기'와 같다. 수십 자리의 전화번호(메모리 주소)를 일일이 누르는 대신, 미리 저장해둔 단축 번호 1번(R1)만 누르면 즉시 통화(연산)가 연결되는 것과 같은 원리다.
- **등장 배경**: 
