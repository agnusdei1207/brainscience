...
- **概念**: 제어 흐름은 컴퓨터의 '기찻길 스위치'다. 일직선으로 달리던 기차(명령어 흐름)가 갈림길을 만났을 때, 어느 쪽 선로로 갈지 철로를 바꿔주는 역할이다. if 조건문, for 반복문, 함수 호출의 진짜 실체가 바로 이것이다.
- **💡 비유**: 제어 흐름 명령어는 '게임북의 선택지'와 같다. 책을 1페이지부터 순서대로 읽는 것이 아니라, "용과 싸우려면 45페이지로 가시오. 도망치려면 80페이지로 가시오"라는 지시를 보고 책의 페이지(PC 값)를 확 넘기는 것과 완벽히 동일하다.
- **등장 배경**: 
