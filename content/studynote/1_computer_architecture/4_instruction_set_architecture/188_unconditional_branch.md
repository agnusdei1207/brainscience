...
- **概念**: 무조건 분기는 컴퓨터의 '순간 이동 마법'이다. 중간에 어떤 과정이나 조건 확인 없이, "이 줄을 읽는 즉시 500번지로 날아가라!"라는 절대적인 명령이다.
- **💡 비유**: 무조건 분기는 '보드게임의 뱀 주사위'와 같다. 주사위를 굴려 뱀 꼬리에 도착하면, 그 즉시 조건 없이 뱀 머리가 있는 칸(다른 주소)으로 쭈르륵 미끄러져 내려가는(점프) 것과 같다.
- **등장 배경**: 
