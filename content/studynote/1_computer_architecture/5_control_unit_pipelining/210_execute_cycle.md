...
- **개념**: 실행 사이클 (Execute Cycle)은 CPU가 인출하고 해독한 명령어의 종류(산술/논리, 데이터 전송, 제어)에 따라, 지정된 산술 연산을 처리하거나 지정된 데이터의 이동을 실제로 수행하는 마이크로 오퍼레이션의 집합이다.

- **💡 비유**: 요리사가 냉장고에서 재료를 꺼내고 조리법을 모두 테이블에 올려놓은 뒤, 실제로 "도마에서 칼질을 하고 불에 볶아 음식을 완성(상태 변화)"하는 핵심 조리 단계와 같다.
