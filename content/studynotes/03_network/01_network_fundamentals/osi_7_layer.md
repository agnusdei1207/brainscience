...
- [TCP/IP 모델](@/studynotes/03_network/01_network_fundamentals/_index.md) : 실질적 인터넷 표준으로, OSI 모델을 어떻게 최적화하고 통합했는지 비교 분석하기 위한 필수 개념.
- [eBPF 프로그래밍](@/studynotes/15_devops_sre/_index.md) : 현대 클라우드 네트워크에서 커널 네트워크 스택을 우회하여 고성능을 내는 핵심 기술.
- [OSPF 라우팅 알고리즘](@/studynotes/03_network/03_network_layer/routing_algorithms.md) : L3 네트워크 계층에서 최적의 경로를 동적으로 계산하는 구체적인 메커니즘.
- [TLS/SSL 암호화](@/studynotes/09_security/02_cryptography/encryption_algorithms.md) : L6 표현 계층의 보안과 데이터 무결성을 보장하는 핵심 암호화 프로토콜.
- [소켓 프로그래밍(IPC)](@/studynotes/02_operating_system/01_process_management/ipc.md) : L4 계층의 기능을 OS 프로세스 레벨에서 사용할 수 있게 해주는 IPC 기술.
