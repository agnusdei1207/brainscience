...
- [`[ACID & BASE]`](@/studynotes/05_database/01_relational_model/acid.md) : 트랜잭션의 엄격함과 유연함 사이의 철학적 차이.
- [`[Consensus Algorithms (Raft/Paxos)]`](@/studynotes/02_operating_system/01_process_management/ipc.md) : 분산 노드 간에 일관된 상태를 합의하기 위한 핵심 알고리즘.
- [`[Microservices Architecture (MSA)]`](@/studynotes/04_software_engineering/01_sdlc_methodology/msa.md) : 서비스별로 독자적인 분산 DB를 가지며 Polyglot Persistence를 구현하는 아키텍처.
