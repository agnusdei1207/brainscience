...
*   [`[딥러닝 기초 (Deep Learning Basics)]`](@/studynotes/10_ai/01_dl/_index.md) : 기존 머신러닝(특성 추출을 인간이 수행)과 달리, 특성 추출(Feature Extraction) 자체를 스스로 학습하는 인공신경망의 진화 형태.
*   [`[AI 윤리 및 거버넌스 (XAI)]`](@/studynotes/10_ai/03_ethics/ai_governance_ethics.md) : 앙상블 모델의 블랙박스 성질을 해석하여 SHAP, LIME 등으로 신뢰성을 부여하는 기술.
*   [`[데이터 전처리 (Data Preprocessing)]`](@/studynotes/14_data_engineering/03_pipelines/etl_vs_elt.md) : 머신러닝 모델의 성능을 결정짓는 핵심 단계(Garbage In, Garbage Out)로, 결측치 처리 및 스케일링을 다룸.
