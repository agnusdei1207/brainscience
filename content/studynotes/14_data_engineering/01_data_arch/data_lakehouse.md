...
- **[데이터 리니지 (Data Lineage)](@/studynotes/14_data_engineering/02_governance/data_lineage.md)**: 레이크하우스 내의 방대한 데이터가 어떻게 생성되고 변환되었는지 추적하는 거버넌스 도구.
- **[컬럼형 스토리지 (Columnar Storage)](@/studynotes/05_database/01_relational/nosql.md)**: Parquet과 같은 대용량 분석에 특화된 물리적 데이터 저장 방식.
- **[데이터 웨어하우스 (DW)](@/studynotes/14_data_engineering/01_data_arch/_index.md)**: 레이크하우스가 극복하고자 하는 과거의 구조화된 데이터 저장 모델.
