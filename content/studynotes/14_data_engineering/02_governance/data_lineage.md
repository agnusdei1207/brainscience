...
- **[데이터 거버넌스 (Data Governance)](@/studynotes/14_data_engineering/02_governance/_index.md)**: 데이터 리니지가 달성하고자 하는 궁극적인 목표인 데이터 관리 체계.
- **[데이터 레이크하우스 (Data Lakehouse)](@/studynotes/14_data_engineering/01_data_arch/data_lakehouse.md)**: 리니지 추적의 주요 대상이 되는 현대적인 복합 데이터 아키텍처.
- **[그래프 데이터베이스 (Graph DB)](@/studynotes/05_database/01_relational/nosql.md)**: 복잡한 리니지의 노드와 엣지를 가장 효율적으로 저장하고 탐색하는 저장소.
